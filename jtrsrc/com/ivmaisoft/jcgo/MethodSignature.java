...
final class MethodSignature
{
 ...
 String getJavaSignature()
 {
  if (jsign == null)
  {
   ...
   Enumeration en = elements();
   while (en.hasMoreElements())
    sb.append(((ExpressionType) en.nextElement()).getJavaSignature());
   sb.append(')');
   jsign = sb.toString();
  }
  return jsign;
 ...
 public String csign(ClassDefinition ourClass, String classCName)
 {
  if (csign == null)
  {
   ...
   csign = Main.dict.nameMapper.methodToCSign(id.equals("<init>") ? "this" :
            id, getJavaSignature(), sb.toString(), ourClass != null ?
            ourClass.countHiddenMethods(signatureString()) : 0, classCName);
  }
  return csign;
 }
 ...
 int match(MethodSignature msig, ClassDefinition forClass)
 {
  ...
  for (int i = 0; i < size; i++)
  {
       ...
       actparmSize != Type.CLASSINTERFACE)
   {
    ...
    {
     if (actparm.signatureDimensions() != formal.signatureDimensions())
      return -1 >>> 1;
     if (actparmSize == formalSize)
      continue;
     if (actparmSize > formalSize || actparmSize == Type.BOOLEAN ||
    ...
    else
    {
         ...
         actparmSize == Type.CLASSINTERFACE)
     {
      ...
      {
       depth = formalClass.getImplementedByDepth(actparmClass, forClass);
       if (depth > 0)
       {
        res += depth + 1;
        continue;
      ...
      {
       depth = actparmClass.getImplementedByDepth(formalClass, forClass);
       if (depth > 0)
       {
        res += depth + 1;
        continue;
      ...
      else if (actparm.signatureDimensions() > 0)
      {
       ...
       {
        if (actparmSize != Type.NULLREF || formalSize != Type.CLASSINTERFACE)
         return -1 >>> 1;
        res += 0x1000;
       }
    }
