...
final class MethodDefinition
{
 ...
 MethodDefinition(Context c, String id, int modifiers, ExpressionType resType,
   Term paramList, Term throwsList, Term body)
 {
  ourClass = c.currentClass;
  this.id = id;
  if ((modifiers & (AccModifier.PRIVATE | AccModifier.PROTECTED |
 ...
 private MethodDefinition(ClassDefinition ourClass, String id, int modifiers,
   ExpressionType resType, Term paramList, ObjVector thrownClasses, Term body,
   boolean copied)
 {
  this.ourClass = ourClass;
  this.id = id;
 ...
 {
  return new MethodDefinition(ourClass, id, isProxyClass ?
          AccModifier.PUBLIC | AccModifier.FINAL | AccModifier.SYNTHETIC :
          modifiers, resType, paramList, isProxyClass ?
          copyCheckedExceptions(ourClass) : thrownClasses, new Block(),
          !isProxyClass);
 }
  ...
  if (size > 0)
  {
   ...
   for (int i = 0; i < size; i++)
   {
    ...
    if (!cd.isUncheckedException(forClass))
    {
     ...
     while (j-- > 0)
     {
      ClassDefinition cd2 = (ClassDefinition) newThrownClasses.elementAt(j);
 ...
 void intersectThrowsWith(MethodDefinition md2)
 {
  ...
  {
   ClassDefinition cd = (ClassDefinition) thrownClasses.elementAt(i);
   if (md2.thrownClasses.size() <= i || md2.thrownClasses.elementAt(i) != cd)
   {
    ...
    while (en.hasMoreElements())
    {
     ...
     if (cd.isAssignableFrom(cd2, 0, ourClass) &&
         (maxClass == null || cd2.isAssignableFrom(maxClass, 0, ourClass)))
      maxClass = cd2;
    }
    if (maxClass != null)
     thrownClasses.setElementAt(maxClass, i);
     else thrownClasses.removeElementAt(i);
   }
  }
 }
 ...
 void setVirtual()
 {
  if (!isVirtual && used && allowVirtual())
  {
   isVirtual = true;
   if (!isFinal())
    ourClass.setVirtualInSubclasses(methodSignature().signatureString());
  }
 }
   ...
   if (isClassMethod() || isConstructor())
    ...
    else
    {
     if (!ourClass.hasInstances())
     {
      ...
      {
       if (!isAbstract() && allowOverride() && !ourClass.hasExactInstance() &&
           ourClass.overriddenByAllUsed(this))
       {
        ...
        else cnt = markUsedThisOnly();
      }
    }
   if (!isConstructor() && allowOverride())
   {
    cnt += receiverClass.markMethodInSubclasses(
 ...
 int markUsedThisOnly()
 {
  ...
  if (id.equals(Main.dict.failOnMethodId) &&
      (Main.dict.failOnClassName == null ||
      ourClassName.equals(Main.dict.failOnClassName)))
  ...
  if (ourClassName.equals(Names.JAVA_LANG_CLASS))
  {
   if (isExactMatch(false, Names.JAVA_LANG_CLASS, 1,
    ...
    else if (ourClassName.equals(Names.JAVA_LANG_ENUM) && isExactMatch(true,
             Names.JAVA_LANG_ENUM, 0, Names.SIGN_VALUEOF_ENUM))
 ...
 void checkMarkedUsed()
 {
  if (innerUsed)
  {
   if (used)
   {
    if (!usedExact && (ourClass.hasExactInstance() ||
        !ourClass.overriddenByAllUsed(this)))
     markUsedThisOnly();
   }
    else
 ...
 {
  singleCaller = singleCaller == null && callerMethod != null ?
                  callerMethod : this;
 ...
 {
  return Integer.toString(id.equals("<clinit>") ? ++ourClass.nextClinitLabel :
          id.equals("<init0>") ? ++ourClass.nextInitZLabel : ++nextLabel);
 }
 ...
 {
  return (modifiers & (AccModifier.PRIVATE | AccModifier.STATIC |
          AccModifier.FINAL)) == 0 && !ourClass.isFinal();
 }
 ...
 boolean isGetterSetter()
 {
  ...
  return id.startsWith("get") ? s1 != Type.VOID && !paramList.notEmpty() :
          ...
          !paramList.notEmpty() : id.startsWith("set") && (s1 == Type.VOID ||
          s1 == Type.BOOLEAN) && paramList.tokenCount() == 1;
 }
 ...
 private MethodDefinition replaceMethodEntry()
 {
  if (!isNative())
  {
   ...
   if (md != null && md != this && md.usedExact() && !md.hasParameters() &&
       resType == md.resType && !md.isClassMethod() &&
       !md.isAbstract() && md.isSynchronized() == isSynchronized())
 ...
 ExpressionType actualExprType()
 {
  ...
  int s1;
  if (!usedExact || isConstructor() ||
      ((s1 = resType.objectSize()) != Type.CLASSINTERFACE &&
      s1 != Type.OBJECTARRAY))
   return resType;
  if (isAbstract())
  {
 ...
 void setActualType(ExpressionType actual, Term t)
 {
  ...
  if (s0 == Type.NULLREF || s0 >= Type.CLASSINTERFACE)
  {
   if (actualType != null)
   {
    actualType = ClassDefinition.maxCommonExprOf(actualType, actual,
                  ourClass);
    if (!ClassDefinition.isAssignableFrom(resType, actualType, ourClass))
     actualType = resType;
   }
    else actualType = actual;
  ...
  if (s0 == Type.CLASSINTERFACE)
  {
   ...
   if (classLiteralValue == null &&
       (resType.name().equals(Names.JAVA_LANG_CLASS) ||
       resType.receiverClass().superClass() == null))
 ...
 {
  return methodSignature().getJavaSignature() + (isConstructor() ?
          Type.sig[Type.VOID] : resType.getJavaSignature());
 }
 ...
 String csign()
 {
  return methodSignature().csign(allowVirtual() && !ourClass.isInterface() ?
          ourClass : null, isClassMethod() || isConstructor() ?
          ourClass.castName() : null);
 }

 boolean isExactMatch(boolean isStatic, String exprTypeName, int exprTypeDims,
   String sigString)
 {
 ...
 private void locateHelperMethod()
 {
  ...
  if (helperClass != null)
  {
   ...
   if (id.equals("<clinit>"))
    helperMethod = helperClass.getMethodNoInheritance(
                    new MethodSignature("_clinit", EMPTY_VECTOR));
    else
    {
     helperMethod = helperClass.getMethodNoInheritance(methodSignature());
     if (helperMethod == null)
     {
      helperMethod = helperClass.getMethodNoInheritance(
                      new MethodSignature(id, EMPTY_VECTOR));
      if (helperMethod == null && isNative())
       helperMethod = helperClass.getMethodNoInheritance(
                       new MethodSignature("_native", EMPTY_VECTOR));
      if (helperMethod == null)
       helperMethod = helperClass.getMethodNoInheritance(
                       new MethodSignature("_", EMPTY_VECTOR));
     }
    }
  }
 }
 ...
 void setMethodBranchFrom(Context c)
 {
  ...
  if (methodBranch != null)
  {
   ...
   else methodBranch = c.saveBranch();
 }
 ...
 boolean discoverObjLeaks()
 {
  ...
  if (isNative())
  {
   if (!ourClass.name().equals(Names.JAVA_LANG_VMOBJECT) ||
       !isExactMatch(true, Names.JAVA_LANG_OBJECT, 0, Names.SIGN_CLONE0))
   {
    if (!isClassMethod() || !ourClass.name().equals(
        Names.JAVA_LANG_VMSYSTEM) || resType.objectSize() != Type.VOID ||
        !methodSignature().signatureString().equals(Names.SIGN_ARRAYCOPY))
    {
     leaksDiscoverDone = true;
     return false;
    }
  ...
  if (!leaksDiscoverDone)
  {
   ...
   if (isArrayCopy)
    paramList.getTermAt(1).getTermAt(1).getTermAt(0).setObjLeaks(
     VariableDefinition.WRITABLE_ARRAY_VAR);
 ...
 boolean copyObjLeaksTo(Term argsList)
 {
  ...
  if (!leaksDiscoverProcessing && !leaksDiscoverDone && isClassMethod())
  {
   ...
   int i = parmSig.size();
   while (i-- > 0)
    if (((ExpressionType) parmSig.elementAt(i)).objectSize() >=
        Type.CLASSINTERFACE)
     break;
   if (i < 0)
    return true;
 ...
 void setWritableArray()
 {
  ...
  if (retWritableArrays != EMPTY_VECTOR)
  {
   if (retWritableArrays != null)
   {
    ...
    while (en.hasMoreElements())
     ((VariableDefinition) en.nextElement()).setWritableArray(null);
   }
    else retWritableArrays = EMPTY_VECTOR;
  }
 }
 ...
 boolean allowStackObjRet()
 {
  return (!usedExact || (returnsCnt == 1 &&
          resType.objectSize() >= Type.CLASSINTERFACE && discoverObjLeaks() &&
          stackObjRetTerm != null)) && !isVMSpecialMethod();
 }
 ...
 static void setRequiredStackObjRets(ObjQueue stackObjRetCalls)
 {
  ...
  {
   MethodDefinition md =
    ((MethodInvocation) en.nextElement()).getNoLeaksScopeMethod();
 ...
 {
  if (isConstructor())
   return (hasParameters() && allowSingleInline()) || allowBodyInline();
  if (isNative())
   return allowSingleInline() && !Names.isVMCoreClass(ourClass.name());
  return ((!isClassMethod() && !ourClass.hasRealInstances()) ||
          allowSingleInline() || (allowBodyInline() &&
          !ourClass.isProxyClass())) && !isVMSpecialMethod();
 }

 private boolean allowBodyInline()
 {
  return body.allowInline(paramList.tokenCount() + MAX_INLINE +
          (isClassMethod() || isConstructor() ? (needsTrigClinit &&
          ourClass.classInitializerNotCalledYet() ? -2 : 0) : 1) -
          (isSynchronized() && resType.objectSize() != Type.VOID ? 2 : 0));
 }

 private boolean allowSingleInline()
 {
  return !isVirtual && hasSingleCaller() && (singleCaller.hasSingleCaller() ||
          singleCaller.id.equals("<clinit>") || !singleCaller.allowInline());
 }
 ...
 {
  if (ourClass.name().equals(Names.JAVA_LANG_OBJECT) && !isClassMethod() &&
      resType.objectSize() == Type.VOID &&
      methodSignature().signatureString().equals(Names.SIGN_FINALIZE))
   return true;
  int pos = id.length();
  char ch;
      ...
      ((ch = id.charAt(pos - 2)) < '0' || ch > '9'))
  {
   ...
   if (className.startsWith(Names.JAVA_LANG_REF_0) ||
       ...
       className.length() - pos > 2 && className.charAt(pos + 1) == 'V' &&
       className.charAt(pos + 2) == 'M'))
    return true;
  }
  return false;
 }
   ...
   ExpressionType origResType)
 {
  ...
  if (used && (receiverClass == null || receiverClass.hasRealInstances()) &&
      (isAbstract() || (receiverClass != null && allowOverride() &&
      receiverClass.subclassHasMethod(methodSignature().signatureString(),
      isProtectedOrPublic() ? null : ourClass.getPackageName()))))
  {
   ...
   oc.cPrint(isNotNull > 0 ? "JCGO_CALL_NZVFUNC" : isNotNull != 0 ?
    "JCGO_CALL_EVFUNC" : "JCGO_CALL_VFUNC");
   oc.cPrint("(");
   ...
   else
   {
    if (isNotNull <= 0)
    {
     ...
     oc.cPrint(isNotNull != 0 ? "JCGO_CALL_EFINALF" : "JCGO_CALL_FINALF");
     oc.cPrint("(");
     oc.cPrint(receiverString);
     oc.cPrint(") ");
    }
    if (usedExact && (receiverClass == null ||
        receiverClass.hasRealInstances()))
    {
     oc.cPrint(hasStackObjRet ? stackObjRetRoutineCName() : routineCName());
 ...
 void produceOutput(OutputContext oc)
 {
  ...
  if (!isClassMethod() || ourClass != Main.dict.mainClass ||
      (modifiers & (AccModifier.PRIVATE | AccModifier.PROTECTED |
      AccModifier.NATIVE)) != 0)
   sb.append(stackObjRetRequired || allowInline() ? (alreadyReferenced ?
    "JCGO_NOSEP_FRWINL " : "JCGO_NOSEP_INLINE ") : "JCGO_NOSEP_STATIC ");
  ...
  if (jniName != null || !isNative())
  {
   ...
   if (stackObjRetRequired)
   {
    oc.cAndHPrint(allowInline() ? (alreadyReferenced ? "JCGO_NOSEP_FRWINL " :
     "JCGO_NOSEP_INLINE ") : "JCGO_NOSEP_STATIC ");
   ...
   if (jniName != null)
   {
    ...
    if (resType.objectSize() != Type.VOID)
    {
     ...
     if (resType.objectSize() > Type.VOID)
     {
      ...
      if (!resType.getJniName().equals(Type.jniName[Type.CLASSINTERFACE]))
      {
       oc.cPrint("(");
    ...
    VariableDefinition.outputJniParam(oc, isClassMethod() ?
     Main.dict.get(Names.JAVA_LANG_CLASS) : ourClass, targetStr);
    ...
    {
     if (ourClass.isProxyClass() && !isConstructor())
      ...
      else
      {
       ...
       {
        boolean needsBrace = false;
        if (isClassMethod() && (needsTrigClinit || isVMSpecialMethod()))
         needsBrace = ourClass.writeTrigClinit(oc);
        Enumeration en = getLocalsNames();
        while (en.hasMoreElements())
         if (getLocalVar((String) en.nextElement()).outputParamNoClobber(oc))
          needsBrace = true;
        if (isSynchronized())
        {
         if (needsBrace)
          oc.cPrint("{");
         oc.cPrint("JCGO_SYNC_BLOCKSAFENZ(");
         oc.cPrint(isClassMethod() ? ourClass.getClassRefStr(false) :
          This.CNAME);
         oc.cPrint(")\010");
        }
         else needsBrace = false;
   ...
   if (stackObjRetRequired)
   {
    ...
    if (hasParams)
    {
     oc.cPrint(OutputContext.paramStringOutputNoComma(paramList, true));
     oc.cPrint(", ");
    }
  ...
  if (isConstructor() && isNewUsed)
  {
   ...
   oc.cAndHPrint(newReferenced ? "JCGO_NOSEP_FRWINL " : "JCGO_NOSEP_INLINE ");
   oc.cAndHPrint(resType.castName());
   oc.cAndHPrint(" CFASTCALL\n");
 ...
 {
  MethodDefinition md =
   Main.dict.get(Names.JAVA_LANG_REFLECT_VMPROXY).getMethod(
  ...
  for (int s1 = Type.INT; s1 <= Type.VOID; s1++)
   if (indices[s1 - Type.INT] > 0)
   {
    oc.cPrint(Main.dict.addArrayTypeDefn(s1, indices[s1 - Type.INT], ourClass,
     null));
    oc.cPrint(" jcgo_");
  ...
  for (int s1 = Type.INT; s1 <= Type.VOID; s1++)
   if (indices[s1 - Type.INT] > 0)
   {
    ...
    if (s1 == Type.VOID)
    {
     oc.cPrint(Main.dict.get(Names.JAVA_LANG_OBJECT).getClassRefStr(false));
     oc.cPrint(", ");
    }
  ...
  for (int index = 0; en2.hasMoreElements(); index++)
  {
   ...
   int s1 = s0 <= Type.INT ? Type.INT : s0 < Type.VOID ? s0 : Type.VOID;
   ...
   if (s0 >= Type.CLASSINTERFACE)
   {
    ...
    {
     if (s0 < Type.INT && s0 > Type.BOOLEAN)
     {
      oc.cPrint("(");
   ...
   Term.assertCond(t != null);
   OutputContext oc2 = new OutputContext();
   t.parameterOutput(oc2, true, s0 < Type.CLASSINTERFACE ? s0 : Type.NULLREF);
  ...
  for (int s1 = Type.INT; s1 <= Type.VOID; s1++)
  {
       ...
       (s0 < Type.VOID && (s0 > Type.INT ? s0 : Type.INT) == s1))
   {
    ...
    else oc.cPrint(LexTerm.NULL_STR);
  }
  oc.cPrint(", ");
     ...
     if (!hasParameters())
     {
          ...
          !md.hasNonEmptyBody())
      {
       md = Main.dict.get(Names.JAVA_LANG_OBJECT).getMethod(
             Names.SIGN_FINALIZE);
       if (md == null || !md.usedExact() || resType != md.resType ||
           md.isClassMethod())
        md = this;
      }
 ...
 void setTraceExprType(ExpressionType curType)
 {
  if (resType.objectSize() >= Type.CLASSINTERFACE)
  {
   ...
   curTraceType = curTraceType != null ? ClassDefinition.maxCommonExprOf(
                   curTraceType, curType, null) : curType;
  }
 }
 ...
 ExpressionType traceBody(ObjVector parmTraceSig)
 {
  ...
  if (isNative())
  {
   if (!Names.isVMCoreClass(ourClass.name()))
   {
    ...
    MethodDefinition md;
    if (cd.used() && (md = cd.getMethod(Names.SIGN_ARRAYCLASSOF0X)) != null &&
        md.isClassMethod())
  ...
  if (hasParameters())
   paramList.setTraceExprType(parmTraceSig != null ?
    parmTraceSig.elements() : null);
  ...
  if (actualType != null && (curType = curTraceType) != null)
  {
   if (!actualType.hasRealInstances())
    curType = Main.dict.classTable[Type.NULLREF];
    else if (!ClassDefinition.isAssignableFrom(actualType, curType, null) ||
             actualType == curType)
     curType = null;
  }
   ...
   ExpressionType curActualClass, ObjVector parmTraceSig)
 {
  ...
  if (usedExact)
  {
   if (isWeak && !Main.dict.classInitWeakDepend)
   {
    ...
    curType = methodTraceClassInit(false, curActualClass, parmTraceSig);
   ...
   if (isClassMethod())
    ...
    else
    {
     Term.assertCond(curActualClass == null ||
      ourClass.isAssignableFrom(curActualClass.receiverClass(), 0, null));
     if (!ourClass.hasRealInstances() && !isConstructor())
      return resType.objectSize() >= Type.CLASSINTERFACE ?
              Main.dict.classTable[Type.NULLREF] : null;
    }
   MethodTraceInfo traceInfo =
    MethodTraceInfo.create(this, curActualClass, isNative() ? null :
    parmTraceSig);
   ...
   if (traceInfo.isNotTraced() && !isConstructor())
   {
    ...
    int s0;
    if (((s0 = actual.objectSize()) == Type.CLASSINTERFACE ||
        s0 == Type.OBJECTARRAY) &&
        actual.signatureClass().hasInstantatedSubclasses(false))
     traceInfo.traceMethod();
   }
   curType = traceInfo.getResTraceType();
  }
  ClassDefinition aclass;
  if (used && curActualClass != null && isVirtual && allowOverride() &&
      (aclass = curActualClass.receiverClass()) == curActualClass &&
      aclass.superClass() != null &&
      (!aclass.name().equals(Names.JAVA_LANG_THROWABLE) ||
      !resType.name().equals(Names.JAVA_LANG_STRING)))
  {
   ExpressionType curType2 =
    aclass.traceClInitInSubclasses(methodSignature().signatureString(),
    isProtectedOrPublic() ? null : ourClass.getPackageName(), !isAbstract(),
    parmTraceSig);
   if (curType != null)
   {
    curType = isAbstract() ? curType2 :
               ClassDefinition.maxCommonExprOf(curType, curType2, null);
    if (actualType != null && !ClassDefinition.isAssignableFrom(actualType,
        curType, null))
     curType = actualType;
   }
    else
  ...
  if (usedExact && ourClass.name().equals(Names.JAVA_LANG_THREAD) &&
      methodSignature().signatureString().equals(
      Names.SIGN_INIT_THREADGROUP_2))
  {
   MethodDefinition md = null;
   if ((curActualClass != null && (md =
       curActualClass.receiverClass().getMethod(Names.SIGN_RUN)) != null &&
       md.definingClass() != ourClass) || (parmTraceSig == null &&
       (md != null || (md = ourClass.getMethod(Names.SIGN_RUN)) != null)))
    md.methodTraceClassInit(false, curActualClass, null);
    else
    {
     ExpressionType curType2;
     if (parmTraceSig != null && parmTraceSig.size() > 1 && (curType2 =
         (ExpressionType) parmTraceSig.elementAt(1)).objectSize() !=
         Type.NULLREF)
     {
      aclass = Main.dict.get(Names.JAVA_LANG_RUNNABLE);
      if (aclass.used() && aclass.isAssignableFrom(curType2.receiverClass(),
          0, null) && (md = curType2.receiverClass().getMethod(
          Names.SIGN_RUN)) != null)
