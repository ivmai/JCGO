...
final class UnaryExpression extends LexNode
{
 ...
 boolean handleAssertionsDisabled(ClassDefinition ourClass)
 {
  if (terms[0].getSym() != LexTerm.NOT ||
   ...
   return false;
  VariableDefinition v = ourClass.getField(Names.ASSERTIONSDISABLED, null);
  if (v != null && (!v.isClassVariable() || !v.isFinalVariable() ||
      v.exprType().objectSize() != Type.BOOLEAN ||
      v.definingClass() != ourClass))
   return false;
 ...
 void processPass1(Context c)
 {
  if (exprType1 == null)
  {
   ...
   if (sym != LexTerm.NOT ? s1 < Type.BYTE || s1 > Type.DOUBLE ||
       (s1 > Type.LONG && sym == LexTerm.BITNOT) : s1 != Type.BOOLEAN)
 ...
 {
  int sym = terms[0].getSym();
  return terms[1].tokenCount() + (sym == LexTerm.PLUS || sym == LexTerm.NOT ||
 ...
 void processOutput(OutputContext oc)
 {
  ...
  if ((s1 == Type.LONG && (value = evaluateConstValue()) != null) ||
      ((sym = terms[0].getSym()) == LexTerm.MINUS && s1 == Type.INT &&
      (value = evaluateConstValue()) != null && value.getIntValue() ==
      ~(-1 >>> 1)))
