...
final class StringLiteral extends LexTerm
{
 ...
 void processPass1(Context c)
 {
  ...
  if (textVal == null)
   textVal = str;
  if ((str = str.trim()).length() > 0 && (dynClass =
      Main.dict.dynamicDefineClass(str, ourClass, c.forClass)) != null)
 ...
 String stringOutput()
 {
  assertCond(ourClass != null);
  if (cname == null)
   cname = Main.dict.addStringLiteral(textVal, ourClass).stringOutput();
  return cname;
 }
 ...
 private static void processDynClass(ClassDefinition dynClass)
 {
  ...
  if (sc != null && (sc.name().equals(Names.JAVA_UTIL_LISTRESOURCEBUNDLE) ||
      sc.name().equals(Names.COM_IVMAISOFT_JPROPJAV_STRLISTRESOURCEBUNDLE)))
  {
   String baseName = dynClass.name();
   if (Main.dict.dynamicDefineClass(baseName + "_en", null,
       dynClass) != null && Main.dict.dynamicDefineClass(baseName + "_en_US",
       null, dynClass) != null)
    Main.dict.dynamicDefineClass(baseName + "_en_US_POSIX", null, dynClass);
  }
 }
