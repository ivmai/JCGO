 ...
 * ShiftExpression FILLSHIFT_RIGHT AdditiveExpression
...
final class BinaryOp extends LexNode
{
 ...
 int getPrecedenceLevel()
 {
  ...
  return sym == LexTerm.BITOR ? 5 : sym == LexTerm.XOR ? 6 :
          ...
          sym == LexTerm.SHIFT_LEFT || sym == LexTerm.SHIFT_RIGHT ? 10 : 11;
 }

 StringLiteral asStrLiteral()
 {
  return terms[1].getSym() == LexTerm.PLUS ? terms[2].asStrLiteral() : null;
 }
 ...
 void processPass1(Context c)
 {
  if (exprType0 == null)
  {
   ...
   int s0;
   int s2;
   if (sym == LexTerm.PLUS && (exprType0.receiverClass().isStringOrNull() ||
       exprType2.receiverClass().isStringOrNull()))
   {
    if (exprType0.objectSize() == Type.VOID ||
        exprType2.objectSize() == Type.VOID)
     fatalError(c, "Inappropriate type of value in string concatenation");
    isString = true;
    ...
    if (strCnt0 + strCnt2 == 1)
    {
     if (strCnt0 == 0)
     {
      if (exprType2.receiverClass().isStringOrNull() ? !terms[2].isNotNull() :
          terms[2].actualExprType().objectSize() == Type.CLASSINTERFACE)
       strCnt0 = 1;
     }
      else if (exprType0.receiverClass().isStringOrNull() ?
               !terms[0].isNotNull() :
               terms[0].actualExprType().objectSize() == Type.CLASSINTERFACE)
       strCnt2 = 1;
    }
    if (!exprType0.receiverClass().isStringOrNull())
     exprType0 = (terms[0] = wrapInValueOf(c, terms[0])).exprType();
    if (!exprType2.receiverClass().isStringOrNull())
     exprType2 = (terms[2] = wrapInValueOf(c, terms[2])).exprType();
    if (strCnt0 + strCnt2 > 2)
    {
     ...
     else Main.dict.get(Names.JAVA_LANG_VMSYSTEM).markMethod(
           Names.SIGN_CONCAT0X);
    ...
    {
     if (sym != LexTerm.XOR && sym != LexTerm.BITOR && sym != LexTerm.BITAND)
      fatalError(c, "Illegal boolean logical operator");
    }
     else if (s0 >= Type.VOID || s0 <= Type.BOOLEAN ||
              (s2 = exprType2.objectSize()) >= Type.VOID ||
              s2 <= Type.BOOLEAN || ((s0 >= Type.FLOAT || s2 >= Type.FLOAT) &&
              (sym == LexTerm.XOR || sym == LexTerm.BITOR ||
              sym == LexTerm.BITAND || sym == LexTerm.FILLSHIFT_RIGHT ||
              sym == LexTerm.SHIFT_RIGHT || sym == LexTerm.SHIFT_LEFT)))
      fatalError(c, "Inappropriate type of value in numeric expression");
   constVal2 = terms[2].evaluateConstValue();
   if (constVal2 == null && !isString && (sym == LexTerm.PLUS ||
       sym == LexTerm.TIMES || sym == LexTerm.BITAND ||
       sym == LexTerm.BITOR || sym == LexTerm.XOR) &&
 ...
 ConstValue evaluateConstValue()
 {
  ...
  return constVal0 != null ? constVal0.arithmeticOp(constVal2, sym) : null;
 }

 boolean isFPZero()
 {
  assertCond(exprType0 != null);
  return sym != LexTerm.DIVIDE && sym != LexTerm.MOD && terms[0].isFPZero() &&
          terms[2].isFPZero();
 }
 ...
 ExpressionType exprType()
 {
  ...
  if (s0 != Type.BOOLEAN || s2 != Type.BOOLEAN)
  {
   if (sym == LexTerm.FILLSHIFT_RIGHT || sym == LexTerm.SHIFT_RIGHT ||
    ...
    return s0 >= Type.INT ? exprType0 : Main.dict.classTable[Type.INT];
 ...
 private boolean t0IsLiteral()
 {
  if (!t0IsLiteralSet)
  {
   t0IsLiteral = terms[0].isLiteral() && (!isString ||
 ...
 private boolean t2IsLiteral()
 {
  if (!t2IsLiteralSet)
  {
   t2IsLiteral = terms[2].isLiteral() && (!isString ||
                  exprType2.receiverClass().isStringOrNull());
   t2IsLiteralSet = true;
  }
  return t2IsLiteral;
 ...
 boolean isLiteral()
 {
  ...
  if (sym == LexTerm.DIVIDE || sym == LexTerm.MOD)
   return exprType0.objectSize() < Type.FLOAT &&
           ...
           t2IsNonZeroLiteral();
  if (exprType0.objectSize() != Type.LONG || (sym != LexTerm.SHIFT_LEFT &&
      sym != LexTerm.FILLSHIFT_RIGHT && sym != LexTerm.SHIFT_RIGHT))
 ...
 boolean isImmutable()
 {
  ...
  return isString ? strCnt0 + strCnt2 <= 1 && t0IsLiteral() &&
          t2IsLiteral() : ((sym != LexTerm.DIVIDE && sym != LexTerm.MOD) ||
          exprType0.objectSize() >= Type.FLOAT ||
          exprType2.objectSize() >= Type.FLOAT ? terms[2].isImmutable() :
          t2IsNonZeroLiteral()) && terms[0].isImmutable();
 }
 ...
 int tokenCount()
 {
  return isString ? (strCnt0 + strCnt2 <= 1 ? 1 : strCnt0 == 0 ?
          terms[2].tokenCount() : strCnt2 == 0 ? terms[0].tokenCount() :
          terms[0].tokenCount() + terms[2].tokenCount() + 2) :
          constVal2 != null && terms[0].evaluateConstValue() != null ? 1 :
          terms[0].tokenCount() + terms[2].tokenCount() +
 ...
 static boolean isRcvrNeeded(Term t0, Term t2)
 {
  return t0.isSafeExpr() ? !t2.isSafeWithThrow() &&
          ...
          !t2.isImmutable() && (!t2.isSafeExpr() ||
          t2.isFieldAccessed(null) || t0.isAnyLocalVarChanged(t2));
 }
 ...
 void allocRcvr(int[] curRcvrs)
 {
  ...
  if (isRcvrNeeded(terms[0], terms[2]))
   rcvr = ++curRcvrs2[!isString && (s0 = exprType0.objectSize()) < Type.VOID ?
           s0 : Type.NULLREF];
 ...
 void writeStackObjs(OutputContext oc, Term scopeTerm)
 {
  if (isString && strCnt0 > 1 && strCnt2 > 0)
  {
   ...
   else terms[0].writeStackObjs(oc, scopeTerm);
   ...
   terms[2].parameterOutput(oc, asArg, type);
    ...
    else
    {
     ...
     if (strCnt0 <= 1)
     {
      ...
      oc.cPrint("(\010 ");
      InstanceCreation.writeNewRoutineCall(oc, cd.getMethod("<init>()"),
       stackObjCode);
      oc.cPrint("), ");
     }
     if (rcvrStr != null)
 ...
 void processOutput(OutputContext oc)
 {
  ...
  if (isString)
  {
   if (strCnt0 > 1 && strCnt2 > 0)
   {
    ...
    str0 = oc2.instanceToString();
                 ...
                 terms[0].stringOutput()) : LexTerm.NULL_STR;
  }
  if (rcvr > 0)
  {
   rcvrStr = OutputContext.getRcvrName(rcvr, isString ? Type.CLASSINTERFACE :
              exprType0.objectSize());
   oc.cPrint("(");
   oc.cPrint(rcvrStr);
   oc.cPrint("= ");
   if (str0 != null)
  ...
  if (str0 != null)
  {
   if (strCnt0 > 0)
   {
    ...
    if (strCnt2 > 0)
    {
     ...
     if (strCnt0 > 1)
     {
      InstanceCreation.writeNewRoutineCall(oc,
       cd.getMethod(Names.SIGN_INIT_STRINGBUILDER), stackObjCode);
      cd = Main.dict.get(Names.JAVA_LANG_STRINGBUILDER);
      md = cd.getMethod(Names.SIGN_APPEND_STRING);
     }
      else md = Main.dict.get(Names.JAVA_LANG_VMSYSTEM).getMethod(
                 Names.SIGN_CONCAT0X);
     oc.cPrint(md != null ? md.routineCName() :
      MethodDefinition.UNKNOWN_NAME);
     Main.dict.normalCalls++;
    ...
    oc.cPrint(rcvrStr != null ? "(" + cd.castName() + ")" + rcvrStr : str0);
    if (strCnt2 > 0)
     oc.cPrint(", ");
   }
   ...
   else
   {
    ...
    if (sm == Type.LONG && (value = evaluateConstValue()) != null)
     ...
     else if (sym != LexTerm.DIVIDE && sym != LexTerm.MOD ?
              sym == LexTerm.FILLSHIFT_RIGHT || sym == LexTerm.SHIFT_RIGHT ||
              sym == LexTerm.SHIFT_LEFT : sm >= Type.FLOAT || (t0IsLiteral() ?
              !t2IsNonZeroLiteral() : sm == Type.LONG || constVal2 == null ||
              !constVal2.isNonZero() || constVal2.isMinusOne()))
     {
      outputDivShift(oc, sym, sym == LexTerm.DIVIDE || sym == LexTerm.MOD ?
       sm : s0);
      ...
      {
       if (constVal2 != null && rcvrStr == null && sym == LexTerm.BITAND &&
           s0 < Type.INT && s2 <= Type.INT &&
           constVal2.getIntValue() == (s0 != Type.BYTE ? 0xffff : 0xff))
       {
        oc.cPrint("(");
        oc.cPrint(Type.cName[Type.INT]);
        oc.cPrint(")((");
        ...
        else
        {
         ...
         if (sm >= Type.LONG && sm > s0)
         {
          ...
          if (rcvrStr != null)
           oc.cPrint(rcvrStr);
           else if (sm >= Type.FLOAT && !asLiteral &&
                    (constVal0 = terms[0].evaluateConstValue()) != null &&
                    !constVal0.isNonZero())
            oc.cPrint(sm == Type.FLOAT ? "JCGO_FP_ZEROF" : "JCGO_FP_ZERO");
            else terms[0].atomaryOutput(oc);
         }
          else if (rcvrStr != null)
           oc.cPrint(rcvrStr);
 ...
 static Term wrapInValueOf(Context c, Term t)
 {
  ...
  return s0 >= Type.CLASSINTERFACE && t.isNotNull() ?
          new MethodInvocation(c, t, Names.TOSTRING) :
          new MethodInvocation(c, Main.dict.get(Names.JAVA_LANG_STRING),
          Names.VALUEOF, s0 == Type.CLASSINTERFACE + Type.CHAR ||
          s0 == Type.NULLREF ? (new CastExpression(new Expression(
          new ClassOrIfaceType(Main.dict.get(Names.JAVA_LANG_OBJECT))),
          t)).processPassOneInner(c, false).setLineInfoFrom(t) : t);
 }
