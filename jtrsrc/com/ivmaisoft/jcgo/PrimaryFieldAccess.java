...
final class PrimaryFieldAccess extends LexNode
{
  ...
  if (fieldVar == null)
  {
   ...
   if (fieldVar != null)
   {
    ...
    if (fieldVar.isClassVariable())
    {
     ...
     {
      c.addAccessedClass(terms[0].actualExprType().receiverClass());
    ...
    else fatalError(c, "Undefined field: " + aclass.name() + "." + name);
  }
 }
 ...
 boolean isImmutable()
 {
  return fieldVar != null && (fieldVar.isClassVariable() ?
          fieldVar.isImmutable(fieldVar.definingClass()) && t0IsSafeExpr() &&
          (isClinitSafe || fieldVar.isLiteral() ||
          !fieldVar.definingClass().classInitializerNotCalledYet()) :
          fieldVar.isImmutable(terms[0].exprType().receiverClass()) &&
          terms[0].isImmutable() && t0IsNotNull());
 }

 boolean isSafeExpr()
 {
  return fieldVar != null && t0IsSafeExpr() && (fieldVar.isClassVariable() ?
          isClinitSafe || fieldVar.isLiteral() ||
          !fieldVar.definingClass().classInitializerNotCalledYet() :
          t0IsNotNull());
 }
 ...
 {
  return fieldVar != null && (allowInstance || fieldVar.isClassVariable() ||
          terms[0].getVariable(false) == VariableDefinition.THIS_VAR) ?
          fieldVar : null;
 }
 ...
 {
  return fieldVar != null && fieldVar.isClassVariable() ?
          fieldVar.tokensExpandedCount() : terms[0].tokensExpandedCount();
 }
 ...
 void processOutput(OutputContext oc)
 {
  if (fieldVar != null && fieldVar.isClassVariable())
  {
   ...
   else oc.cPrint(fieldVar != null ?
         fieldVar.stringOutput(terms[0].stringOutput(), t0IsNotNull() ? 1 :
         forceCheck ? -1 : 0, lvalue) : VariableDefinition.UNKNOWN_NAME);
 }
