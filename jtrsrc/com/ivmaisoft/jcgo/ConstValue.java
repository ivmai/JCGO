...
final class ConstValue
{
 ...
 {
  return value != null ? new ConstValue(low & value.low, high & value.high,
          isLong || value.isLong) : null;
 }
 ...
 ConstValue isLessThan(ConstValue value)
 {
  return value != null ? new ConstValue(high < value.high ||
          (high == value.high && ((low ^ value.low) >= 0 ?
          low - value.low : value.low) < 0)) : null;
 }
 ...
 {
  return value != null ? (isLong || value.isLong ?
          new ConstValue(low * value.low, mulHigh(low, value.low) +
          low * value.high + high * value.low) :
          new ConstValue(low * value.low)) : null;
 }
 ...
 private ConstValue divide(ConstValue value)
 {
  if (value != null)
  {
   ...
   if (value.isNonZero())
   {
    ...
    if (isNonZero())
    {
     ConstValue remainder = isNeg() ? castToLong().neg() : castToLong();
     ConstValue shiftedValue = value.isNeg() ? value.castToLong().neg() :
                                value.castToLong();
     ...
     do
     {
      if (shiftedValue.isNeg() ? shiftedValue.isEqual(remainder).isNonZero() :
          shiftedValue.isLessOrEqual(remainder).isNonZero() ||
          remainder.isNeg())
      {
 ...
 {
  return value != null ? (isLong ? ((value.low & 0x20) != 0 ?
          new ConstValue(0, low << value.low) :
          new ConstValue(low << value.low, ((value.low & 0x1f) != 0 ?
          low >>> (-value.low) : 0) | (high << value.low))) :
          new ConstValue(low << value.low)) : null;
 }
 ...
 {
  return value != null ? (isLong ? ((value.low & 0x20) != 0 ?
          new ConstValue(high >> value.low, high < 0 ? -1 : 0) :
          new ConstValue(((value.low & 0x1f) != 0 ?
          high << (-value.low) : 0) | (low >>> value.low),
          high >> value.low)) : new ConstValue(low >> value.low)) : null;
 }

 private ConstValue fillShiftRight(ConstValue value)
 {
  return value != null ? (isLong ? ((value.low & 0x20) != 0 ?
          new ConstValue(high >>> value.low, 0) :
          new ConstValue(((value.low & 0x1f) != 0 ?
          high << (-value.low) : 0) | (low >>> value.low),
          high >>> value.low)) : new ConstValue(low >>> value.low)) : null;
 }
 ...
 {
  if (!isLong)
   return low != 0 ? (low >= 0x1000000 || low <= -0x1000000 ?
           "(jint)0x" + Integer.toHexString(low) + "L" :
           "(jint)" + Integer.toString(low) +
           (low >= 30000 || low <= -30000 ? "L" : "")) : "0";
  return "JLONG_C(" + ((low < 0 ? high + 1 : high) != 0 ||
          low >= 0x1000000 || low <= -0x1000000 ? "0x" +
          (high != 0 ? Integer.toHexString(high) + toPaddedHex(low, false) :
          Integer.toHexString(low)) : Integer.toString(low)) + ")";
 }
 ...
 static String toPaddedHex(int intValue, boolean upperCase)
 {
  ...
  while (pos-- > 0)
  {
   chars[pos] = (char) (((intValue & 0xf) < 10 ? '0' : upperCase ? 'A' - 10 :
                 'a' - 10) + (intValue & 0xf));
   intValue >>>= 4;
  }
