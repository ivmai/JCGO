...
final class IfThenElse extends LexNode
{
 ...
 {
  super(c, e.isBlock() ? e : new Block(e), !g.notEmpty() || g.isBlock() ? g :
        new Block(g));
 }
 ...
 void processPass1(Context c)
 {
  ...
  if (constVal0 != null)
   ...
   else
   {
    ...
    {
     VariableDefinition v = md.getLocalVar((String) en.nextElement());
     if (v.isUnassigned())
      unsetVars.addLast(v);
    }
    ...
    if (terms[2].notEmpty())
    {
     boolean hasBreakOrContinue1 = c.hasBreakSimple || c.hasBreakDeep ||
     ...
     if (!c.hasBreakSimple && !c.hasBreakDeep && !c.hasContinueSimple &&
         !c.hasContinueDeep && terms[2].hasTailReturnOrThrow())
      c.swapBranch(oldBranch);
      else if (hasBreakOrContinue1 || !terms[1].hasTailReturnOrThrow())
 ...
 int tokenCount()
 {
  return isAssertion ? 1 : constVal0 != null ?
 ...
 boolean hasTailReturnOrThrow()
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].hasTailReturnOrThrow() :
          terms[1].hasTailReturnOrThrow() && terms[2].hasTailReturnOrThrow();
 }

 boolean isReturnAtEnd(boolean allowBreakThrow)
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isReturnAtEnd(
          allowBreakThrow) : terms[1].isReturnAtEnd(allowBreakThrow) &&
 ...
 {
  return isAssertion || (constVal0 != null ? terms[constVal0.isNonZero() ?
          1 : 2].allowInline(tokenLimit) : !terms[2].notEmpty() &&
