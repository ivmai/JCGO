...
final class NameMapper
{
 ...
 {
  "CHKALL_", "ERROR_", "GC_", "HAVE_", "JCGO_", "JNI_", "Java_", "MAXT_",
  "NOJAVA_", "OBJT_", "PTHREAD_", "_", "jcgo_"
 };
 ...
 {
  Names.JAVA_LANG_CLASS, "c",
  Names.JAVA_LANG_OBJECT, "o",
  Names.JAVA_LANG_STRING, "s",
  Names.JAVA_LANG_THROWABLE, "t"
 };
 ...
 String cnameToShort(String cname)
 {
  ...
  {
   if (basename.length() - pos > 3 && isLetter(basename.charAt(pos + 1)))
   {
  ...
  String cname2;
  if (pos <= 1 || ((cname2 = (String) shortLowNames.get(lowername)) != null &&
      !cname2.equals(cname)) || lowername.startsWith("jcgo"))
  {
   ...
   do
   {
    ...
    if (!lowername.startsWith("jcgo") &&
        ((cname2 = (String) shortLowNames.get(lowername)) == null ||
        cname2.equals(cname)))
     break;
    if (pos > 3 || ++hash == 0)
     pos--;
 ...
 {
  String name = convertInvalidChars(id.replace('$', '_').replace('.', '_'));
  boolean changed = false;
  if (name.length() > 0)
  {
   ...
   int hiddenCount, String classCName)
 {
  ...
  do
  {
   String fullname = classCName != null ? classCName + "__" + csign : csign;
 ...
 {
  String outputName =
   convertId(originalName.endsWith("_") && originalName.length() > 1 ?
   originalName.substring(0, originalName.length() - 1) : originalName);
  do
  {
 ...
 String localVarToOutputName(String classCName, String originalName)
 {
  String outputName = convertId(originalName.startsWith("this_") ||
                       originalName.startsWith("val_") ?
                       "_" + originalName : originalName);
  classCName = classCName + ".";
  String cname;
  String name2;
  if (usedNames.get(outputName) != null || ((name2 =
      (String) localVarCNames.get(cname = classCName + outputName)) != null &&
      !name2.equals(originalName)))
  {
   outputName = outputName + "_" +
                 hashToPackedStr(hashString(originalName), 3);
   while (usedNames.get(outputName) != null ||
          ((name2 = (String) localVarCNames.get(cname =
          classCName + outputName)) != null && !name2.equals(originalName)))
    outputName += "0";
  }
  if (name2 == null)
 ...
 {
  String outputName =
   convertId((originalName.startsWith("this_") ||
   originalName.startsWith("val_") ? "_" : "") +
   (originalName.endsWith("_") && originalName.length() > 1 ?
   originalName.substring(0, originalName.length() - 1) : originalName) +
