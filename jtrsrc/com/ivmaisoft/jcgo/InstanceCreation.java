 ...
 **
 * Formats:
 * NEW ClassOrIfaceType LPAREN [ArgumentList] RPAREN [ClassBody]
...
final class InstanceCreation extends LexNode
{
 ...
 void processPass0(Context c)
 {
  ...
  if (classBody.notEmpty())
  {
   ...
   cd.definePass0(c, c.modifiers & AccModifier.STATIC, id,
    terms[0].notEmpty() ? new ClassOrIfaceType(new LexTerm(LexTerm.ID,
    terms[1].dottedName())) : terms[1], Empty.newTerm(), classBody, false);
  }
 }
  ...
  if (!analysisDone)
  {
   ...
   if (terms[0].notEmpty())
   {
    ...
    if (exprType0.objectSize() != Type.CLASSINTERFACE)
     fatalError(c, "Illegal type of expression for qualified 'new'");
    String name = exprType0.receiverClass().resolveInnerClass(
                   terms[1].dottedName(), false, c.forClass);
    aclass = name != null ? Main.dict.get(name) :
              c.resolveClass(terms[1].dottedName(), true, false);
   ...
   do
   {
    ...
    {
     Term t = new Argument(new Expression((new QualifiedName(new LexTerm(
               LexTerm.ID, ((VariableDefinition) locals.elementAt(
               i)).id()))).setLineInfoFrom(this)));
     t.processPass1(c);
    ...
    if (!aclass.isStaticClass() && param == null)
    {
     ...
     if (cd != null)
      for (ClassDefinition cdOuter = cd.outerClass();
           cdOuter != null; cdOuter = cdOuter.outerClass())
       if (outerClass.isAssignableFrom(cdOuter, 0, c.forClass))
       {
        outerClass = cdOuter;
        break;
       }
    ...
    if (cd == null && aclass.isAbstractOrInterface())
     fatalError(c, "Cannot create an instance of an abstract class: " +
      aclass.name());
   ...
   if (!c.currentClass.name().equals(Names.JAVA_LANG_STRING) ||
       md.definingClass().used() || !aclass.name().equals(
       Names.JAVA_LANG_STRINGINDEXOUTOFBOUNDSEXCEPTION) ||
       (paramList.notEmpty() &&
       !md.methodSignature().signatureString().equals(Names.SIGN_INIT_INT)))
   {
    ...
    else Main.dict.markStrIndexOutInit = true;
 ...
 ExpressionType exprType()
 {
  ...
  return md != null ? md.exprType() : Main.dict.get(Names.JAVA_LANG_OBJECT);
 }
 ...
 void allocRcvr(int[] curRcvrs)
 {
  if (md == null || md.used())
  {
   ...
   if (primaryIndex >= 0)
   {
    Term t0 = (primaryIndex > 0 ? t2.getTermAt(1) : t2).getTermAt(0);
    if (t0.isNotNull())
     primaryIndex = -1;
     else
     {
      if (t0 == t2)
      {
       ...
       t2.allocParamRcvr(curRcvrs, OutputContext.copyRcvrs(curRcvrs),
        curRcvrs2);
       return;
      }
 ...
 void discoverObjLeaks()
 {
  ...
  if (md != null && md.used())
  {
   ...
   if ((mf != null && mf.definingClass().superClass() != null &&
       (vrtmClass = Main.dict.get(Names.JAVA_LANG_VMRUNTIME)).used() &&
       (mf = vrtmClass.getMethod(Names.SIGN_FINALIZEOBJECT0X)) != null &&
       mf.isClassMethod()) || md.hasThisObjLeak(false) ||
 ...
 {
  if (v == VariableDefinition.RETURN_VAR && md != null &&
      !isClinitSafe && md.needsTrigClinit() &&
   ...
   boolean insideAssertStmt, boolean needsLocalVolatile)
 {
  ...
  if (insideAssertStmt)
   oc.cPrint("\n#endif\010");
  return "JCGO_STACKOBJ" + (needsLocalVolatile ? "VLT" : "") + "_NEW" +
          (needsTrigClinit && ourClass.classInitializerNotCalledYet() ?
          "TRIG(" : "(") + stackObjName + ", " + ourClass.vTableCName() + ")";
 }
 ...
 static void writeNewRoutineCall(OutputContext oc, MethodDefinition md,
   String stackObjCode)
 {
  oc.cPrint(md != null ? (stackObjCode != null ? md.routineCName() :
   md.newRoutineCName()) : MethodDefinition.UNKNOWN_NAME);
  oc.cPrint("(");
 ...
 void processOutput(OutputContext oc)
 {
  ...
  if (md == null || md.used())
  {
   if (t2.notEmpty())
   {
    ...
    if (primaryIndex >= 0)
    {
     oc.cPrint(forceCheck ? "JCGO_CALL_EFINALF" : "JCGO_CALL_FINALF");
     oc.cPrint("(");
     OutputContext oc2 = new OutputContext();
     (primaryIndex > 0 ? t2.getTermAt(1) :
      t2).getTermAt(0).parameterOutput(oc2, true, Type.CLASSINTERFACE);
     oc.cPrint(oc2.instanceToString().substring(2));
     oc.cPrint(") ");
    }
   }
   ClassDefinition ourClass;
   writeNewRoutineCall(oc, md,
    stackObjCode != null || md == null ? stackObjCode : (ourClass =
    md.definingClass()).classInitializerNotCalledYet() && isClinitSafe &&
    md.needsTrigClinit() ? ourClass.cNewObjectCode() : null);
 ...
 private void processReflection(ClassDefinition forClass)
 {
  if (!md.isPublic())
   return;
  if (md.definingClass().name().equals(
      Names.JAVAX_SWING_UIDEFAULTS_PROXYLAZYVALUE))
  {
   ...
   {
    ExpressionType exprType =
     MethodInvocation.decodeClassForNameArg(decodeArgAsString(0), null);
    if (exprType != null)
    {
     ClassDefinition aclass = exprType.receiverClass();
     aclass.markUsed();
     reflectConstructors(aclass, sigString.equals(Names.SIGN_INIT_STRING) ?
      new ObjVector() : decodeArgAsValuesArray(1));
    }
    return;
   }
       ...
       sigString.equals(Names.SIGN_INIT_STRING_STRING_OBJECTS))
   {
    ...
    if (exprType != null)
    {
     ...
     ObjVector parmSig = sigString.equals(Names.SIGN_INIT_STRING_STRING) ?
                          new ObjVector() : decodeArgAsValuesArray(2);
     if (t != null && t.actualExprType().objectSize() == Type.NULLREF)
      reflectConstructors(aclass, parmSig);
      else reflectMethods(aclass, decodeArgAsString(1), parmSig);
    }
   }
  }
 }
   ...
   ObjVector parmSig)
 {
  ...
  {
   literalClass.reflectConstructors(false, parmSig != null ?
 ...
 ExpressionType traceClassInit()
 {
  ...
  if (md != null)
  {
   ...
   if (mf != null && mf.definingClass().superClass() != null)
   {
    ClassDefinition vrtmClass = Main.dict.get(Names.JAVA_LANG_VMRUNTIME);
    MethodDefinition mr;
    if (vrtmClass.used() &&
        (mr = vrtmClass.getMethod(Names.SIGN_FINALIZEOBJECT0X)) != null &&
        mr.isClassMethod())
   ...
   if (reflectedClass != null)
   {
    ...
    if (reflectedMethodId.equals("<init>"))
     aclass.traceReflectedConstructor(false, reflectedParmSig != null ?
      (new MethodSignature("<init>", reflectedParmSig)).signatureString() :
      null, reflectedClass != aclass);
     else aclass.traceReflectedMethod(reflectedMethodId.length() > 0 ?
           reflectedMethodId : null, false, reflectedParmSig);
   }
  }
  return null;
 }
