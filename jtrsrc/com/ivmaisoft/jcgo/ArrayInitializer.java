...
/**
 * Grammar production for an array initializer.
 * An array initializer is the code that follow array definition and
 * it may be either a literal one or built at run-time.
 */
...
final class ArrayInitializer extends LexNode
{
 ...
 void processPass1(Context c)
 {
  if (exprType == null)
  {
   ...
   if (!isStatic)
   {
    ...
    if (ArrayCreation.MAX_ONSTACK_SIZE / Type.sizeInBytes[
        s0 < Type.CLASSINTERFACE ? s0 : Type.NULLREF] >= count)
    {
 ...
 boolean storeClassLiteralsGuess(ObjVector parmSig, boolean isActual)
 {
  ...
  return exprType.objectSize() == Type.OBJECTARRAY &&
          exprType.signatureDimensions() == 1 && (isActual ||
          exprType.signatureClass().name().equals(Names.JAVA_LANG_CLASS)) &&
   ...
   String cname, boolean insideAssertStmt, boolean needsLocalVolatile)
 {
  ...
  if (insideAssertStmt)
   oc.cPrint("\n#endif\010");
  return (cname != null ? "(" + Type.cName[s0 < Type.CLASSINTERFACE ?
          s0 + Type.CLASSINTERFACE : Type.OBJECTARRAY] + ")JCGO_STACKOBJ" +
          (needsLocalVolatile ? "VLT" : "") + "_ARRCLONE" :
          ...
          "_OBJARRNEW") + "(" + stackObjName + ", " + (cname != null ?
          cname : ClassDefinition.arrayVTableCName(s0 < Type.CLASSINTERFACE ?
          s0 : Type.VOID, innerType.signatureDimensions()) +
          (s0 < Type.CLASSINTERFACE ? "" : ", " +
          innerType.signatureClass().getClassRefStr(false)) + ", " +
 ...
 String writeStackObjDefn(OutputContext oc, boolean needsLocalVolatile)
 {
  ...
  return writeStackObjArrayDefn(oc, ourClass, exprType.indirectedType(),
          count, allowLiteral ? addArrayLiteral(false, false).cname() : null,
  ...
  if (allowLiteral)
  {
   if (stackObjCode != null)
    oc.cPrint(stackObjCode);
     ...
     else outputArrayCreate(oc, true);
  }
   else
   {
    oc.cPrint("(");
    if (rcvr > 0)
    {
     ...
     String rcvrStr = OutputContext.getRcvrName(rcvr, Type.CLASSINTERFACE);
     oc.cPrint(rcvrStr);
     oc.cPrint("= ");
     if (stackObjCode != null)
     {
      ...
      else outputArrayCreate(oc, false);
     oc.cPrint(",");
