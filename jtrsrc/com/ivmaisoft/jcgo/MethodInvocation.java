 ...
 * Primary DOT ID LPAREN [ArgumentList] RPAREN
 * Super DOT ID LPAREN [ArgumentList] RPAREN
 */
...
final class MethodInvocation extends LexNode
{
 ...
 MethodInvocation(Context c, ClassDefinition resultClass, String id,
   Term exprTerm)
 {
  super(Empty.newTerm(), Empty.newTerm(),
   (new Argument(exprTerm, true)).setLineInfoFrom(exprTerm));
 ...
 void processPass1(Context c)
 {
  if (resultClass == null)
  {
   ...
   if (terms[0].isSafeExpr())
   {
    ...
    else terms[2].processPass1(c);
   processPassOneInner(c, aclass, terms[0].actualExprType(), vecSize, id);
  }
 }
   ...
   ExpressionType actualType0, int vecSize, String id)
 {
  if (vecSize == 0)
  {
   MethodSignature msig = new MethodSignature(id, terms[2].getSignature());
   ...
   else
   {
    if (vecSize > 1)
    {
     if (aclass == null)
     {
      fatalError(c, "Undefined qualified name: " + terms[0].dottedName());
     ...
     else
     {
      ...
      if (!md.isClassMethod() && c.currentMethod != null &&
       ...
       fatalError(c, "Instance method used in a static context: " + md.id());
      actualClass = resultClass;
      actualMethod = md;
     }
   }
  boolean useMethodBranch = false;
  if (c.currentClass.superClass() != null || md.used() ||
      ...
      Names.SIGN_ARRAYCLASSOF0X))
  {
   if (isExact && !md.isClassMethod())
    ...
    else actualMethod.markUsed(actualClass,
   ...
   VariableDefinition v = md.isClassMethod() ? null :
                           terms[0].getVariable(false);
   if (!actualMethod.isNative() ||
       !Names.isVMCoreClass(actualMethod.definingClass().name()))
   {
    if (md.isClassMethod())
    ...
    {
     actualMethod.processBranch(c, v == VariableDefinition.THIS_VAR);
 ...
 {
  assertCond(resultClass != null);
  return actualMethod != null && (isExact || !actualMethod.allowOverride()) ?
          actualMethod.actualExprType() : exprType();
 }
 ...
 boolean isSwitchMapAssign(boolean isMethodCall)
 {
  ...
  return v != null && isMethodCall && actualMethod != null &&
          actualMethod.definingClass().name().equals(Names.JAVA_LANG_ENUM) &&
          !actualMethod.isClassMethod() && v.isClassVariable() &&
          actualMethod.exprType().objectSize() == Type.INT &&
          actualMethod.methodSignature().signatureString().equals(
          Names.SIGN_ORDINAL) && v.isFinalVariable();
 }
 ...
 String strLiteralValueGuess()
 {
  ...
  if (md != null)
  {
   ...
   if (resultClass.name().equals(Names.JAVA_LANG_STRING) &&
       md.isClassMethod() && actualMethod.id().equals(Names.VALUEOF))
 ...
 {
  assertCond(resultClass != null);
  return classLiteralValue != null ?
          (classLiteralValue.receiverClass().isProxyClass() ? null :
          classLiteralValue) : actualMethod != null ?
          actualMethod.classLiteralValGuess() : null;
 }
 ...
 MethodInvocation getClassNewInstanceCall()
 {
  ...
  if (md != null &&
      ...
      Names.SIGN_NEWINSTANCE) : actualMethod.definingClass().name().equals(
      Names.GNU_CLASSPATH_SERVICEFACTORY_SERVICEITERATOR) &&
      md.isExactMatch(false, Names.JAVA_LANG_OBJECT, 0, Names.SIGN_NEXT)))
   mcall = this;
  return mcall;
 }
 ...
 MethodSignature getConstructorInstanceSign()
 {
  ...
  if (className.equals(Names.JAVA_LANG_CLASS) && (md.isExactMatch(false,
      Names.JAVA_LANG_REFLECT_CONSTRUCTOR, 0, Names.SIGN_GETCONSTRUCTOR) ||
      md.isExactMatch(false, Names.JAVA_LANG_REFLECT_CONSTRUCTOR, 0,
  ...
  if (actualMethod != null && actualMethod.used())
  {
   if (!isExact)
    while (actualMethod.allowOverride())
    {
     ...
     if (cd != actualClass && (md2 = cd.getSameMethod(actualMethod)) != null)
     {
      assertCond(md2.used());
      actualClass = cd;
      actualMethod = md2;
     ...
     md2 = actualClass.getSingleRealMethodInSubclasses(actualMethod, cdArr);
     if (md2 == null)
      break;
     actualMethod = md2;
     actualClass = cdArr[0];
   ...
   if (!actualMethod.copyObjLeaksTo(terms[2]) &&
   ...
   if (actualMethod.hasThisObjLeak(false) ||
       (!isExact && actualMethod.allowOverride() &&
       actualClass.hasThisObjLeakInSubclasses(sigString, false)))
    terms[0].setObjLeaks(null);
    else if (actualMethod.isThisStackObjVolatile() ||
             (!isExact && actualMethod.allowOverride() &&
             actualClass.isThisStackObjVltInSubclasses(sigString)))
     terms[0].setStackObjVolatile();
   if (!actualMethod.allowStackObjRet() ||
       (!isExact && actualMethod.allowOverride() &&
       actualClass.subclassHasMethod(sigString, null)))
    noStackObjRet = true;
 ...
 void setStackObjVolatile()
 {
  ...
  if (actualMethod != null && actualMethod.used() && !needsLocalVolatile)
  {
   ...
   if (actualMethod.hasThisObjLeak(true) || (!isExact &&
       actualMethod.allowOverride() && actualClass.hasThisObjLeakInSubclasses(
       actualMethod.methodSignature().signatureString(), true)))
    terms[0].setStackObjVolatile();
 ...
 void setObjLeaks(VariableDefinition v)
 {
  ...
  if (actualMethod != null && actualMethod.used())
  {
   ...
   noLeaksScope = VariableDefinition.addSetObjLeaksTerm(noLeaksScope, v, this,
                   isConditional || noStackObjRet);
   if (noLeaksScope == null || v == VariableDefinition.WRITABLE_ARRAY_VAR)
   {
    actualMethod.setWritableArray();
    if (!isExact && actualMethod.allowOverride())
     actualClass.setWritableArrayRetInSubclasses(
 ...
 {
  assertCond(actualMethod != null && (noLeaksScope == null || noStackObjRet));
  return actualMethod;
 }
 ...
 void allocRcvr(int[] curRcvrs)
 {
  if (actualMethod != null && actualMethod.used())
  {
   ...
   if (!actualMethod.isClassMethod() && ((t0.isSafeExpr() ?
       !t0.isNotNull() && ((v = t0.getVariable(true)) == null ||
       !v.isLocalOrParam()) && !t0.isImmutable() :
       actualMethod.isAbstract() || !t2.isSafeExpr() || !t0.isNotNull() ||
       t2.isFieldAccessed(null) || (!t2.isImmutable() &&
       t0.isAnyLocalVarChanged(null)) || (!isExact &&
       actualMethod.allowOverride() && actualClass.subclassHasMethod(
       actualMethod.methodSignature().signatureString(), null))) ||
       (!t2.isSafeWithThrow() && !t0.isImmutable() &&
   ...
   if (!md.isClassMethod() && t2.notEmpty() && !t0.isNotNull())
   {
    ...
    t2.allocParamRcvr(curRcvrs1, OutputContext.copyRcvrs(curRcvrs1),
     curRcvrs2);
   }
    else t2.allocRcvr(curRcvrs1);
 ...
 void writeStackObjs(OutputContext oc, Term scopeTerm)
 {
  if (actualMethod != null && actualMethod.used())
  {
   ...
   if (noLeaksScope == scopeTerm && !noStackObjRet)
   {
    ...
    stackObjCode = actualMethod.writeStackObjDefn(oc, needsLocalVolatile);
   }
  }
 }
 ...
 {
  assertCond(resultClass != null);
  if (isVoidExpr && md != null && !md.isClassMethod() &&
      md.exprType().objectSize() != Type.VOID && !terms[0].isSuper(false) &&
      (!actualMethod.used() || (!isExact && !actualClass.hasRealInstances())))
  {
   oc.cPrint("(");
  ...
  if (md == null || md.isClassMethod() || terms[0].isSuper(false))
  {
   ...
   if (!terms[0].isSafeExpr())
   {
    ...
    if (rcvrStr != null)
     oc.cPrint(rcvrStr);
     else terms[0].atomaryOutput(oc);
    oc.cPrint(", ");
   }
   if (md != null)
   {
    if (md.used())
     oc.cPrint(stackObjCode != null ? md.stackObjRetRoutineCName() :
      md.routineCName());
     else assertCond(md.definingClass().name().equals(
           Names.JAVA_LANG_VMCLASS) && md.isExactMatch(true,
           Names.JAVA_LANG_CLASS, 0, Names.SIGN_ARRAYCLASSOF0X));
   }
    else oc.cPrint(MethodDefinition.UNKNOWN_NAME);
   ...
   else
   {
    if (actualMethod.used() && actualClass.hasRealInstances())
     terms[2].produceRcvr(oc);
    rightParenNeeded =
     actualMethod.writeMethodCall(oc, isExact ? null : actualClass,
     (rcvrStr != null || actualClass != resultClass ? "(" +
     actualClass.castName() + ")" : "") + (rcvrStr != null ? rcvrStr :
     resultString != null ? resultString : actualClass == resultClass ||
     terms[0].isAtomary() ? (primaryStr = terms[0].stringOutput()) :
     "(" + (primaryStr = terms[0].stringOutput()) + ")"),
     terms[0].isNotNull() ? 1 : forceCheck ? -1 : 0, stackObjCode != null,
     md.exprType());
   }
   ...
   if (md != null && !md.isClassMethod())
   {
    ...
    else
    {
     if (md == null || md.used())
     {
      if (terms[2].notEmpty())
       oc.cPrint("\010 ");
      oc.cPrint(OutputContext.paramStringOutputNoComma(terms[2], true));
     }
      else
      {
       oc.cPrint("(");
   ...
   ClassDefinition forClass)
 {
      ...
      Names.JAVA_LANG_OBJECT, 0, Names.SIGN_NEWPROXYINSTANCE)))
  {
   ...
   if (t != null)
   {
    ...
    if (t.storeClassLiteralsGuess(parmSig, false))
     classLiteralValue = Main.dict.addProxyClass(parmSig, forClass);
   }
   return;
  }
  if (md.isExactMatch(false, Names.JAVA_LANG_CLASS, 0, Names.SIGN_GETCLASS))
  {
   ...
   if (actualType0.signatureClass().objectSize() == Type.CLASSINTERFACE &&
       (actualType0.objectSize() == Type.OBJECTARRAY ||
       (!isExact && actualType0.receiverClass().superClass() == null)))
    Main.dict.get(Names.JAVA_LANG_VMCLASS).markUsed();
   return;
  }
  if (!className.equals(Names.JAVA_LANG_CLASS))
   return;
  if (md.isExactMatch(false, Names.JAVA_LANG_STRING, 0, Names.SIGN_GETNAME))
  {
   Main.dict.addGetNameClass(terms[0].classLiteralValGuess(), forClass);
   return;
  }
  boolean isForName2 =
   md.isExactMatch(true, Names.JAVA_LANG_CLASS, 0, Names.SIGN_FORNAME_2);
  if (isForName2 || md.isExactMatch(true, Names.JAVA_LANG_CLASS, 0,
      Names.SIGN_FORNAME))
  {
   ExpressionType exprType =
    decodeClassForNameArg(decodeFirstArgAsString(), currentClass);
   if (exprType != null)
   {
    ...
    if (exprType.signatureDimensions() == 0 && (!isForName2 ||
        ((t = terms[2].getArgumentTerm(1)) != null && ((constVal1 =
        t.evaluateConstValue()) == null || constVal1.isNonZero()))))
  ...
  if (exprType != null)
  {
   ...
   if (literalClass != null)
   {
    ...
    if (md.isExactMatch(false, Names.JAVA_LANG_REFLECT_CONSTRUCTOR, 1,
        Names.SIGN_GETDECLAREDCONSTRUCTORS))
    {
     reflectConstructors(literalClass, true, null, isExactType);
     return;
    }
    if (md.isExactMatch(false, Names.JAVA_LANG_REFLECT_CONSTRUCTOR, 1,
        Names.SIGN_GETCONSTRUCTORS))
    {
     reflectConstructors(literalClass, false, null, isExactType);
     return;
    }
    if (md.isExactMatch(false, Names.JAVA_LANG_REFLECT_CONSTRUCTOR, 0,
        Names.SIGN_GETDECLAREDCONSTRUCTOR))
    {
     reflectConstructors(literalClass, true, decodeArgAsClassArray(0),
      isExactType);
     return;
    }
    if (md.isExactMatch(false, Names.JAVA_LANG_REFLECT_CONSTRUCTOR, 0,
        Names.SIGN_GETCONSTRUCTOR))
    {
     reflectConstructors(literalClass, false, decodeArgAsClassArray(0),
      isExactType);
     return;
    }
    ...
    if (isExactType && md.isExactMatch(false, Names.JAVA_LANG_OBJECT, 0,
        Names.SIGN_NEWINSTANCE))
    {
     reflectConstructors(literalClass, true, new ObjVector(), true);
     return;
    }
   }
    else literalClass = Main.dict.get(Names.JAVA_LANG_OBJECT);
   if (md.isExactMatch(false, Names.JAVA_LANG_REFLECT_METHOD, 1,
      ...
      Names.SIGN_GETDECLAREDFIELD))
  {
   ...
   reflectedClass = name != null ? literalClass.reflectField(name, true,
                     isExactType) : literalClass.reflectAllFields(true);
   return;
  }
  if (md.isExactMatch(false, Names.JAVA_LANG_REFLECT_FIELD, 0,
      Names.SIGN_GETFIELD))
  {
   String name = decodeFirstArgAsString();
   if (name != null)
    reflectedClass = literalClass.reflectField(name, false, isExactType);
    else
    {
     reflectedClass = literalClass.reflectAllFields(false);
     reflectInSuper = true;
 ...
 private ObjVector decodeArgAsClassArray(int index)
 {
  ...
  return t.storeClassLiteralsGuess(parmSig, false) ||
          t.actualExprType().objectSize() == Type.NULLREF ? parmSig : null;
 }
   ...
   boolean declaredOnly, ObjVector parmSig, boolean isExactType)
 {
  ...
  {
   literalClass.reflectConstructors(declaredOnly, parmSig != null ?
    (new MethodSignature("<init>", parmSig)).signatureString() : null,
    isExactType);
 ...
 private void reflectMethods(ClassDefinition literalClass,
   boolean declaredOnly, String id, ObjVector parmSig, boolean isExactType)
 {
  if (reflectedMethodId == null)
  {
   ...
   ClassDefinition curClass)
 {
  ...
  if ((curClass != null && !Main.dict.alreadyKnown(str) &&
      (str.indexOf('.', 0) < 0 ? curClass.name().indexOf('.', 0) >= 0 :
      !str.startsWith(curClass.getPackageName() + "."))) ||
 ...
 private void traceReflected()
 {
  ...
  if (reflectedClass != null)
  {
    ...
    else
    {
     if (reflectedMethodId != null)
     {
      if (reflectedMethodId.equals("<init>"))
       cd.traceReflectedConstructor(!reflectInSuper,
        reflectedParmSig != null ? (new MethodSignature("<init>",
        reflectedParmSig)).signatureString() : null, reflectedClass != cd);
       else cd.traceReflectedMethod(reflectedMethodId.length() > 0 ?
             reflectedMethodId : null, !reflectInSuper, reflectedParmSig);
     }
      else
      {
   ...
   {
    if (md != null && md.definingClass().name().equals(
        Names.JAVA_LANG_CLASS) && md.isExactMatch(false,
        Names.JAVA_LANG_OBJECT, 0, Names.SIGN_NEWINSTANCE))
    {
     String ourClassName = Main.dict.curTraceInfo.getDefiningClassName();
     if (!ourClassName.equals(
         Names.GNU_CLASSPATH_SERVICEPROVIDERLOADINGACTION) &&
 ...
 ExpressionType traceClassInit()
 {
  ...
  if (curTraceType0 != null && !actualMethod.isClassMethod())
  {
   if (curTraceType0.objectSize() == Type.NULLREF)
    return actualMethod.exprType().objectSize() >= Type.CLASSINTERFACE ?
            curTraceType0 : null;
   ...
   if (curClass != actualClass)
   {
    ...
    {
     if (actualMethod.allowOverride() && ((curMethod =
         curClass.getSameMethod(actualMethod)) == null || !curMethod.used()))
     {
      curClass = actualClass;
      curMethod = actualMethod;
     }
    }
     else curClass = actualClass;
   }
  }
  ...
  curTraceType0 = curMethod.methodTraceClassInit(false, isExact ||
   (curTraceType0 != null && curTraceType0 != curClass &&
   curTraceType0.signatureClass() == curClass &&
   curTraceType0.signatureDimensions() == 0) ? curClass.asExactClassType() :
   curClass, parmTraceSig);
