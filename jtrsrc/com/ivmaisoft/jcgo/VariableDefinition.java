...
final class VariableDefinition
{
 ...
 static final VariableDefinition RETURN_VAR =
  new VariableDefinition("", false);

 static final VariableDefinition WRITABLE_ARRAY_VAR =
  new VariableDefinition("", false);

 static final VariableDefinition THIS_VAR =
  new VariableDefinition("this", true);

 private static final ObjVector EMPTY_VECT = new ObjVector();

 private static final MethodSignature EMPTY_MSIG =
  new MethodSignature("", EMPTY_VECT);

 private static final MethodInvocation EMPTY_INVOCATION =
  new MethodInvocation(Empty.newTerm(), Empty.newTerm());
  ...
  if (!isLocalOrParam())
  {
   if (defnClass.isInterface())
    this.modifiers |= AccModifier.PUBLIC | AccModifier.STATIC |
                       AccModifier.FINAL;
   if (hasInitializer())
    init = defnClass.addInitializer(initializerTerm, isClassVariable());
  }
 }
 ...
 {
  Term.assertCond((modifiers & (AccModifier.LOCALVAR |
   AccModifier.PARAMETER)) == 0);
 ...
 int markUsed()
 {
  ...
  if (!isLocalOrParam())
  {
   ...
   if (name.equals(Main.dict.failOnFieldName) &&
       ...
       defnClass.name().equals(Main.dict.failOnClassName)))
 ...
 {
  return isFinalVariable() && (hasInitializer() || callerClass != defnClass ||
          callerClass == null || (modifiers & AccModifier.PARAMETER) != 0 ||
          defnClass.objectSize() != Type.CLASSINTERFACE ||
          (isClassVariable() ? name.startsWith("$SwitchMap$") :
 ...
 String strLiteralValueGuess()
 {
  ...
  return strLiteralValueGuess != NO_STR_LITERAL ? strLiteralValueGuess : null;
 }

 void setClassLiteralValGuess(ExpressionType exprType)
 {
  if (classLiteralValue == null || (classLiteralValue ==
      Main.dict.classTable[Type.VOID] && exprType != null))
 ...
 ExpressionType classLiteralValGuess()
 {
  ...
  return classLiteralValue != Main.dict.classTable[Type.VOID] ?
          classLiteralValue : null;
 }
 ...
 {
  if (reflectedSign == null || (reflectedSign == EMPTY_MSIG && msig != null))
   reflectedSign = msig;
 }
 ...
 MethodSignature getConstructorInstanceSign()
 {
  if (reflectedSign == null)
  {
   ...
   producePassOne();
   setConstructorInstanceSign(initializerTerm.getConstructorInstanceSign());
  }
 ...
 void setStoredClassLiteralsGuess(ObjVector parmSig, boolean isActual)
 {
  if (isActual)
  {
   if (classLiteralsActualArr == null || classLiteralsActualArr == EMPTY_VECT)
    classLiteralsActualArr = parmSig;
  }
   else
   {
    if (classLiteralsValueArr == null || classLiteralsValueArr == EMPTY_VECT)
 ...
 {
  ObjVector classLiterals =
   isActual ? classLiteralsActualArr : classLiteralsValueArr;
  ...
  if (isActual)
   ...
   else classLiteralsValueArr = EMPTY_VECT;
  producePassOne();
  ...
  if (isActual)
  {
   ...
   {
    if (classLiteralsValueArr == EMPTY_VECT)
     classLiteralsValueArr = parmSig;
   }
  return true;
 }
 ...
 void initializerPassOne(Context c)
 {
  if (!analysisDone)
  {
   ...
   if (hasInitializer())
   {
    ...
    if (s0 == Type.BOOLEAN ? s1 != Type.BOOLEAN : s0 >= Type.CLASSINTERFACE ?
        s1 < Type.CLASSINTERFACE : s0 >= Type.LONG && s1 < s0)
     initializerTerm.fatalError(c,
   ...
   if (v != RETURN_VAR)
   {
    if (v != null)
    {
     ...
     if (v.curTraceType == null && v.isLocalOrParam())
     {
      ...
      for (LeftBrace scope = v.ourScope; scope != null;
      ...
      else if (v.isClassVariable())
      {
       ...
       {
        v.singleAssignedTerm.setObjLeaks(WRITABLE_ARRAY_VAR);
        v.singleAssignedTerm = Empty.newTerm();
       }
      }
    }
 ...
 void setWritableArray(VariableDefinition v)
 {
  if (isClassVariable() && v != this)
  {
   ...
   {
    MethodDefinition md;
    if ((singleAssignedTerm == null || singleAssignedTerm.notEmpty()) &&
        ((md = Main.dict.ourMethod) == null ||
        !md.attachRetWritableArray(this)))
    {
 ...
 String outputName()
 {
  if (outputCName == null)
  {
   Term.assertCond(defnClass != null);
   outputCName =
    isClassVariable() ? Main.dict.nameMapper.classVarToOutputName(
    defnClass.castName(), name) : isLocalOrParam() ?
    Main.dict.nameMapper.localVarToOutputName(defnClass.castName(), name) :
    defnClass.objectSize() != Type.CLASSINTERFACE ? name :
    Main.dict.nameMapper.fieldToOutputName(defnClass, name,
 ...
 String stringOutputForStatic(boolean isClinitSafe, boolean lvalue)
 {
  ...
  return wrapStrOutputForVolatile(lvalue || !isThreadVolatile(), resType,
          (isLiteral() ? (isClinitSafe ? "" : "JCGO_CLINIT_LITERACC(" +
          classCName + "__class, ") + classCName + "__" : (isClinitSafe ? "" :
          "JCGO_CLINIT_VARACC(" + classCName + "__class, ") + classCName +
          (defnClass.isReflectedField(name) ? "__class." : "__")) +
 ...
 {
  int s1 = resType.objectSize();
  return lvalueOrNonVolatile ? str : s1 >= Type.CLASSINTERFACE ?
          "JCGO_VLT_LFETCH(" + resType.castName() + ", " + str + ")" :
 ...
 private String stringOutputZero()
 {
  ...
  return s1 >= Type.CLASSINTERFACE ? "((" + resType.castName() + ")" +
          LexTerm.NULL_STR + ")" : s1 == Type.BOOLEAN ?
          LexTerm.outputBoolean(false) : "(" + resType.castName() + ")0";
 }
 ...
 private String initTermStringOutput(boolean needsLiteral)
 {
  ...
  if (strOutValue == null)
  {
   ...
   strOutValue = tokensExpCount >= TERMS_BIG_LITERAL && constVal != null ?
                  constVal.stringOutput() : initializerTerm.stringOutput();
   if (needsLiteral)
 ...
 String stringOutput(String prefix, int isNotNull, boolean lvalue)
 {
  Term.assertCond(defnClass != null);
  Term.assertCond((modifiers & AccModifier.STATIC) == 0);
  return isLocalOrParam() ? outputName() : used ?
          (defnClass.objectSize() != Type.CLASSINTERFACE ? (isNotNull > 0 ?
          "JCGO_ARRAY_NZLENGTH(" : isNotNull != 0 ? "JCGO_ARRAY_ELENGTH(" :
          "JCGO_ARRAY_LENGTH(") + prefix + ")" : wrapStrOutputForVolatile(
          lvalue || !isThreadVolatile(), resType, (isNotNull > 0 ?
          "JCGO_FIELD_NZACCESS(" : (isNotNull != 0 ? "JCGO_FIELD_EACCESS(" :
          "JCGO_FIELD_ACCESS(") + defnClass.castName() + ", ") + prefix +
          ", " + outputName() + ")")) : isNotNull > 0 ? stringOutputZero() :
          "(" + (isNotNull != 0 ? "JCGO_CALL_EFINALF" : "JCGO_CALL_FINALF") +
          "(" + prefix + ") (" + resType.castName() + ")" +
 ...
 private String stringOutputDecl(String typeName, String ourOutputName)
 {
  if ((!needsLocalVolatile && !isThreadVolatile()) ||
      (modifiers & (AccModifier.LOCALVAR | AccModifier.PARAMETER)) ==
      AccModifier.PARAMETER)
  ...
  return resType.objectSize() < Type.CLASSINTERFACE ?
          volatileDecl + typeName + " " + ourOutputName :
          typeName + " " + volatileDecl + ourOutputName;
 }
 ...
 boolean outputParamNoClobber(OutputContext oc)
 {
  Term.assertCond((modifiers & (AccModifier.LOCALVAR |
   AccModifier.PARAMETER)) != 0);
  if (!needsLocalVolatile || !used || (modifiers & (AccModifier.LOCALVAR |
      AccModifier.PARAMETER)) != AccModifier.PARAMETER)
   return false;
 ...
 {
  if (type == Type.VOID || resType.objectSize() == type ||
      (type == Type.NULLREF && resType.objectSize() >= Type.CLASSINTERFACE))
  {
   Term.assertCond((modifiers & AccModifier.PARAMETER) != 0);
   oc.cPrint(", ");
   if (!asArg)
    oc.cPrint(stringOutputDecl(type == Type.VOID ? resType.getJniName() :
     resType.castName(), outputName()));
    else if (type == Type.VOID)
     outputJniParam(oc, resType, outputName());
     else oc.cPrint(outputName());
  }
 }
 ...
 {
  Term.assertCond((modifiers & (AccModifier.STATIC | AccModifier.LOCALVAR |
   AccModifier.PARAMETER)) == 0 && md != null);
  if (used)
  {
 ...
 void produceOutput(OutputContext oc)
 {
  ...
  if (isLiteral() && isClassVariable())
  {
   ...
   else
   {
    ...
    {
     oc.instancePrint(stringOutputDecl(resType.castName(), outputName()));
     oc.instancePrint(";");
    }
    if (!isClassVariable() || !initializerTerm.isLiteral())
    {
     ...
     if (code.length() > 0)
     {
      ...
      if ((!code.equals(LexTerm.NULL_STR) && !code.equals("JLONG_C(0)") &&
          !code.equals("0") && !code.equals(LexTerm.outputBoolean(false))) ||
          !init.isPrevSafeWithThrow())
      {
       if (initializerTerm.exprType() != resType)
        code = "(" + resType.castName() + ")" + (initializerTerm.isAtomary() ?
                code : "(" + code + ")");
   ...
   else
   {
    ...
    if (initializerTerm.isLiteral())
    {
     ...
     if (code.length() > 0)
     {
      ...
      if (initializerTerm.exprType() != resType)
       code = "(" + cast + ")" + (initializerTerm.isAtomary() ? code :
               "(" + code + ")");
     }
    }
    if (code.length() == 0)
     code = resType.objectSize() < Type.CLASSINTERFACE ? "(" + cast + ")0" :
             LexTerm.NULL_STR;
   }
  if (isReflected)
   ...
   else
   {
    oc.cAndHPrint(stringOutputDecl(cast, defnClass.castName() + "__" +
     outputName()));
    if (isValueChanged)
     oc.cPrint(resType.objectSize() < Type.CLASSINTERFACE ? (isNonZero ?
      " ATTRIBNONGC" : "") : isNonZero ? " ATTRIBGCDATA" : " ATTRIBGCBSS");
    oc.cPrint("= ");
   }
  oc.hPrint(";");
  oc.cPrint(code);
 ...
 {
  if (isLocalOrParam() && resType.objectSize() >= Type.CLASSINTERFACE &&
      (curTraceType == null || curTraceType.objectSize() != Type.VOID))
  {
   ...
   {
    curTraceType = ClassDefinition.maxCommonExprOf(curTraceType, curType,
                    null);
    ExpressionType actual = actualType != null ? actualType : resType;
    if (!ClassDefinition.isAssignableFrom(actual, curTraceType, null))
     curTraceType = actual;
   }
    else curTraceType = curType;
  }
 }
 ...
 private boolean isInitializerNonZero()
 {
  ...
  return s0 != Type.NULLREF &&
          ((constVal0 = initializerTerm.evaluateConstValue()) != null ?
          constVal0.isNonZero() : (s0 != Type.FLOAT && s0 != Type.DOUBLE) ||
 ...
 ExpressionType traceClassInit()
 {
  if (used && isClassVariable() && !isLiteral())
   defnClass.classTraceClassInit(Main.dict.classInitWeakDepend ||
    (hasInitializer() && initializerTerm.isLiteral() &&
    isInitializerNonZero()));
  if (curTraceType == null || curTraceType.objectSize() == Type.VOID)
   return null;
  ExpressionType actual = actualType != null ? actualType : exprType();
  return actual.hasRealInstances() ? (ClassDefinition.isAssignableFrom(actual,
          curTraceType, null) && actual != curTraceType ?
          curTraceType : null) : Main.dict.classTable[Type.NULLREF];
 }
}
