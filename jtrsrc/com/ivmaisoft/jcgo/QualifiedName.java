...
final class QualifiedName extends LexNode
{
 ...
 boolean isJavaConstant(ClassDefinition ourClass)
 {
  if (callerClass != null)
   return resultFields.size() == 1 &&
           ((VariableDefinition) resultFields.elementAt(0)).isJavaConstant();
  ...
  VariableDefinition v =
   aclass.getField((String) vec.elementAt(lastInd), ourClass);
 ...
 void processPass1(Context c)
 {
  if (callerClass == null)
  {
   ...
   if (vec.size() > 1)
   {
    ...
    if (v == null)
    {
     fatalError(c, "Undefined qualified variable: " + aclass.name() + "." +
      name);
     setUndefined();
     return;
    }
    ...
    else
    {
     ...
     if (v == null)
     {
      ...
      if (!v.isClassVariable() && c.currentMethod != null &&
       ...
       fatalError(c, "Instance variable used in a static context: " + name);
     }
    }
   ...
   if (lvalue)
   {
    if (v.isLocalOrParam())
    {
     ...
     else v.setChangedSpecial();
   }
   processFields(c);
   if (!v.used() && ((resultFields.size() > 1 && lvalue) || !isSafeExpr()))
    v.markUsed();
   if (c.currentTry != null)
    c.currentTry.setVarAccessed(
  ...
  if (v != null)
  {
   ...
   else
   {
    ...
    if (v != null && !v.isClassVariable())
    {
     ...
     else for (int j = resultFields.size() - 1; j >= 0; j--)
 ...
 private void processFields(Context c)
 {
  int i = resultFields.size();
  while (i-- > 0)
   if (((VariableDefinition) resultFields.elementAt(i)).isClassVariable())
    break;
  while (i > 0)
   resultFields.removeElementAt(--i);
  ...
  i = 0;
  while (++i < count)
   c.addAccessedClass(((VariableDefinition)
    resultFields.elementAt(i)).definingClass());
 }
 ...
 ExpressionType exprType()
 {
  ...
  return resultFields.size() > 0 ?
          ((VariableDefinition) resultFields.elementAt(
          resultFields.size() - 1)).exprType() : callerClass;
 }
 ...
 ExpressionType actualExprType()
 {
  assertCond(callerClass != null);
  int count = resultFields.size();
  return count > 0 ? (count == 1 && firstActualType != null ?
          firstActualType : ((VariableDefinition) resultFields.elementAt(
          count - 1)).actualExprType()) : exprType();
 }
 ...
 ConstValue evaluateConstValue()
 {
  assertCond(callerClass != null);
  return resultFields.size() == 1 ?
   ((VariableDefinition) resultFields.elementAt(0)).evaluateConstValue() :
   null;
 }
 ...
 boolean isImmutable()
 {
  ...
  {
   VariableDefinition v = (VariableDefinition) resultFields.elementAt(0);
   if ((!isClinitSafe && v.isClassVariable() &&
       v.definingClass().classInitializerNotCalledYet()) ||
       !v.isImmutable(callerClass))
    return false;
   for (int i = 1; i < count; i++)
    if (!((VariableDefinition) resultFields.elementAt(i)).isImmutable(null))
     return false;
  }
  return true;
 }
  ...
  if (lastInd >= 0)
  {
   ...
   for (int i = lastInd - 1; i >= 0; i--)
    if ((i > 0 || !isFirstNotNull) &&
        !((VariableDefinition) resultFields.elementAt(i)).isNotNull())
     return false;
  }
  return true;
 }
 ...
 boolean isFieldAccessed(VariableDefinition v)
 {
  ...
  if (resultFields.size() == 0)
   return false;
  VariableDefinition field = (VariableDefinition) resultFields.elementAt(0);
  return v != null ? field == v : resultFields.size() > 1 ||
          (!field.isLocalOrParam() && (!field.isImmutable(callerClass) ||
          (!isClinitSafe && field.isClassVariable() &&
          field.definingClass().classInitializerNotCalledYet())));
 }
 ...
 boolean isNotNull()
 {
  assertCond(callerClass != null);
  int count = resultFields.size();
  return count == 0 || (count == 1 && isFirstNotNull) ||
          ((VariableDefinition) resultFields.elementAt(
          count - 1)).isNotNull();
 }
 ...
 VariableDefinition getVariable(boolean allowInstance)
 {
  assertCond(callerClass != null);
  int count = resultFields.size();
  return count == 0 ? VariableDefinition.THIS_VAR :
          allowInstance || count == 1 ?
          (VariableDefinition) resultFields.elementAt(count - 1) : null;
 }
 ...
 String strLiteralValueGuess()
 {
  ...
  return resultFields.size() > 0 ?
          ((VariableDefinition) resultFields.elementAt(
          resultFields.size() - 1)).strLiteralValueGuess() : null;
 }
 ...
 ExpressionType classLiteralValGuess()
 {
  ...
  return resultFields.size() > 0 ?
          ((VariableDefinition) resultFields.elementAt(
          resultFields.size() - 1)).classLiteralValGuess() : null;
 }
 ...
 boolean storeClassLiteralsGuess(ObjVector parmSig, boolean isActual)
 {
  ...
  return resultFields.size() > 0 &&
          ((VariableDefinition) resultFields.elementAt(
          resultFields.size() - 1)).storeClassLiteralsGuess(parmSig,
          isActual);
 }
 ...
 MethodInvocation getClassNewInstanceCall()
 {
  ...
  return resultFields.size() > 0 ?
          ((VariableDefinition) resultFields.elementAt(
          resultFields.size() - 1)).getClassNewInstanceCall() : null;
 }
 ...
 MethodSignature getConstructorInstanceSign()
 {
  ...
  return resultFields.size() > 0 ?
          ((VariableDefinition) resultFields.elementAt(
          resultFields.size() - 1)).getConstructorInstanceSign() : null;
 }
 ...
 void discoverObjLeaks()
 {
  ...
  {
   assertCond(callerClass != null);
   ((VariableDefinition) resultFields.elementAt(0)).discoverLocalVolatile();
  }
 }
 ...
 void setStackObjVolatile()
 {
  ...
  if (resultFields.size() == 1)
   ((VariableDefinition) resultFields.elementAt(0)).setStackObjVolatile();
   else
   {
    if (resultFields.size() == 0)
    {
     if (Main.dict.ourMethod != null)
      ...
      else callerClass.setInitThisStackObjVolatile();
    }
   }
 }
  ...
  if (resultFields.size() == 1)
  {
   ...
   if (field != v)
   {
    if (field.isLocalOrParam())
    {
     ...
     ObjVector assignedTerms = (ObjVector) assignedLocals.get(field);
   ...
   else
   {
        ...
        (v == null || !v.addSetObjLeaksTerm(this)))
    {
     if (Main.dict.ourMethod != null)
      ...
      else callerClass.setInstanceInitLeaks();
    }
   }
 }
 ...
 {
  return resultFields.size() == 1 ? ((VariableDefinition)
          resultFields.elementAt(0)).tokensExpandedCount() : 0;
 }
 ...
 String stringOutput()
 {
  ...
  int count = resultFields.size();
  if (count == 0)
   return setOnlyOrError ? VariableDefinition.UNKNOWN_NAME : This.CNAME;
  VariableDefinition v = (VariableDefinition) resultFields.elementAt(0);
  String resultString = v.isClassVariable() ? v.stringOutputForStatic(
                         isClinitSafe, lvalue && count == 1) :
                         v.stringOutput(true, lvalue && count == 1);
  if (count > 1)
  {
 ...
 ExpressionType traceClassInit()
 {
  ...
  return setOnlyOrError ? null : lastInd < 0 ?
          Main.dict.curTraceInfo.curThisClass() :
          ((VariableDefinition) resultFields.elementAt(
          lastInd)).traceClassInit();
 }
}
