...
public final class Main
{
 ...
 static final String VER_ABBR =
  "JCGO_" + VERSION.substring(0, VERSION.length() - 3) +
 ...
 {
  System.out.println(
   "Usage: jcgo [options] <target_class> [<additional_class1> ...]");
  ...
  try
  {
   ...
   for (int i = 0; i < argv.length; i++)
    if (argv[i].equals("-verbose") || argv[i].equals("-v"))
     dict.verbose = true;
     else if (argv[i].equals("-sourcepath") || argv[i].equals("-src"))
     {
      if (i + 1 < argv.length)
        ...
        else if (argv[i].startsWith("-"))
        {
          ...
          dict.ignoreErrs = true;
           ...
           dict.failOnClassName = argv[++i];
            ...
            dict.failOnFieldName = argv[++i];
             ...
             else if (argv[i].startsWith("-l") && dict.failOnClassLimit == 0)
         ...
         else
         {
          ...
          if (className.charAt(0) == '@')
           readResponseFile(classnames, className.length() == 1 &&
            i + 1 < argv.length ? argv[++i] : className.substring(1),
            progBasePath);
           else if (mainClassName != null)
            classnames.addLast(dict.javaFiles.toClassName(className));
            else mainClassName = dict.javaFiles.toClassName(className);
         }
   ...
   if (dict.outPath.length() == 0)
   {
    ...
    {
     System.err.println("Use -d to specify existing directory for output");
     System.exit(2);
    }
   ...
   {
    ObjQueue proxyIfaceNames =
     dict.nameMapper.decodeProxyClassName(sortednames[i]);
    if (proxyIfaceNames != null)
    {
     Enumeration en = proxyIfaceNames.elements();
     while (en.hasMoreElements())
      dict.get((String) en.nextElement()).predefineClass(null);
    }
     else dict.get(sortednames[i]).predefineClass(null);
   }
  ...
  catch (OutOfMemoryError e)
  {
   ...
   try
   {
    System.err.print("(consumed " +
     Integer.toString(getAmountOfUsedMemory() - consumed) + " KiB)");
   }
 ...
 private static void processReflectOption(String flags, String packageOrClass)
 {
  ...
  if (dict.forcedReflectClassOrPkg.put(packageOrClass, flags) != null ||
      packageOrClass.indexOf('/', 0) > 0 || (File.separatorChar != '/' &&
      packageOrClass.indexOf(File.separatorChar, 0) > 0))
  {
   System.err.println("Invalid or duplicate -r<flags> for: " +
    (packageOrClass.length() > 0 ? packageOrClass : "(unnamed package)"));
   System.exit(2);
  }
 }
 ...
 // DataInputStream infile = new DataInputStream(new BufferedInputStream(new FileInputStream(fname)));
   String str;
  ...
  while (en.hasMoreElements())
  {
       ...
       ch != ';')
   {
    ...
    if (path != null || (path = cutLinePrefix(str, "-src")) != null)
     ...
     else if (str.startsWith("-r") && ((pos = str.indexOf(' ', 2)) >= 0 ||
              (pos = str.indexOf('\t', 2)) >= 0))
      processReflectOption(str.substring(2, pos),
       str.substring(pos + 1).trim());
      else classnames.addLast(dict.javaFiles.toClassName(str));
   }
  }
 }
 ...
 private static String cutLinePrefix(String str, String prefix)
 {
  return str.startsWith(prefix) && str.length() > prefix.length() &&
          ...
          str.substring(prefix.length() + 1).trim() : null;
 }
   ...
   String[] sortednames, int time, boolean skipClinitTrace)
 {
  ...
  for (int i = 0; i < sortednames.length; i++)
  {
   ObjQueue proxyIfaceNames =
    dict.nameMapper.decodeProxyClassName(sortednames[i]);
   if (proxyIfaceNames == null || dict.addProxyClass(
  ...
  if (dict.forcedReflectClassOrPkg.size() > 0)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (packageOrClass.length() > 0 && !packageOrClass.endsWith(".*"))
  ...
  if (dict.hasReflectedMethods && !mainReflected &&
   ...
   dict.producePassOne();
  dict.allowConstStr =
   Main.dict.get(Names.JAVA_LANG_STRING).otherInstanceFieldsUnused(
  ...
  dict.get(Names.JAVA_LANG_OBJECT).produceOutputRecursive(new ObjHashSet());
  ...
  System.out.println("Produced: " + Integer.toString(dict.outFilesCount) +
   " c/h files (" + Integer.toString(dict.outBytesCount >> 10) + " KiB).");
  System.out.println("Contains: " + Integer.toString(dict.methodsWritten) +
   " java methods, " + Integer.toString(dict.normalCalls) + " normal and " +
   Integer.toString(dict.indirectCalls) + " indirect calls.");
  System.out.println("Done conversion in " +
   Integer.toString(((int) System.currentTimeMillis() - time) / 1000) +
   " seconds. Total heap size: " + Integer.toString(
   (int) (Runtime.getRuntime().totalMemory() >> 10)) + " KiB.");
  if (dict.errorsCnt > 0)
  {
   System.out.println("");
   System.out.println(" Translated with " + dict.errorsCnt + " errors!");
   System.exit(3);
  }
 }
}
