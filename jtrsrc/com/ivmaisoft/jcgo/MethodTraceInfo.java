...
final class MethodTraceInfo
{
   ...
   ExpressionType curActualClass, ObjVector parmTraceSig)
 {
  ...
  if (curActualClass == null)
   curActualClass = md.definingClass();
    ...
    curActualClass = cd;
  if (parmTraceSig != null && md.methodSignature().isSignEqual(parmTraceSig))
   parmTraceSig = null;
  ...
  MethodTraceInfo traceInfo =
   (MethodTraceInfo) knownMethodInfos.get(traceSig);
  if (traceInfo == null)
   knownMethodInfos.put(traceSig, traceInfo =
    new MethodTraceInfo(md, curActualClass, parmTraceSig));
  return traceInfo;
 }
   ...
   ExpressionType curActualClass, ObjVector parmTraceSig)
 {
  return md != null ? md.methodSignature().signatureString() +
          (md.definingClass() != curActualClass ?
          curActualClass.getJavaSignature() : "") +
 ...
 String getTraceSig()
 {
  return getDefiningClassName() + "." + getTraceShortSig(md, curActualClass,
          parmTraceSig);
 }

 boolean addToProcessed(ObjHashtable processed)
 {
  ClassDefinition cd = md != null ? md.definingClass() :
                        curActualClass.receiverClass();
 ...
 {
  Term.assertCond(resTraceType == null);
  if (classInitDepend == null || classInitDepend.identityLastIndexOf(cd) < 0)
  {
   int i = weakInitDepend != null ?
            weakInitDepend.identityLastIndexOf(cd) : -1;
   if (isWeak)
   {
    if (i < 0)
    {
 ...
 void addMethodCall(MethodTraceInfo traceInfo)
 {
  ...
  {
   int i = weakMethodInfos != null ?
            weakMethodInfos.identityLastIndexOf(traceInfo) : -1;
 ...
 void traceMethod()
 {
  if (resTraceType == null && Main.dict.inProgressMethodInfos.add(this))
  {
   ...
   doneMethodTracing(curTraceType != null ? curTraceType : md.exprType());
 ...
 {
  return resTraceType != null ? resTraceType : md != null ?
          md.actualExprType() : null;
 }
 ...
 ClassDefinition getClassDependElementAt(boolean isWeak, int index)
 {
  ...
  if (classDepend != null && classDepend.size() > index)
  {
   do
   {
    ClassDefinition cd = (ClassDefinition) classDepend.elementAt(index);
