...
final class RelationalOp extends LexNode
{
 ...
 void processPass1(Context c)
 {
  if (exprType0 == null)
  {
   ...
   if (s0 < Type.CLASSINTERFACE && s0 != Type.NULLREF ? (s0 != Type.BOOLEAN ?
       s0 == Type.VOID || s2 <= Type.BOOLEAN || s2 > Type.DOUBLE :
       s2 != Type.BOOLEAN) : (sym != LexTerm.EQ && sym != LexTerm.NE) ||
       (s2 >= Type.BOOLEAN && s2 <= Type.VOID))
    fatalError(c, "Inappropriate type of expressions for comparison");
   ConstValue constVal0 = terms[0].evaluateConstValue();
   if (constVal0 != null && (constVal =
       constVal0.comparisonOp(terms[2].evaluateConstValue(), sym)) == null &&
 ...
 {
  int sym;
  if (constVal == null && ((sym = terms[1].getSym()) == LexTerm.EQ ||
      sym == LexTerm.NE))
  {
   int s0 = exprType0.objectSize();
   if (sym != LexTerm.EQ)
   ...
   if (s0 == Type.BOOLEAN)
   {
    ...
    if (constVal2 != null)
     terms[0].updateCondBranch(c, constVal2.isNonZero() == forTrue);
   }
    else
    {
     if (!forTrue && (s0 >= Type.CLASSINTERFACE || s0 == Type.NULLREF))
     {
 ...
 boolean isLiteral()
 {
  return constVal != null || (terms[0].isLiteral() && terms[2].isLiteral());
 }
 ...
 boolean isSafeExpr()
 {
  return constVal != null || (terms[0].isSafeExpr() && terms[2].isSafeExpr());
 }
 ...
 int tokenCount()
 {
  ...
  Term t0 = terms[0];
  Term t2 = terms[2];
  return t0.tokenCount() + t2.tokenCount() +
          (t0.isSafeExpr() || t2.isSafeExpr() ?
          (t0.isImmutable() || t2.isImmutable() ? 0 : 1) : 2);
 }
 ...
 void processOutput(OutputContext oc)
 {
  ...
  int sym = terms[1].getSym();
  if ((s0 == Type.FLOAT || s0 == Type.DOUBLE || s2 == Type.FLOAT ||
      s2 == Type.DOUBLE) && !isLiteral())
  {
   if (s0 < Type.FLOAT)
    cast0 = Type.cName[s2];
    else if (s2 < Type.FLOAT)
     cast2 = Type.cName[s0];
   oc.cPrint(sym == LexTerm.EQ ? "JCGO_FP_EQU" : sym == LexTerm.NE ?
    "!JCGO_FP_EQU" : sym == LexTerm.LT || sym == LexTerm.GT ? "JCGO_FP_LT" :
    "JCGO_FP_LQ");
    ...
    if (s2 != Type.CHAR)
     ...
     else
     {
      if (s0 >= Type.CLASSINTERFACE && s2 >= Type.CLASSINTERFACE &&
          exprType0 != exprType2 && (s0 == Type.CLASSINTERFACE ||
          s2 == Type.CLASSINTERFACE))
      {
       if (s0 != Type.CLASSINTERFACE)
        cast2 = exprType0.castName();
        else if (s2 != Type.CLASSINTERFACE ||
                 ClassDefinition.isAssignableFrom(exprType2, exprType0, null))
         cast0 = exprType2.castName();
         else cast2 = exprType0.castName();
      }
