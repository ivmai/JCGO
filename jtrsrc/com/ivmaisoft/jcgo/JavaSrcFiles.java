...
final class JavaSrcFiles
{
 ...
 static String adjustPathPrefix(String path, String progBasePath)
 {
  ...
  return path.startsWith("$~" + File.separator) ?
          progBasePath + path.substring(2) : path;
 }
  ...
  if (pathList != null && pathList.length() > 0)
  {
   ...
   do
   {
    ...
    if (i < j)
    {
     ...
     if (k >= 0 && path.length() > k + 3 && path.indexOf("..", k + 3) < 0 &&
         path.charAt(k + 3) != '.' && path.charAt(path.length() - 1) != '.' &&
         path.indexOf(File.separatorChar, k + 3) < 0)
      realPath = path.substring(0, k + 1) +
                  path.substring(k + 3).replace('.', File.separatorChar);
     if (!(new File(realPath)).isDirectory() && (realPath == path ||
         !(new File(realPath + JAVA_EXT)).isFile()))
     {
      System.err.println("Source path does not exist: " + realPath);
      if (!Main.dict.ignoreErrs)
       System.exit(2);
     }
     if (path != origPath)
     {
      pathList = pathList.substring(0, i) + path + pathList.substring(j);
 ...
 String classFilename(String className, boolean allowInner)
 {
  ...
  do
  {
   ...
   if (i < j)
   {
    ...
    if (k >= 0)
    {
     condOfPrefix = prefix.substring(k + 3).replace('.', File.separatorChar);
     prefix = prefix.substring(0, k + 1);
    }
    String name = pathName;
    do
    {
     if ((k < 0 || name.startsWith(condOfPrefix + File.separatorChar) ||
         name.equals(condOfPrefix + JAVA_EXT)) && pathExists(prefix, name))
     {
