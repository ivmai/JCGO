 ...
 **
 * Formats:
 * NEW ClassOrIfaceType/PrimitiveType DimsList
 * NEW ClassOrIfaceType/PrimitiveType DimsList [Dims]
 */
...
final class ArrayCreation extends LexNode
{
 ...
 void processPass1(Context c)
 {
  if (cd == null)
  {
   ...
   if (dims == 1 && (constVal1 = terms[1].evaluateConstValue()) != null &&
       constVal1.getIntValue() >= 0)
   {
    ...
    if ((oldModifiers & AccModifier.STATIC) != 0 &&
        ...
        MAX_LITERAL_SIZE) / elemSize >= constVal1.getIntValue())
    {
     allowLiteral = true;
     ...
     {
      if (MAX_ONSTACK_SIZE / elemSize >= constVal1.getIntValue())
      {
 ...
 private void outputNewArray(OutputContext oc)
 {
  ...
  oc.cPrint(Type.cName[s0 < Type.CLASSINTERFACE ?
   s0 + Type.CLASSINTERFACE : Type.OBJECTARRAY]);
  ...
  if (allowLiteral)
  {
   ...
   if (count > 0)
   {
    String element = innerType.objectSize() < Type.CLASSINTERFACE ? "(" +
                      innerType.castName() + ")0, " : LexTerm.NULL_STR + ", ";
    for (int i = 0; i < count; i++)
     sb.append(element);
   }
   ArrayLiteral literal = new ArrayLiteral(innerType, sb.toString(), count,
                           isWritable);
   literal.setNotSharable();
   oc.cPrint(Main.dict.addArrayLiteral(literal, ourClass,
    false).stringOutput());
  }
