...
final class ClassDictionary
{
 ...
 final ClassDefinition[] classTable =
  new ClassDefinition[Type.OBJECTARRAY + 1];
 ...
 void setupCoreClasses()
 {
  ...
  for (int arrtype = Type.CLASSINTERFACE + Type.BOOLEAN;
       arrtype <= Type.CLASSINTERFACE + Type.VOID; arrtype++)
  {
   ...
   cd.addField(new VariableDefinition(cd, "length", AccModifier.PUBLIC |
    AccModifier.FINAL | AccModifier.SYNTHETIC, classTable[Type.INT],
    Empty.newTerm(), false));
  }
   ...
   ClassDefinition forClass)
 {
  ...
  if ((lookLikeClassname(s) || ((s1 = s.replace('/', '.')) != s &&
      lookLikeClassname(s1))) && ((currentClass != null &&
      (i = currentClass.name().lastIndexOf('.')) > 0 &&
      existsOrInner(s = currentClass.name().substring(0, i + 1) + s1)) ||
      existsOrInner(s = s1)))
  {
   cd = get(s);
 ...
 {
  int i = str.length();
  if (i > 0 && str.charAt(i - 1) != '.' && NameMapper.isLetter(str.charAt(0)))
  {
   while (--i > 0)
   {
    char ch = str.charAt(i);
    if (!NameMapper.isLetter(ch) && ch != '_' && ch != '$' && ((ch != '.' &&
        (ch < '0' || ch > '9')) || str.charAt(i - 1) == '.'))
     break;
   }
   if (i == 0)
    return true;
   ...
   ClassDefinition forClass)
 {
  ...
  if (classLiteralValue != null)
  {
   ...
   if (cd.isFinal() || (cd != classLiteralValue &&
       classLiteralValue.objectSize() == Type.CLASSINTERFACE))
   {
    exactGetNameClasses.add(cd);
   ...
   else cd = get(Names.JAVA_LANG_OBJECT);
  int i = usedGetNameClasses.size();
  while (i-- > 0)
   if (((ClassDefinition) usedGetNameClasses.elementAt(i)).isAssignableFrom(
       cd, 0, forClass))
    return;
  i = usedGetNameClasses.size();
  while (i-- > 0)
   if (cd.isAssignableFrom((ClassDefinition) usedGetNameClasses.elementAt(i),
       0, forClass))
 ...
 boolean isClassNameUsed(ClassDefinition ourClass)
 {
  ...
  while (en.hasMoreElements())
   if (((ClassDefinition) en.nextElement()).isAssignableFrom(ourClass, 0,
       null))
    return true;
  return false;
 }
 ...
 void putCharSubArray(ArrayLiteral liter)
 {
  ...
  do
  {
   ...
   ArrayLiteral liter2 =
    (ArrayLiteral) arrayStrs.addStartsWith(key, liter);
 ...
 void producePassOne()
 {
  ...
  do
  {
   ...
   while (en.hasMoreElements())
    cnt += ((ClassDefinition) allClasses.get(
            en.nextElement())).producePassOne();
  } while (cnt > 0);
 }
   ...
   for (int i = 0; i < count; i++)
   {
    ...
    for (int j = 0; j < i; j++)
     if (parmSig.elementAt(j) == cd)
      return null;
    className = className + nameMapper.nameToProxyNamePart(cd.name());
   }
  ...
  if (get(Names.JAVA_LANG_VMSTRING).used())
  {
   ...
   if (len > 0)
   {
    ...
    while (en.hasMoreElements())
    {
     ...
     oc.cPrint(")");
     oc.cPrint(((LiteralStr) strpool.get(en.nextElement())).stringOutput());
     next = true;
    }
   }
