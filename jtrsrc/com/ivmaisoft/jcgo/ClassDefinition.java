...
final class ClassDefinition extends ExpressionType
{
 ...
 {
  return "(" + castName() + ")jcgo_newObject((jvtable)&" +
          vTableCName() + ")";
 }
 ...
 static String arrayVTableCName(int type, int dims)
 {
  ...
  return Type.cName[Type.CLASSINTERFACE + type] + (dims > 0 ?
          Integer.toString(dims + 1) : "") + "_methods";
 }
 ...
 ExpressionType asExprType(int dims)
 {
  ...
  ExpressionType exprType = (ExpressionType) arrayClasses.elementAt(dims - 1);
  if (exprType == null)
  {
 ...
 boolean hasInstantatedSubclasses(boolean allowArrays)
 {
  ...
  if (implementedBy != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if ((cd.hasInstances || (allowArrays &&
        (cd.forArraysUsed || cd.vTableUsed))) && cd.used)
     return true;
   }
  }
 ...
 {
  return exprType1 == exprType2 || exprType2.objectSize() == Type.NULLREF ||
          exprType1.signatureClass().isAssignableFrom(
          exprType2.signatureClass(), exprType1.signatureDimensions() -
  ...
  if (cd1 != cd2)
  {
   ...
   if (cd1.isInterface() && cd2.isInterface())
   {
    ...
    while (en.hasMoreElements())
    {
     ...
     {
      ClassDefinition cd =
       maxSuperclassOf(iface1, (ClassDefinition) en2.nextElement(), forClass);
   ...
   ExpressionType exprType2, ClassDefinition forClass)
 {
  ...
  return (dims1 != dims2 ? Main.dict.get(Names.JAVA_LANG_OBJECT) :
          ...
          exprType2.signatureClass(), forClass)).asExprType(dims1 < dims2 ?
          dims1 : dims2);
 }
 ...
 int getVMExcMask()
 {
  return name.equals(Names.JAVA_LANG_THROWABLE) ||
          ...
          name.equals(Names.JAVA_LANG_RUNTIMEEXCEPTION) ?
          ARR_STORE_EXC | CLASS_CAST_EXC | INDEX_OUT_EXC | NULL_PTR_EXC :
          ...
          name.equals(Names.JAVA_LANG_ARRAYINDEXOUTOFBOUNDSEXCEPTION) ?
          INDEX_OUT_EXC : name.equals(Names.JAVA_LANG_NULLPOINTEREXCEPTION) ?
          NULL_PTR_EXC : 0;
 }

 String getJniName()
 {
  return type != Type.CLASSINTERFACE ? Type.jniName[type] :
          name.equals(Names.JAVA_LANG_CLASS) ? "jclass" :
          name.equals(Names.JAVA_LANG_STRING) ? "jstring" :
          !isInterface() && isThrowable() ? "jthrowable" :
  ...
  while (en.hasMoreElements())
  {
   ...
   if (cd.used || cd.hasExactInstance)
   {
    ...
    if (md2 != null ? md2.isClassMethod() ||
        ...
        cd.hasExactInstance || !cd.overriddenByAllUsed(md))
     return false;
   }
  }
  return true;
  ...
  if (type == Type.CLASSINTERFACE && processed.add(this))
  {
   if (used)
   {
        ...
        !name.equals(Names.JAVA_LANG_STRING))
    {
     if (superClass == null)
     {
      oc.addIncludeCFile(Main.dict.get(Names.JAVA_LANG_STRING).shortname());
      oc.addIncludeCFile(Main.dict.get(Names.JAVA_LANG_CLASS).shortname());
     }
    ...
    oc.hPrint("\010");
    if (superClass == null)
     n = Main.dict.get(Names.JAVA_LANG_STRING).enumerateClass(oc,
          Main.dict.get(Names.JAVA_LANG_CLASS).enumerateClass(oc, 63,
          processed), processed);
   }
   Enumeration en = subclasses.elements();
   while (en.hasMoreElements())
    n = ((ClassDefinition) en.nextElement()).enumerateClass(oc, n, processed);
   if (used)
   {
 ...
 void produceOutputRecursive(ObjHashSet processed)
 {
  if (type == Type.CLASSINTERFACE && processed.add(this))
  {
   ...
   Enumeration en = subclasses.elements();
   while (en.hasMoreElements())
    ((ClassDefinition) en.nextElement()).produceOutputRecursive(processed);
  }
 }
    ...
    if (src[mid - 1].compareTo(src[mid]) <= 0)
     ...
     else
     {
      ...
      for (int i = low; i < high; i++)
       dest[i] = q < high && (p >= mid || src[p].compareTo(src[q]) > 0) ?
                  src[q++] : src[p++];
     }
   }
 }
 ...
 private void createSubclassList(ObjHashSet classnames)
 {
  if (type == Type.CLASSINTERFACE)
  {
   ...
   Enumeration en = subclasses.elements();
   while (en.hasMoreElements())
    ((ClassDefinition) en.nextElement()).createSubclassList(classnames);
  }
 }
 ...
 private void addToHeaderFile(OutputContext oc)
 {
  if (type == Type.CLASSINTERFACE && !addedToHeader)
  {
   ...
   {
    if (superClass == null)
     oc.addIncludeHFile(Main.dict.get(Names.JAVA_LANG_STRING).shortname());
    oc.addIncludeHFile(shortname());
    if (superClass == null)
     oc.addIncludeHFile(Main.dict.get(Names.JAVA_LANG_CLASS).shortname());
   }
   Enumeration en = subclasses.elements();
   while (en.hasMoreElements())
 ...
 void defineProxyClass(ObjVector interfaces, ClassDefinition forClass)
 {
  ...
  for (i = 0; i < count; i++)
  {
   ...
   if (i > 0)
   {
    ...
    while (en.hasMoreElements())
    {
     ...
     ExpressionType minType = cd.getMethodNoInheritance(sigString).exprType();
     ExpressionType maxType = minType;
     for (int j = 0; j < i; j++)
     {
      MethodDefinition md =
       ((ClassDefinition) interfaces.elementAt(j)).getMethod(sigString);
      if (md != null)
      {
       ...
       if (isAssignableFrom(maxType, resType, forClass))
        ...
        {
         if (isAssignableFrom(resType, minType, forClass))
          minType = resType;
          else
          {
           if (maxType == minType || !isAssignableFrom(minType, resType,
               forClass) || !isAssignableFrom(resType, maxType, forClass))
            return;
          }
        }
      }
  ...
  if (count > 0)
  {
   i = count - 1;
   ifaceListTerm =
    new ClassOrIfaceType((ClassDefinition) interfaces.elementAt(i));
   while (i-- > 0)
    ifaceListTerm = new Seq(new ClassOrIfaceType(
                     (ClassDefinition) interfaces.elementAt(i)),
                     ifaceListTerm);
  }
  definePass0(new Context(), AccModifier.PUBLIC | AccModifier.FINAL,
  ...
  while (en.hasMoreElements())
  {
   ...
   if (!md.isConstructor())
   {
    MethodDefinition md2 = getOverridenMethod(sigString, md.exprType());
 ...
 MethodDefinition getOverridenMethod(String sigString, ExpressionType resType)
 {
  ...
  while (en.hasMoreElements())
  {
   md = ((ClassDefinition) en.nextElement()).getMethodNoInheritance(
         sigString);
 ...
 private boolean checkMarkIfDefined()
 {
  ...
  if (!checkMarkDone)
  {
   ...
   if (flags != null || (outerMethodDefnTerm == null &&
 ...
 private void forceReflection(String flags)
 {
  ...
  if (reflectFields >= 0)
  {
   ...
   while (en.hasMoreElements())
   {
    VariableDefinition v =
     (VariableDefinition) fieldDictionary.get(en.nextElement());
  ...
  if ((reflectCtors & reflectMethods) >= 0)
  {
   ...
   while (en.hasMoreElements())
   {
    MethodDefinition md = getMethodNoInheritance((String) en.nextElement());
    if (md.isConstructor() ? reflectCtors >= 0 && (reflectCtors == 0 ||
        md.isPublic()) : reflectMethods >= 0 && md.definingClass() == this &&
        (reflectMethods == 0 || (md.isPublic() && (reflectMethods == 1 ||
        md.isGetterSetter()))))
     reflectMethodInner(md);
   }
  }
 ...
 void predefineClassNoMark()
 {
  if (!parsed && type == Type.CLASSINTERFACE)
  {
   ...
   if (!parsed)
    throw new TranslateException("File bad for class: " + name +
 ...
 {
  predefining = true;
  if (fromClass == null && forClass != this && type == Type.CLASSINTERFACE)
   fromClass = forClass;
  predefineClassNoMark();
 }
   ...
   Term ifaceListTerm, Term classbody, boolean isInterface)
 {
  ...
  if (parsed)
  {
   classbody.fatalError(c,
    "Class duplicate or recursive definition: " + name);
   return;
  }
  parsed = true;
  context = c;
 ...
 void processPass1(Context c)
 {
  ...
  if (outerScope != null)
  {
   ...
   if (isDefined)
   {
    classbody.fatalError(c, "Local class is used outside its scope: " + name);
    return;
   }
   ...
   {
    VariableDefinition v =
     c.currentMethod.getLocalVar((String) en.nextElement());
   ...
   if (implementedBy == null || !ifaceListTerm.notEmpty())
   {
    ...
    if (superClass.implementedBy != null)
     classbody.fatalError(context,
      "A class cannot be extended from an interface: " + superClass.name);
 ...
 private void addFieldsToInner(ClassDefinition cd)
 {
  ...
  if (superClass != null)
  {
   ...
   if (!cd.interfaceClasses.contains(this))
   {
    ...
    while (en.hasMoreElements())
    {
     ClassDefinition classDefn = (ClassDefinition) en.nextElement();
 ...
 void addFieldsTo(ClassDefinition cd)
 {
  ...
  if (implementedBy == null)
   classbody.fatalError(context,
    "Not an interface is specified after 'implements': " + name);
 ...
 private void definePass1(ClassDefinition forClass)
 {
  ...
  {
   int nesting = 0;
   for (ClassDefinition cd = outerClass; !cd.isStaticClass();
        cd = cd.outerClass)
    nesting++;
  ...
  if (outerScope != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (superClass == null || superClass.outerClass() != outerClass ||
        (field = superClass.getOuterField(varName, this)) == null)
     addField(field = new VariableDefinition(this, "val$" + varName,
      AccModifier.FINAL | AccModifier.SYNTHETIC, v.exprType(),
      Empty.newTerm(), false));
  ...
  if (!context.hasConstructor && implementedBy == null)
  {
   ...
   if (isProxy)
   {
    ...
    {
     MethodDefinition md =
      objectClass.getMethodNoInheritance((String) en.nextElement());
     if (md.isPublic() && !md.isConstructor() && md.allowOverride())
     {
    ...
    constr =
     new TypeDeclaration(new AccModifier(AccModifier.PUBLIC |
     AccModifier.SYNTHETIC), new ConstrDeclaration(new LexTerm(LexTerm.ID,
     id), new FormalParameter(new AccModifier(AccModifier.SYNTHETIC),
     new ClassOrIfaceType(Main.dict.get(
     Names.JAVA_LANG_REFLECT_INVOCATIONHANDLER)), Empty.newTerm(),
     new VariableIdentifier(new LexTerm(LexTerm.ID, "handler")),
     Empty.newTerm()), Empty.newTerm(), new ExprStatement(new ConstructorCall(
     Empty.newTerm(), new Super(), new Argument(new Expression(
     new QualifiedName(new LexTerm(LexTerm.ID, "handler"))))))));
   }
    else
    {
     Term paramList = constrParamList;
     if (constrSuperType != null)
      paramList = FormalParamList.prepend(new FormalParameter(
                   new AccModifier(AccModifier.FINAL | AccModifier.SYNTHETIC),
                   new ClassOrIfaceType(constrSuperType.receiverClass()),
                   Empty.newTerm(), (new VariableIdentifier(new LexTerm(
                   LexTerm.ID, "this$00"))).setLineInfoFrom(classbody),
                   Empty.newTerm()), paramList);
     constr =
      new TypeDeclaration(new AccModifier((modifiers &
      (AccModifier.PUBLIC | AccModifier.PRIVATE | AccModifier.PROTECTED)) |
      ((modifiers & AccModifier.PUBLIC) != 0 ? 0 : AccModifier.SYNTHETIC)),
      (new ConstrDeclaration(new LexTerm(LexTerm.ID, id), paramList,
      Empty.newTerm(), Empty.newTerm())).setLineInfoFrom(classbody));
     classbody = (new Seq(classbody, constr)).setLineInfoFrom(classbody);
    }
   constr.processPass1(context);
   Main.dict.methodsAnalyzed--;
  }
  context.hasConstructor = oldHasConstructor;
  if (classInitializers != null && !classInitializers.isJavaConstant(this))
  ...
  while (en.hasMoreElements())
  {
   ...
   if (!checkOverridenMethodRetType(md))
    classbody.fatalError(context, "Incompatible return type in: " + name +
 ...
 private boolean checkOverridenMethodRetType(MethodDefinition md)
 {
  ...
  if (cd != null)
  {
   MethodDefinition md2 = cd.getMethodSamePkg(sigString, getPackageName());
   if (md2 != null)
   {
   ...
   boolean isProxy)
 {
  ...
  while (en.hasMoreElements())
  {
   MethodDefinition md2 =
    ((ClassDefinition) en.nextElement()).getMethodNoInheritance(sigString);
   if (md2 != null)
   {
    ExpressionType resType2 = md2.exprType();
    if (md == null || (resType != resType2 &&
        isAssignableFrom(resType, resType2, this)))
    {
     md = md2;
     resType = resType2;
    }
 ...
 void setHasAssertStmt()
 {
  ...
  if (fieldDictionary.get(Names.ASSERTIONSDISABLED) == null)
   addField(new VariableDefinition(this, Names.ASSERTIONSDISABLED,
    AccModifier.STATIC | AccModifier.FINAL | AccModifier.SYNTHETIC,
 ...
 {
  MethodDefinition md =
   Main.dict.get(Names.JAVA_LANG_OBJECT).getMethod(Names.SIGN_FINALIZE);
  if (md != null && Main.dict.get(Names.JAVA_LANG_VMRUNTIME).used)
   md.setVirtual();
  Main.dict.allowConstClass =
   Main.dict.get(Names.JAVA_LANG_CLASS).otherInstanceFieldsUnused(
   Names.fieldsOrderClass);
  VariableDefinition v = Main.dict.get(Names.JAVA_LANG_STRING).getField(
                          Names.fieldsOrderString[
                          Names.fieldsOrderString.length - 1], null);
 ...
 static void markAllDirectIfaces()
 {
  if (!Main.dict.markDirectIfaces)
  {
   ...
   {
    Enumeration en2 =
     ((ClassDefinition) en.nextElement()).specifiedInterfaces.elements();
    while (en2.hasMoreElements())
 ...
 static void processEnumValueOf()
 {
  if (!Main.dict.hasEnumValueOf)
  {
   ...
   Enumeration en = Main.dict.get(Names.JAVA_LANG_ENUM).subclasses.elements();
   while (en.hasMoreElements())
   {
    ClassDefinition cd = (ClassDefinition) en.nextElement();
    if (cd.used)
 ...
 private boolean isEnumValueField(VariableDefinition v)
 {
  ...
  return v.isClassVariable() && v.isFinalVariable() && !v.isPrivate() &&
          ((resType = v.exprType()) == this ||
          resType.receiverClass().superClass() == this);
 }
 ...
 private void processSerialization()
 {
  if (!isProxyClass())
  {
   if (name.equals(Names.JAVA_IO_OBJECTINPUTSTREAM))
   {
    Enumeration en = Main.dict.usedClasses();
    while (en.hasMoreElements())
     ((ClassDefinition) en.nextElement()).markForObjectInputStream();
   }
   ...
   if (name.equals(Names.JAVA_IO_OBJECTOUTPUTSTREAM))
   {
    Enumeration en = Main.dict.usedClasses();
    while (en.hasMoreElements())
     ((ClassDefinition) en.nextElement()).markForObjectOutputStream();
   }
   ...
   if (name.equals(Names.JAVA_IO_OBJECTSTREAMCLASS))
   {
    Enumeration en = Main.dict.usedClasses();
    while (en.hasMoreElements())
     ((ClassDefinition) en.nextElement()).markForObjectStreamClass();
   }
 ...
 {
  ClassDefinition serializableClass =
   Main.dict.get(Names.JAVA_IO_SERIALIZABLE);
  if (!isInterface() && serializableClass.isAssignableFrom(this, 0, this))
  {
   if (!isAbstractOrInterface())
   {
    ...
    {
     ClassDefinition sc = superClass;
     while (sc != null && serializableClass.isAssignableFrom(sc, 0, this))
      sc = sc.superClass();
 ...
 {
  if (!isInterface() && Main.dict.get(
      Names.JAVA_IO_SERIALIZABLE).isAssignableFrom(this, 0, this))
  {
 ...
 private void markForObjectStreamClass()
 {
  ...
  {
   VariableDefinition v =
    (VariableDefinition) fieldDictionary.get(Names.SERIALVERSIONUID);
   if (v == null)
   {
    v = new VariableDefinition(this, Names.SERIALVERSIONUID,
         AccModifier.PRIVATE | AccModifier.STATIC | AccModifier.FINAL |
         AccModifier.SYNTHETIC, Main.dict.classTable[Type.LONG],
         new Expression(new IntLiteral(computeSerialVersion())), false);
    addField(v);
   }
   reflectFieldInner(v);
   if (!Main.dict.get(Names.JAVA_IO_EXTERNALIZABLE).isAssignableFrom(this, 0,
       this) && !name.equals(Names.JAVA_LANG_CLASS) &&
       !name.equals(Names.JAVA_LANG_STRING))
   {
    v = (VariableDefinition) fieldDictionary.get(
         Names.SERIALPERSISTENTFIELDS);
    if (v != null && v.isClassVariable())
     reflectFieldInner(v);
    Enumeration en = fieldDictionary().keys();
    while (en.hasMoreElements())
    {
     v = (VariableDefinition) fieldDictionary.get(en.nextElement());
 ...
 InitializerPart addInitializer(Term term, boolean isClassVariable)
 {
  return isClassVariable ? (classInitializers = new InitializerPart(
          classInitializers, term)) : (instanceInitializers =
          new InitializerPart(instanceInitializers, term));
 }
 ...
 private ConstValue computeSerialVersion()
 {
  ...
  sha.updateUTF(name);
  int classaccess = modifiers & (AccModifier.PUBLIC | AccModifier.INTERFACE |
  ...
  if (size > 0)
  {
   ...
   for (int i = 0; i < size; i++)
   {
    VariableDefinition v = (VariableDefinition) fieldDictionary.get(names[i]);
    if (!v.isPrivate() || (!v.isClassVariable() && !v.isTransient()))
    {
     sha.updateUTF(names[i]);
     sha.updateInt(v.getJavaModifiers() & (AccModifier.PUBLIC |
      AccModifier.PRIVATE | AccModifier.PROTECTED | AccModifier.STATIC |
      AccModifier.FINAL | AccModifier.VOLATILE | AccModifier.TRANSIENT));
  ...
  if (size > 0)
  {
   ...
   for (int i = 0; i < size; i++)
   {
    ...
    sha.updateInt(
     getMethodNoInheritance(s.substring(k + 1)).getJavaModifiers() &
     (AccModifier.PUBLIC | AccModifier.PRIVATE | AccModifier.PROTECTED |
     AccModifier.STATIC | AccModifier.FINAL | AccModifier.SYNCHRONIZED |
 ...
 {
  if (Main.dict.get("java.rmi.server." + "RemoteStub").isAssignableFrom(this,
      0, this))
  {
   ...
   {
    if (Main.dict.get("java.rmi." + "Remote").isAssignableFrom(this, 0, this))
    {
     ...
     do
     {
      ...
      {
       MethodDefinition md =
        cd.getMethodNoInheritance((String) en.nextElement());
       if (!md.isConstructor())
       {
        ClassDefinition classDefn = md.definingClass();
        if (!md.isPrivate() && !classDefn.name.startsWith(Names.JAVA_0))
  ...
  if (Main.dict.get("org.omg.CORBA.portable." +
      "Streamable").isAssignableFrom(this, 0, this))
  {
 ...
 void markUsed()
 {
  if (!used)
  {
   ...
   if (--Main.dict.failOnClassLimit == 0)
    throw new TranslateException("Processed classes limit exceeded!");
   if (name.equals(Main.dict.failOnClassName) &&
       Main.dict.failOnMethodId == null && Main.dict.failOnFieldName == null)
   ...
   if (sc != null)
   {
    ...
    if (Main.dict.markDirectIfaces)
    {
     Enumeration en = specifiedInterfaces.elements();
     while (en.hasMoreElements())
      ((ClassDefinition) en.nextElement()).setVTableUsed(true);
    }
    if (sc.used)
    {
     ...
     {
      String sigString = (String) en.nextElement();
      String pkgName = (String) sc.usedMethodSigns.get(sigString);
      if (pkgName.length() == 0)
       pkgName = null;
      MethodDefinition md = getMethodNoInheritance(sigString);
      if (md != null && !md.isClassMethod() &&
          (pkgName == null || pkgName.equals(getPackageName())))
       md.markUsed(this, false);
       else markMethodInSubclasses(sigString, pkgName);
     }
    }
    ...
    if (sc.name.equals(Names.JAVA_UTIL_LISTRESOURCEBUNDLE) ||
        sc.name.equals(Names.COM_IVMAISOFT_JPROPJAV_STRLISTRESOURCEBUNDLE))
    {
     int pos = name.lastIndexOf('_');
     if (pos > 0 && name.indexOf('.', pos + 1) < 0)
      Main.dict.dynamicDefineClass(name.substring(0, pos), null, this);
    }
   }
   if (basicConstructor != null && !isInterface() &&
       ((Main.dict.markBasicCtors && !basicConstructor.isPrivate()) ||
   ...
   if (isObjectOrCloneable() || name.equals(Names.JAVA_LANG_THROWABLE))
   {
    ...
    else setHasExactInstance(false);
   ...
   if (name.equals(Names.JAVA_LANG_REFLECT_PROXY))
    reflectConstructors(false, Names.SIGN_INIT_INVOCATIONHANDLER, false);
   if (name.equals(Names.JAVA_LANG_CLASSLOADER_STATICDATA))
   {
    VariableDefinition v =
     (VariableDefinition) fieldDictionary.get(Names.SYSTEMCLASSLOADER);
    if (v != null)
     reflectFieldInner(v);
   }
 ...
 {
  boolean isJavaCore =
   name.startsWith(Names.JAVA_0) || name.startsWith(Names.GNU_0);
  Enumeration en = methodDictionary().keys();
  while (en.hasMoreElements())
  {
   MethodDefinition md = getMethodNoInheritance((String) en.nextElement());
 ...
 private void markFieldInitializers(boolean isStatic)
 {
  ...
  {
   VariableDefinition v =
    (VariableDefinition) fieldDictionary.get((String) en.nextElement());
 ...
 int markMethodInSubclasses(String sigString, String pkgName)
 {
  ...
  if (implementedBy != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (cd.used)
    {
     ...
     cnt += md != null ? md.markUsed(cd, false) :
             cd.markMethodInSubclasses(sigString, null);
    }
   }
  }
  ...
  while (en.hasMoreElements())
  {
   ClassDefinition cd = (ClassDefinition) en.nextElement();
   MethodDefinition md;
   cnt += cd.used && (md = cd.getMethodNoInheritance(sigString)) != null &&
           !md.isClassMethod() && (pkgName == null || pkgName.equals(
           cd.getPackageName())) ? md.markUsed(cd, false) :
           cd.markMethodInSubclasses(sigString, pkgName);
  }
  return cnt;
 }
  ...
  if (implementedBy != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (cd.used)
    {
     ...
     if (md != null)
      md.setVirtual();
      else cd.setVirtualInSubclasses(sigString);
    }
   }
  }
  Enumeration en = subclasses.elements();
 ...
 boolean subclassHasMethod(String sigString, String pkgName)
 {
  ...
  while (en.hasMoreElements())
  {
   ...
   MethodDefinition md;
   if ((cd.used && (md = cd.getMethodNoInheritance(sigString)) != null &&
       !md.isAbstract() && !md.isClassMethod() &&
       (pkgName == null || pkgName.equals(cd.getPackageName()))) ||
   ...
   ClassDefinition[] cdArr)
 {
  ...
  getTwoRealMethodsInSubclasses(methods,
   md.methodSignature().signatureString(), md.isProtectedOrPublic() ?
   null : md.definingClass().getPackageName(), cdArr);
  return methods.size() == 1 ? (MethodDefinition) methods.elementAt(0) : null;
 }
   ...
   String sigString, String pkgName, ClassDefinition[] cdArr)
 {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (cd.hasInstances && cd.used)
    {
     ...
     if (md != null && md.usedExact() && !md.isAbstract())
     {
      ...
      else cd.getTwoRealMethodsInSubclasses(methods, sigString, null, cdArr);
     if (methods.size() > 1)
      return;
    }
   }
  ...
  while (en.hasMoreElements())
  {
   ...
   if (cd.hasInstances && cd.used)
   {
    ...
    if (md != null && md.usedExact() && !md.isAbstract() &&
        !md.isClassMethod() && (pkgName == null ||
        pkgName.equals(cd.getPackageName())))
    {
     if (methods.identityLastIndexOf(md) < 0)
     {
      methods.addElement(md);
     ...
     else cd.getTwoRealMethodsInSubclasses(methods, sigString, pkgName,
           cdArr);
    if (methods.size() > 1)
     break;
   }
 ...
 boolean copyObjLeaksInSubclasses(String sigString, Term argsList)
 {
  ...
  if (implementedBy != null)
  {
   Enumeration en = implementedBy.elements();
   while (en.hasMoreElements())
    if ((cd = (ClassDefinition) en.nextElement()).hasInstances && cd.used)
    {
     MethodDefinition md = cd.getMethod(sigString);
     if ((md != null && !md.isAbstract() && md.copyObjLeaksTo(argsList)) ||
 ...
 boolean hasThisObjLeakInSubclasses(String sigString, boolean isReturned)
 {
  ...
  if (implementedBy != null)
  {
   ...
   while (en.hasMoreElements())
    ...
    {
     MethodDefinition md = cd.getMethod(sigString);
     if ((md != null && !md.isAbstract() && md.hasThisObjLeak(isReturned)) ||
 ...
 boolean isThisStackObjVltInSubclasses(String sigString)
 {
  ...
  if (implementedBy != null)
  {
   ...
   while (en.hasMoreElements())
    ...
    {
     MethodDefinition md = cd.getMethod(sigString);
     if ((md != null && !md.isAbstract() && md.isThisStackObjVolatile()) ||
  ...
  if (implementedBy != null)
  {
   ...
   while (en.hasMoreElements())
    if ((cd = (ClassDefinition) en.nextElement()).hasInstances && cd.used)
    {
     ...
     if (md != null && !md.isAbstract())
      md.setWritableArray();
      else cd.setWritableArrayRetInSubclasses(sigString);
    }
  }
  Enumeration en = subclasses.elements();
  while (en.hasMoreElements())
 ...
 MethodDefinition addMethod(MethodDefinition md)
 {
  ...
  if (md.isConstructor() && !md.hasParameters())
   basicConstructor = md;
  return (MethodDefinition) methodDictionary.put(
          md.methodSignature().signatureString(), md);
 }
 ...
 {
  define(forClass);
  return outerFields != null ?
          (VariableDefinition) outerFields.get(fieldName) : null;
 }
 ...
 VariableDefinition getField(String fieldName, ClassDefinition forClass)
 {
  ...
  do
  {
   ...
   if ((v != null || (v =
       (VariableDefinition) cd.fieldDictionary.get(fieldName)) != null) &&
       (cd == this || v.isProtectedOrPublic() ||
       (!v.isPrivate() && cd.getPackageName().equals(getPackageName()))))
    return v;
   Enumeration en = cd.specifiedInterfaces.elements();
   while (en.hasMoreElements())
    if ((v = ((ClassDefinition) en.nextElement()).getField(fieldName,
        forClass)) != null)
     return v;
 ...
 int countHiddenFields(String fieldName)
 {
  ...
  {
   VariableDefinition v =
    (VariableDefinition) cd.fieldDictionary().get(fieldName);
   if (v != null && !v.isClassVariable())
    hiddenCount++;
  }
  return hiddenCount;
 ...
 ClassDefinition getReflectHelperClass()
 {
  ...
  {
   helperClass.classbody.fatalError(helperClass.context,
    "VMReflector inner class name shall begin with '_': " + helperClass.name);
   return helperClass;
  }
  helperClass = this;
  return null;
 ...
 ClassDefinition reflectAllFields(boolean declaredOnly)
 {
  ...
  do
  {
   ...
   {
    VariableDefinition v =
     (VariableDefinition) cd.fieldDictionary.get((String) en.nextElement());
 ...
 boolean addFieldOutputName(String outputName)
 {
  ...
  Enumeration en = subclasses.elements();
  while (en.hasMoreElements())
   if (((ClassDefinition) en.nextElement()).fieldCNames.contains(outputName))
    return false;
 ...
 private void reflectMethodInner(MethodDefinition md)
 {
  ...
  if (md.isCopiedFromIface())
  {
   ...
   {
    MethodDefinition md2 =
     ((ClassDefinition) en.nextElement()).getMethod(md.methodSignature());
    if (md2 != null)
    {
  ...
  if (reflectedMethods == null)
  {
       ...
       !isInterface() && !basicConstructor.isPrivate())
   {
    ...
    reflectedMethods.put(basicConstructor.methodSignature().signatureString(),
     basicConstructor);
   }
  }
 ...
 {
  return sigString != null && !isFinal() && superClass != null &&
          (!isInterface() || (!sigString.equals("<init>(*)") &&
          (!sigString.equals("<init>()") || (!name.startsWith(Names.JAVA_0) &&
          !name.startsWith(Names.JAVAX_0)))));
 }
 ...
 void reflectMethods(String methodId, boolean declaredOnly, ObjVector parmSig,
   boolean isExactType)
 {
  if (methodId != null && parmSig != null)
  {
   String sigString =
    (new MethodSignature(methodId, parmSig)).signatureString();
   MethodDefinition md = getMethod(sigString);
   if (md != null && !md.isConstructor() &&
       (declaredOnly ? md.definingClass() == this : !md.isPrivate()))
   {
    ...
    do
    {
     ...
     while (en.hasMoreElements())
     {
      ...
      if (!md.isConstructor() &&
          ...
          (parmSig == null || md.methodSignature().isSignEqual(parmSig)))
      {
       ClassDefinition classDefn = md.definingClass();
 ...
 {
  return (MethodDefinition) methodDictionary().get(msig.signatureString());
 }
 ...
 {
  define(null);
  MethodDefinition md = (MethodDefinition) methodDictionary.get(sigString);
  if (md != null)
   return md;
  ClassDefinition aclass =
   isInterface() ? Main.dict.get(Names.JAVA_LANG_OBJECT) : superClass();
  if (aclass != null)
  {
   ...
   do
   {
    ...
    if (md != null && !md.isPrivate() && !md.isConstructor() &&
        (md.isProtectedOrPublic() || aclass.getPackageName().equals(pkgName)))
     return md;
   } while ((aclass = aclass.superClass()) != null);
  }
  return null;
 ...
 private MethodDefinition getMethodSamePkg(String sigString, String pkgName)
 {
  ...
  do
  {
   ...
   if (md != null && !md.isPrivate() && !md.isClassMethod() &&
    ...
    return aclass != this && md.isProtectedOrPublic() ?
            getMethod(sigString) : md;
   aclass = aclass.superClass();
  } while (aclass != null);
  return null;
 }
 ...
 MethodDefinition getSameMethod(MethodDefinition md)
 {
  return md.isProtectedOrPublic() ? getMethod(md.methodSignature()) :
          getMethodSamePkg(md.methodSignature().signatureString(),
          md.definingClass().getPackageName());
 }
   ...
   ClassDefinition forClass)
 {
  ...
  define(forClass);
  MethodDefinition bestMethod =
   getMethodNoInheritance(msig.signatureString());
  if (bestMethod == null || !bestMethod.isConstructor())
  {
   ...
   while (en.hasMoreElements())
   {
    MethodDefinition md2 = getMethodNoInheritance((String) en.nextElement());
    int value;
 ...
 int producePassOne()
 {
  ...
  if (used)
  {
   ...
   {
    VariableDefinition v =
     (VariableDefinition) fieldDictionary.get(en2.nextElement());
  ...
  if (classInitializers != null)
  {
   if (classInitializers.hasSomeCode())
   {
    ...
    if (superClass != null && !name.equals(Names.JAVA_LANG_CLASS))
     return;
    classbody.fatalError(context, "Object and Class cannot have <clinit>()");
   }
 ...
 private void produceAllFields()
 {
  ...
  if (size > 0)
  {
   ...
   if (name.equals(Names.JAVA_LANG_CLASS))
    VariableDefinition.orderFirstFields(fields, size, Names.fieldsOrderClass);
   ...
   for (int i = 0; i < size; i++)
   {
    ...
    v.produceOutput(outputContext);
    if (v.exprType().objectSize() >= Type.CLASSINTERFACE && v.used())
 ...
 {
  return instanceInitializers != null && instanceInitializers.hasSomeCode();
 }
 ...
 {
  if (instanceInitializers != null && !isInterface() && superClass != null &&
      hasInstances && used)
  {
   ...
   if (instanceInitializers.hasSomeCode())
   {
    ...
    else instanceInitializers = null;
   outputContext.cPrint("}\n\n");
  }
 }
 ...
 private void gatherChainedIfaces(ObjVector ifaces, ObjHashSet processed)
 {
  if (hasInstances && superClass != null)
  {
   if (processed.add(this))
   {
    ...
    Enumeration en = specifiedInterfaces.elements();
    while (en.hasMoreElements())
     ((ClassDefinition) en.nextElement()).gatherChainedIfaces(ifaces,
      processed);
    if (used)
    {
     Enumeration en2 = subclasses.elements();
     while (en2.hasMoreElements())
      ((ClassDefinition) en2.nextElement()).gatherChainedIfaces(ifaces,
       processed);
     if (implementedBy != null)
     {
      Enumeration en3 = implementedBy.elements();
      while (en3.hasMoreElements())
       ((ClassDefinition) en3.nextElement()).gatherChainedIfaces(ifaces,
        processed);
     }
    }
   }
    ...
    {
     int i;
     if (isInterface() && (i = ifaces.identityLastIndexOf(this)) >= 0)
     {
 ...
 private boolean hasUsedIfaceMethods()
 {
  if (used)
  {
   ...
   while (en.hasMoreElements())
   {
    MethodDefinition md = (MethodDefinition) en.nextElement();
    MethodDefinition md2;
    if (md.used() && ((md2 = objectClass.getMethodNoInheritance(
        md.methodSignature())) == null || !md2.isVirtualUsed()))
     return true;
   }
   Enumeration en2 = specifiedInterfaces.elements();
   while (en2.hasMoreElements())
  ...
  while (i-- > 0)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (cd.getMethodNoInheritance(sigString).used() &&
        ((md2 = objectClass.getMethodNoInheritance(sigString)) == null ||
        !md2.isVirtualUsed()) && cd.methodTableSigns.indexOf(sigString) < 0)
     fillJumpTableEntryFor(getJumpTableIndexFor(sigString, ifaces, i),
      sigString, ifaces, i);
   }
  }
 }
   ...
   int i)
 {
  ...
  do
  {
   processed.clear();
   changedArr[0] = false;
   index = ((ClassDefinition) ifaces.elementAt(i)).guessEmptyJumpTableIndex(
            index, sigString, changedArr, processed);
   if (!changedArr[0])
   {
    int j = i;
    while (j-- > 0)
    {
     ...
     MethodDefinition md = cd.getMethodNoInheritance(sigString);
     if (md != null && md.used())
      index = cd.guessEmptyJumpTableIndex(index, sigString, changedArr,
               processed);
    }
   }
  } while (changedArr[0]);
   ...
   boolean[] changedArr, ObjHashSet processed)
 {
  if (used && processed.add(this))
  {
   ...
   Enumeration en = subclasses.elements();
   while (en.hasMoreElements())
    index = ((ClassDefinition) en.nextElement()).guessEmptyJumpTableIndex(
             index, sigString, changedArr, processed);
   if (implementedBy != null)
   {
    Enumeration en2 = implementedBy.elements();
    while (en2.hasMoreElements())
     index = ((ClassDefinition) en2.nextElement()).guessEmptyJumpTableIndex(
              index, sigString, changedArr, processed);
   }
   if (!isInterface())
   {
    ...
    while (sc.superClass != null)
    {
     ...
     if (md != null && md.isVirtualUsed() && md.isProtectedOrPublic())
      index = sc.guessEmptyJumpTableIndex(index, sigString, changedArr,
               processed);
     sc = sc.superClass;
    }
   }
 ...
 private int getClassEmptyJumpTableIndex(int index)
 {
  if (used)
  {
   ...
   do
   {
    ...
    Enumeration en = subclasses.elements();
    while (en.hasMoreElements())
     index = ((ClassDefinition) en.nextElement()).getClassEmptyJumpTableIndex(
              index);
 ...
 {
  ((ClassDefinition) ifaces.elementAt(i)).fillJumpTableAt(index, sigString);
  while (i-- > 0)
  {
   ClassDefinition cd = (ClassDefinition) ifaces.elementAt(i);
   MethodDefinition md = cd.getMethodNoInheritance(sigString);
 ...
 private void fillJumpTableAt(int index, String sigString)
 {
  if (used && fillJumpTableOneEntry(index, sigString))
  {
   Enumeration en = subclasses.elements();
   while (en.hasMoreElements())
    ((ClassDefinition) en.nextElement()).fillJumpTableAt(index, sigString);
   if (implementedBy != null)
   {
    Enumeration en2 = implementedBy.elements();
    while (en2.hasMoreElements())
     ((ClassDefinition) en2.nextElement()).fillJumpTableAt(index, sigString);
   }
   if (!isInterface())
   {
    ClassDefinition sc = superClass;
 ...
 private void fillClassJumpTableAt(int index, String sigString)
 {
  if (used)
  {
   ...
   Enumeration en = subclasses.elements();
   while (en.hasMoreElements())
    ((ClassDefinition) en.nextElement()).fillClassJumpTableAt(index,
     sigString);
  }
 }
 ...
 private void prepareClassJumpTable()
 {
  ...
  while (en.hasMoreElements())
  {
   ...
   Term.assertCond(md != null);
   MethodDefinition md2;
   if (md.isVirtualUsed() && methodTableSigns.indexOf(sigString) < 0 &&
       ((md2 = objectClass.getMethodNoInheritance(sigString)) == null ||
       !md2.isVirtualUsed()) && (md.isProtectedOrPublic() ||
       methodTableSigns.indexOf(sigString = getPackageName() + ":" +
       sigString) < 0))
 ...
 private void produceJumpTable()
 {
  ...
  while (en.hasMoreElements())
  {
   ...
   if (pos >= 0)
   {
    ...
    else md = getMethod(sigString);
   if (md != null && md.isVirtualUsed())
   {
    while (dummyCnt-- > 0)
     writeDummyJumpEntry();
    md.produceJumpTableEntry(this, pos >= 0 &&
     methodTableSigns.indexOf(sigString) >= 0 && md.isProtectedOrPublic());
   }
   dummyCnt++;
  }
 }
 ...
 private boolean hasReflectedStaticFields(boolean isNonPrimOnly)
 {
  ...
  if (reflectedFieldNames != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (v.isClassVariable() && (!isNonPrimOnly ||
        v.exprType().objectSize() >= Type.CLASSINTERFACE))
     return true;
   }
  }
  ...
  while (en.hasMoreElements())
  {
   ...
   if (cd.vTableUsed)
   {
    if (cd.used)
    {
     ...
     else ifaceCnt += cd.buildIfacesArrContent(sb);
   }
  }
  return ifaceCnt;
 }
 ...
 private void writeClassVariables()
 {
  ...
  if (vTableUsed)
  {
   ...
   if (name.equals(Names.JAVA_LANG_STRING))
   {
    ...
    {
     if (Main.dict.allowConstClass >= 0 && !hasReflectedStaticFields(true))
     {
      ...
      else outputContext.cPrint("JCGO_NOSEP_GCDATA ");
    }
   ...
   outputContext.cPrint("),\010");
   outputContext.cPrint(Main.dict.classNameStringOutput(name, this, false));
   outputContext.cPrint(",\010");
   outputContext.cPrint(!isInterface() && superClass != null ?
    superClass.getClassRefStr(false) : LexTerm.NULL_STR);
   outputContext.cPrint(",\010");
   StringBuffer sb = new StringBuffer();
   int ifaceCnt = buildIfacesArrContent(sb);
   outputContext.cPrint(addImmutableArray(classClassDefn, sb.toString(),
    ifaceCnt));
   outputContext.cPrint(",\010");
   outputContext.cPrint("0x");
   outputContext.cPrint(Integer.toHexString((isAbstractOrInterface() ?
    modifiers | AccModifier.ABSTRACT : modifiers) |
    (classInitializers != null ? AccModifier.VOLATILE |
    AccModifier.TRANSIENT : 0) | (isEnum() ? AccModifier.ENUM : 0)));
   outputContext.cPrint("}");
   if (reflectedFieldNames != null)
   {
    VariableDefinition[] fields =
     new VariableDefinition[reflectedFieldNames.size()];
    int size = 0;
    Enumeration en2 = fieldDictionary.keys();
    while (en2.hasMoreElements())
    {
   ...
   outputContext.hPrint(";");
   outputContext.hPrint(isNonGc ? "JCGO_SEP_EXTERN" : "JCGO_SEP_GCEXTERN");
   if (allowConst)
  ...
  if (size > 0)
  {
   ...
   while (en.hasMoreElements())
   {
    VariableDefinition v =
     (VariableDefinition) fieldDictionary.get(en.nextElement());
    if (v.used() && v.isClassVariable() && !v.isLiteral() &&
        (reflectedFieldNames == null ||
        !reflectedFieldNames.contains(v.id())))
     fields[size++] = v;
   }
   VariableDefinition.sortBySize(fields, size);
 ...
 private boolean isEnum()
 {
  Term.assertCond(used);
  VariableDefinition v;
  return superClass != null && superClass.name.equals(Names.JAVA_LANG_ENUM) &&
 ...
 private void outputCoreClasses()
 {
  ...
  ArrayLiteral arrayIfacesArr =
   Main.dict.addArrayLiteral(new ArrayLiteral(classClassDefn, sb.toString(),
   cnt, false), this, false);
  ...
  outputContext.hPrint(";\n\n");
  outputContext.cPrint(Main.dict.allowConstClass > 0 ?
   " JCGO_THRD_ATTRNONGC" : Main.dict.allowConstClass == 0 ? " ATTRIBNONGC" :
   " ATTRIBGCDATA");
  outputContext.cPrint("={");
  boolean next = false;
  boolean isArray = false;
  boolean createNames = Main.dict.isClassNameUsed(Main.dict.classTable[
                         Type.CLASSINTERFACE + Type.BOOLEAN]);
   ...
   do
   {
    ...
    if (type == Type.NULLREF)
    {
     ...
     else
     {
      ...
      outputContext.cPrint(cname);
      outputContext.cPrint(")&");
      outputContext.cPrint(Main.dict.classTable[isArray ?
       Type.CLASSINTERFACE + type : type].vTableCName());
      outputContext.cPrint("),\010");
      outputContext.cPrint(Main.dict.classNameStringOutput(isArray ?
       Type.sig[Type.CLASSINTERFACE + type] : Type.name[type], this,
       createNames));
      outputContext.cPrint(",\010");
      outputContext.cPrint(isArray ?
       Main.dict.classTable[type].getClassRefStr(false) : LexTerm.NULL_STR);
      outputContext.cPrint(",\010");
      outputContext.cPrint(
       (isArray ? arrayIfacesArr : noIfacesArr).stringOutput());
      outputContext.cPrint(",\010");
      outputContext.cPrint("0x");
 ...
 private void outputReflectionInfo()
 {
  ...
  if (reflectedMethods != null && (count = reflectedMethods.size()) > 0)
  {
   ...
   outputContext.cPrint("JCGO_NOSEP_DATA CONST struct jcgo_methodentry_s ");
   outputContext.cPrint(cname);
   ...
   {
    MethodDefinition md =
     (MethodDefinition) reflectedMethods.get((String) en.nextElement());
    if (md != null)
    {
  ...
  if (reflectedFieldNames != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    VariableDefinition v =
     (VariableDefinition) fieldDictionary.get(fieldName);
    if (v.used() && reflectedFieldNames.contains(fieldName))
    {
     if (infoSBuf != null)
      ...
      else
      {
       ...
       infoSBuf.append("Reflected fields for ").append(name).append(": ");
      }
     ...
     namesSBuf.append(')');
     namesSBuf.append(
      Main.dict.addStringLiteral(fieldName, this).stringOutput());
     namesSBuf.append(", ");
     ...
     if (dims == 1 && signClass.objectSize() < Type.CLASSINTERFACE)
     {
      typesSBuf.append(signClass.getClassRefStr(true));
      dims--;
     }
      else typesSBuf.append(signClass.getClassRefStr(false));
     ...
     if (superClass != null && superClass.name.equals(Names.JAVA_LANG_ENUM) &&
         isEnumValueField(v))
      mods |= AccModifier.ENUM;
     modsSBuf.append(")0x");
     modsSBuf.append(Integer.toHexString(mods));
     modsSBuf.append(", ");
     if ((isInterface() ? AccModifier.PUBLIC | AccModifier.STATIC |
         AccModifier.FINAL : 0) != mods)
      hasMods = true;
     count++;
    }
   }
  ...
  outputContext.cPrint(namesSBuf != null ? addImmutableArray(
   Main.dict.get(Names.JAVA_LANG_STRING), namesSBuf.toString(), count) :
   LexTerm.NULL_STR);
  outputContext.cPrint(",\010");
  ...
  outputContext.cPrint(typesSBuf != null ? addImmutableArray(
   Main.dict.get(Names.JAVA_LANG_CLASS), typesSBuf.toString(), count) :
   LexTerm.NULL_STR);
  outputContext.cPrint(",\010");
    ...
    if (md != null)
    {
     ...
     if (isConstr)
      ...
      else
      {
       ...
       namesSBuf.append(')');
       namesSBuf.append(
        Main.dict.addStringLiteral(md.id(), this).stringOutput());
       hasNames = true;
      }
     namesSBuf.append(", ");
         ...
         md.hasParameters())
     {
      ...
      do
      {
       ...
       sb.append(')');
       ClassDefinition signClass = exprType.signatureClass();
       ...
       if (dims == 1 && signClass.objectSize() < Type.CLASSINTERFACE)
       {
        ...
        else sb.append(signClass.getClassRefStr(false));
       sb.append(",\010");
       sb2.append('(');
      ...
      typesSBuf.append(Type.cName[Type.CLASSINTERFACE]);
      typesSBuf.append(')');
      typesSBuf.append(addImmutableArray(Main.dict.get(Names.JAVA_LANG_CLASS),
       sb.toString(), cnt));
      if (hasDims)
      {
       dimsPresent = true;
       ...
       dimsSBuf.append(')');
       dimsSBuf.append(addImmutableArray(Main.dict.classTable[Type.BYTE],
        sb2.toString(), cnt));
      }
       else dimsSBuf.append(LexTerm.NULL_STR);
     }
      else
      {
     ...
     if (en2.hasMoreElements())
     {
      ...
      do
      {
       ...
       sb.append(Type.cName[Type.CLASSINTERFACE]);
       sb.append(')');
       sb.append(((ClassDefinition) en2.nextElement()).getClassRefStr(false));
       sb.append(",\010");
       cnt++;
      } while (en2.hasMoreElements());
      throwsPresent = true;
      ...
      throwsSBuf.append(addImmutableArray(
       Main.dict.get(Names.JAVA_LANG_CLASS), sb.toString(), cnt));
     }
      else throwsSBuf.append(LexTerm.NULL_STR);
     ...
     modsSBuf.append(Integer.toHexString(mods));
     modsSBuf.append(", ");
     if ((isInterface() ? AccModifier.PUBLIC | AccModifier.ABSTRACT :
         AccModifier.PUBLIC) != mods)
      hasMods = true;
     count++;
    }
   }
  ...
  outputContext.cPrint(",\010");
  outputContext.cPrint(typesSBuf != null ? addImmutableArray(
   Main.dict.get(Names.JAVA_LANG_CLASS).asExprType(1), typesSBuf.toString(),
   count) : LexTerm.NULL_STR);
  outputContext.cPrint(",\010");
  outputContext.cPrint(dimsPresent ? addImmutableArray(
   Main.dict.classTable[Type.BYTE].asExprType(1), dimsSBuf.toString(),
   count) : LexTerm.NULL_STR);
  outputContext.cPrint(",\010");
  outputContext.cPrint(throwsPresent ? addImmutableArray(Main.dict.get(
   Names.JAVA_LANG_CLASS).asExprType(1), throwsSBuf.toString(), count) :
   LexTerm.NULL_STR);
  outputContext.cPrint(",\010");
  outputContext.cPrint(modsSBuf != null ? addImmutableArray(
   Main.dict.classTable[Type.SHORT], modsSBuf.toString(), count) :
  ...
  if (reflectedMethods != null && reflectedMethods.size() > 0)
  {
   ...
   else outputContext.cPrint("NULL");
  outputContext.cPrint("};\n\n");
 }
 ...
 private void startMethodDefinitions()
 {
  ...
  outputContext.hPrint(
   "JCGO_TYPEID_T jcgo_typeid;JCGO_OBJSIZE_T jcgo_objsize;");
  ...
  if (vTableUsed)
  {
       ...
       hasPrimInstanceFields())
   {
    ...
    else outputContext.cPrint("JCGO_GCJDESCR_ZEROINIT\010");
   ...
   outputContext.cPrint(classInitializers != null ? clinitCName() : "0");
   outputContext.cPrint(")\010");
 ...
 void buildMain(boolean skipClinitTrace)
 {
  ...
  ClassDefinition throwableClassDefn =
   Main.dict.get(Names.JAVA_LANG_THROWABLE);
 ...
 private String addImmutableArray(ExpressionType exprType, String data,
  int count)
 {
  return Main.dict.addArrayLiteral(new ArrayLiteral(exprType, data, count,
          false), this, false).stringOutput();
 }
 ...
 private void outputPool()
 {
  ...
  if (arrpool != null)
  {
   ...
   while (en.hasMoreElements())
    ((ArrayLiteral) en.nextElement()).processOutput(outputContext, this);
   arrpool = null;
  }
  if (strpool != null)
  {
   ...
   boolean isExactType)
 {
  ...
  {
   MethodDefinition md = getMethodNoInheritance(sigString);
   if (md != null && md.isConstructor() && (declaredOnly || !md.isPrivate()))
   ...
   ObjVector parmSig)
 {
  if (methodId != null && parmSig != null)
  {
   MethodDefinition md = getMethod(new MethodSignature(methodId, parmSig));
   if (md != null)
   {
    if (!md.isConstructor() &&
        (declaredOnly ? md.definingClass() == this : !md.isPrivate()))
   ...
   else
   {
    ...
    } while (!declaredOnly && (cd = cd.superClass()) != null &&
             (!isInterface() || !cd.name.equals(Names.JAVA_LANG_OBJECT)));
   }
 }
   ...
   boolean isWeak, ObjVector parmTraceSig)
 {
  ...
  if (implementedBy != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (cd.used)
    {
     ...
     if (md != null && !md.isAbstract())
     {
      curType = md.methodTraceClassInit(isWeak, cd, parmTraceSig);
      if (curType == null)
       curType = md.exprType();
     }
      else curType = cd.traceClInitInSubclasses(sigString, null, isWeak,
                      parmTraceSig);
  ...
  while (en.hasMoreElements())
  {
       ...
       pkgName.equals(cd.getPackageName())))
   {
    ...
    else curType = cd.traceClInitInSubclasses(sigString, pkgName, isWeak,
                    parmTraceSig);
   curTraceType = maxCommonExprOf(curTraceType, curType, null);
  }
  return curTraceType;
 ...
 void constructorTraceClassInitInSubclasses()
 {
  Enumeration en = subclasses.elements();
  while (en.hasMoreElements())
   ((ClassDefinition) en.nextElement()).constructorTraceClassInit(true);
 }
 ...
 {
  return !hasInstances || !used || (finished &&
          !Main.dict.instancesCreatedOnTrace.contains(this));
 }
 ...
 void instanceCreatedOnTrace()
 {
  if (hasInstances && used && finished)
  {
   ...
   {
    ObjQueue traceInfos;
    if (Main.dict.notYetCallableTraceInfos != null && (traceInfos =
        (ObjQueue) Main.dict.notYetCallableTraceInfos.remove(cd)) != null)
     Main.dict.pendingTraceInfos.addAllMovedFrom(traceInfos);
    Enumeration en = cd.specifiedInterfaces.elements();
    while (en.hasMoreElements())
     ((ClassDefinition) en.nextElement()).instanceCreatedOnTrace();
    if (isInterface() || (cd = cd.superClass()) == null || !cd.finished)
     break;
   }
  }
 }
  ...
  while ((dynCount = Main.dict.dynClassesToTrace.size()) > oldDynCount)
  {
   for (int i = 0; Main.dict.dynCallerTraceInfos.size() > i; i++)
    recursiveTracingInner(
     (MethodTraceInfo) Main.dict.dynCallerTraceInfos.keyAt(i), processed);
  ...
  if (traceInfo.addToProcessed(processed))
  {
   if (traceInfo.isCallableNow())
    ...
    else
    {
     ...
     ObjQueue traceInfos =
      (ObjQueue) Main.dict.notYetCallableTraceInfos.get(cd);
     if (traceInfos == null)
      Main.dict.notYetCallableTraceInfos.put(cd, traceInfos = new ObjQueue());
   ...
   ObjHashtable processed)
 {
  ...
  while (!Main.dict.pendingTraceInfos.isEmpty())
   recursiveTracingInner(
    (MethodTraceInfo) Main.dict.pendingTraceInfos.removeFirst(), processed);
  ...
  {
   ClassDefinition[] dynClasses =
    new ClassDefinition[Main.dict.dynClassesToTrace.size()];
   ...
   for (int i = 0; i < dynClasses.length; i++)
    if (dynClasses[i].isBasicConstructorNormal())
     recursiveTracing(MethodTraceInfo.create(dynClasses[i].basicConstructor,
      null, null), processed);
  ...
  do
  {
   ClassDefinition cd;
   for (int i = 0;
        (cd = traceInfo.getClassDependElementAt(isWeak, i)) != null; i++)
 ...
 {
  ObjHashSet curInstancesCreatedOnTrace =
   (ObjHashSet) classToInstancesMap.get(this);
  if (curInstancesCreatedOnTrace == null)
  {
   Term.assertCond(clinitTraceInfo != null && classInitializers != null);
   ...
   do
   {
    ...
    for (int j = 0; classInitDepend.size() > j; j++)
     ((ClassDefinition) classInitDepend.keyAt(j)).collectClassInitDepend(
      classToInstancesMap);
 ...
 private void collectClassInitDepend(ObjVector traceInfos, int weakness,
   ObjHashtable processed)
 {
  MethodTraceInfo traceInfo =
   (MethodTraceInfo) traceInfos.elementAt(traceInfos.size() - 1);
  if (traceInfo.addToProcessed(processed) && traceInfo.isCallableNow())
  {
   ...
   if (traceInfo.usesDynClasses())
   {
    ...
    while (en.hasMoreElements())
    {
     ...
     {
      traceInfos.addElement(MethodTraceInfo.create(cd.basicConstructor,
       null, null));
      collectClassInitDepend(traceInfos, weakness, processed);
      traceInfos.removeElementAt(traceInfos.size() - 1);
     }
 ...
 {
  Term.assertCond(classInitializers != null && cd.classInitializers != null);
  ...
  if (constVal == null || constVal.getIntValue() > weakness)
  {
   ...
   if (Main.dict.verboseTracing)
   {
    Main.dict.message("Clinit dependency: " + name + " -> " + cd.name);
    int i = traceInfos.size();
    while (i-- > 0)
     Main.dict.message(" Called from: " +
      ((MethodTraceInfo) traceInfos.elementAt(i)).getTraceSig());
   }
  }
 }
 ...
 void printClassInitGroup(OutputContext oc)
 {
  if (used && classInitializers != null)
  {
   ...
   Main.dict.message("Grouping initializers for: " + name + " (" +
    Integer.toString(Main.dict.tracedInfosCount) + " methods traced)");
   ...
   if (!printClassInitSimpleWithSub(oc, clinitBeginSet))
   {
    ...
    do
    {
     if (((ClassDefinition) cdRoots.elementAt(i)).printClassInitSimpleWithSub(
         oc, clinitBeginSet))
    ...
    if (i > 0)
    {
     ...
     do
     {
      if (((ClassDefinition) cdRoots.elementAt(i)).printClassInitSimple(oc,
          processed, null))
       cdRoots.removeElementAt(i);
       else i++;
     } while (cdRoots.size() > i);
     ...
     while (en2.hasMoreElements())
      ((ClassDefinition) en2.nextElement()).printClassInitCyclic(oc,
       new ObjHashSet(), null);
    }
   }
   oc.cPrint("}");
   ...
   ObjHashSet processed, ObjHashSet clinitBeginSet)
 {
  if (classInitializers != null && processed.add(this))
  {
    ...
    do
    {
     ...
     while (en.hasMoreElements())
          ...
          cd.classInitializers != null)
      {
       ...
       if (sc == this)
        return cd.printClassInitForSubclasses(oc, processed, clinitBeginSet);
   ...
   while (en.hasMoreElements())
    if (((ClassDefinition) en.nextElement()).printClassInitForSubclasses(oc,
 ...
 private boolean printClassInitSimple(OutputContext oc, ObjHashSet processed,
   ObjHashSet clinitBeginSet)
 {
  Term.assertCond(used);
  if (classInitializers != null)
  {
   ...
   while (en.hasMoreElements())
   {
    ...
    if (!cd.printClassInitSimple(oc, processed, clinitBeginSet) &&
        (clinitBeginSet == null || !clinitBeginSet.contains(cd) ||
        ((ConstValue) classInitDepend.get(cd)).getIntValue() == 0))
     full = false;
   }
   if (!full)
    return false;
   ...
   ObjHashSet commented, ClassDefinition prevCommented)
 {
  ...
  if (headerWritten)
  {
   ...
   while (en.hasMoreElements())
    prevCommented =
     ((ClassDefinition) en.nextElement()).printClassInitCyclic(oc, commented,
     prevCommented);
   headerWritten = true;
   ...
   ObjHashSet clinitBeginSet)
 {
  ...
  if (commented != null)
  {
   if (commented.add(this))
    System.out.println("Warning: cannot detect class init order: " + name);
   oc.cPrint("/" + "*");
  }
   else if (clinitBeginSet != null && !isInterface() && superClass != null &&
            superClass.classInitializers != null &&
   ...
   int nonWeakMax)
 {
  ...
  do
  {
   ((ClassDefinition) cdRoots.elementAt(i)).breakClassInitWeakCycles(cdRoots,
    nonWeakMax, new ObjVector(), new ObjHashSet());
  ...
  if (classInitializers != null)
  {
   ...
   if (i >= 0)
   {
    ...
    do
    {
     ...
     ConstValue constVal = (ConstValue) cd.classInitDepend.get(cdNext);
     if (constVal == null)
      return;
    ...
    if (k >= 0)
    {
     if (cdStack.size() - 1 > k)
     {
      ...
      if (i > 0)
      {
       ...
       ConstValue weaknessVal = (ConstValue) cd.classInitDepend.get(this);
       if (weaknessVal == null)
        return;
       ConstValue constVal = (ConstValue) cd.classInitDepend.get(cdNext);
       if (constVal == null ||
           constVal.getIntValue() > weaknessVal.getIntValue())
        cd.classInitDepend.put(cdNext, weaknessVal);
      }
     }
     if (cdRoots.identityLastIndexOf(cdNext) < 0)
      cdRoots.addElement(cdNext);
     ((ClassDefinition) cdStack.elementAt(k)).classInitDepend.remove(cdNext);
    }
   }
    else
    {
     if (processed.add(this))
     {
      ...
      for (int j = 0; classInitDepend.size() > j; j++)
       ((ClassDefinition) classInitDepend.keyAt(j)).breakClassInitWeakCycles(
        cdRoots, nonWeakMax, cdStack, processed);
