...
final class ArrElementInit extends LexNode
{
 ...
 boolean storeClassLiteralsGuess(ObjVector parmSig, boolean isActual)
 {
  ...
  if (isActual)
  {
   ...
   if (s0 == Type.CLASSINTERFACE)
   {
    ...
    exprType0 = cd.name().equals(Names.JAVAX_SWING_PLAF_COLORUIRESOURCE) ?
                 cd.superClass() : cd.mapToPrimType();
   }
  }
   else
   {
 ...
 void processOutput(OutputContext oc)
 {
  ...
  if (oc.arrInitCount >= 0)
  {
   ...
   if (s0 >= Type.CLASSINTERFACE ? terms[0].exprType().objectSize() !=
       Type.NULLREF : (constVal0 = terms[0].evaluateConstValue()) == null ||
       constVal0.isNonZero())
   {
    ...
    oc.cPrint(Type.cName[s0 < Type.CLASSINTERFACE ? s0 :
     Type.CLASSINTERFACE]);
    oc.cPrint(", (");
    oc.cPrint(Type.cName[s0 < Type.BOOLEAN || s0 > Type.DOUBLE ?
     Type.OBJECTARRAY : Type.CLASSINTERFACE + s0]);
    oc.cPrint(")");
    ...
    oc.cPrint(")= ");
    if (s0 >= Type.CLASSINTERFACE || s0 == Type.BOOLEAN ||
