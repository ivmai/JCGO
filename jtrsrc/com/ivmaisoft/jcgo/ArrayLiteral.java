...
final class ArrayLiteral
{
 ...
 String stringOutput()
 {
  return "((" + exprType.signatureClass().asExprType(
          exprType.signatureDimensions() + 1).castName() +
          ")JCGO_OBJREF_OF(" + cname() + "))";
 }
 ...
 void processOutput(OutputContext oc, ClassDefinition ourClass)
 {
  ...
  String arrtype = Main.dict.addArrayTypeDefn(s0, count, ourClass, suffix);
  ...
  if (isWritable)
   oc.cPrint(count == 0 ? " JCGO_THRD_ATTRNONGC" :
    s0 < Type.CLASSINTERFACE ? " ATTRIBNONGC" : " ATTRIBGCDATA");
  oc.cPrint("={(jvtable)&");
  oc.cPrint(ClassDefinition.arrayVTableCName(s0 < Type.CLASSINTERFACE ? s0 :
   Type.VOID, exprType.signatureDimensions()));
  oc.cPrint(",\010");
  ...
  oc.cPrint(count > 0 ? data.substring(0, data.lastIndexOf(',')) :
   s0 < Type.CLASSINTERFACE ? "(" + Type.cName[s0] + ")0" : LexTerm.NULL_STR);
  oc.cPrint("}}");
  oc.cAndHPrint(";\n\n");
 }
