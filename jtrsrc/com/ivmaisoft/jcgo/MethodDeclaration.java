 ...
 **
 * Formats:
 * VoidType ID LPAREN [FormalParamList] RPAREN [Throws] MethodBody
 * PrimitiveType/ClassOrIfaceType [Dims] ID LPAREN [FormalParamList] RPAREN [Dims] [Throws] MethodBody
 */
...
final class MethodDeclaration extends LexNode
{
 ...
 void processPass1(Context c)
 {
  ...
  if (((c.modifiers & AccModifier.ABSTRACT) != 0 ||
      c.currentClass.isInterface()) && (c.modifiers & (AccModifier.PRIVATE |
      AccModifier.STATIC | AccModifier.SYNCHRONIZED | AccModifier.NATIVE |
      AccModifier.FINAL | AccModifier.STRICT)) != 0)
   fatalError(c, "Illegal modifier found for an abstract method: " + id);
  ...
  MethodDefinition md2 =
   c.currentClass.addMethod(md = new MethodDefinition(c, id, c.modifiers,
   c.typeClassDefinition.asExprType(c.typeDims), terms[3], terms[5],
   terms[6]));
