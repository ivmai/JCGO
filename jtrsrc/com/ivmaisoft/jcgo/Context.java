...
final class Context
{
 ...
 void addImport(String name)
 {
  if (name.endsWith(".*"))
   ...
   else importsSingle.addLast(name);
 }
 ...
 void initBranch()
 {
  ...
  currentBranch.accessedClasses.addElement(
 ...
 boolean addAccessedClass(ClassDefinition cd)
 {
  ...
  do
  {
   ...
   if (i >= 0)
   {
    ...
    else accessedClasses.addElement(cd2);
 ...
 void setActualType(VariableDefinition v, ExpressionType actualType)
 {
  ...
  int s0;
  if ((v.isLocalOrParam() || v.isFinalVariable()) &&
      ((s0 = v.exprType().objectSize()) == Type.CLASSINTERFACE ||
      s0 == Type.OBJECTARRAY))
  {
   ...
   boolean checkFields)
 {
  ...
  do
  {
   if (qualified != null && Main.dict.exists(qualified))
   {
    ...
    if (qualified != null)
     return qualified.length() > 0 ? Main.dict.get(qualified) : null;
   }
   i = name.indexOf('.', i + 1);
   if (i < 0)
    break;
  ...
  return allowNotFound || Main.dict.exists(name) ? Main.dict.get(name) : null;
 }
 ...
 private String getQualifiedName(String id)
 {
  ...
  if (!Main.dict.exists(qualified))
  {
   ...
   while (en2.hasMoreElements())
   {
    ...
    if (Main.dict.exists(qualified) && Main.dict.get(qualified).isPublic())
     return qualified;
    do
    {
     qualified = qualified.substring(0, i) + "$" + qualified.substring(i + 1);
