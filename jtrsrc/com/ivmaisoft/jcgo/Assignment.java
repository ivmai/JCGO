 ...
 **
 * Formats:
 * LeftHandSide EQUALS AssignmentExpression
 * LeftHandSide FLSHIFT_EQUALS AssignmentExpression
 * LeftHandSide SHRIGHT_EQUALS AssignmentExpression
 * LeftHandSide SHLEFT_EQUALS AssignmentExpression
 * LeftHandSide XOR_EQUALS AssignmentExpression
 * LeftHandSide BITOR_EQUALS AssignmentExpression
 * LeftHandSide BITAND_EQUALS AssignmentExpression
 * LeftHandSide MOD_EQUALS AssignmentExpression
 * LeftHandSide DIVIDE_EQUALS AssignmentExpression
 * LeftHandSide TIMES_EQUALS AssignmentExpression
 * LeftHandSide MINUS_EQUALS AssignmentExpression
 * LeftHandSide PLUS_EQUALS AssignmentExpression
 */
...
final class Assignment extends LexNode
{
  ...
  if (exprType0 == null)
  {
       ...
       exprType0.receiverClass().isStringOrNull())
   {
    ...
    if (!exprType2.receiverClass().isStringOrNull())
     exprType2 = (terms[2] = BinaryOp.wrapInValueOf(c, terms[2])).exprType();
    actualType2 = exprType2;
    Main.dict.get(Names.JAVA_LANG_VMSYSTEM).markMethod(Names.SIGN_CONCAT0X);
    if (v != null)
    {
     c.setVarNotNull(v);
    ...
    else
    {
     ...
     if (s0 == Type.BOOLEAN)
     {
      ...
      if (sym != LexTerm.EQUALS && sym != LexTerm.XOR_EQUALS &&
          sym != LexTerm.BITOR_EQUALS && sym != LexTerm.BITAND_EQUALS)
       fatalError(c, "Illegal assignment operator");
     }
      else if (s0 < Type.CLASSINTERFACE)
      {
       if ((s0 >= Type.FLOAT || s2 >= Type.FLOAT) && sym != LexTerm.EQUALS &&
           sym != LexTerm.PLUS_EQUALS && sym != LexTerm.MINUS_EQUALS &&
           sym != LexTerm.TIMES_EQUALS && sym != LexTerm.DIVIDE_EQUALS &&
           sym != LexTerm.MOD_EQUALS)
        fatalError(c, "Illegal assignment operator");
       if (s2 <= Type.BOOLEAN || s2 >= Type.CLASSINTERFACE ||
       ...
       else
       {
        ...
        if (v != null)
        {
         if (terms[2].isNotNull())
          c.setVarNotNull(v);
          else c.clearVarNotNull(v);
         c.setActualType(v, actualType2);
         v.setClassNewInstanceCall(terms[2].getClassNewInstanceCall());
         v.setConstructorInstanceSign(terms[2].getConstructorInstanceSign());
         v.setClassLiteralValGuess(terms[2].classLiteralValGuess());
 ...
 void allocRcvr(int[] curRcvrs)
 {
  ...
  if (t1 != null)
  {
   ...
   if (t0.isSafeExpr() ? (t2.isSafeExpr() ? !t1.isSafeWithThrow() &&
       ...
       t1.isAnyLocalVarChanged(t0) || t1.isAnyLocalVarChanged(t2)) &&
       (!t0.isImmutable() || !t2.isImmutable()) :
       !t0.isImmutable() || !t1.isImmutable() ||
       (sym != LexTerm.EQUALS && !t2.isSafeWithThrow())) :
       !t2.isSafeExpr() || !t1.isSafeExpr() || (!t0.isSafeWithThrow() &&
       (t1.isFieldAccessed(null) || t2.isFieldAccessed(null) ||
       t0.isAnyLocalVarChanged(t1) || t0.isAnyLocalVarChanged(t2)) &&
       (!t1.isImmutable() || !t2.isImmutable())))
   {
    rmask = 1;
    if (!t2.isSafeExpr() || (!t1.isSafeWithThrow() && !t2.isImmutable()))
    {
     rmask = 2;
     if (sym == LexTerm.EQUALS &&
         exprType2.objectSize() >= Type.CLASSINTERFACE &&
         exprType0.signatureClass().hasInstantatedSubclasses(true))
     {
      if (t2.isSafeWithThrow() && t0.isSafeExpr() && t1.isSafeExpr())
       rmask = 0;
       else if (!t0.isImmutable() && (!t2.isSafeWithThrow() ||
                !t1.isSafeWithThrow() || !t0.isSafeExpr()))
       {
        rmask = 1;
        if (!t1.isImmutable() && (!t1.isSafeExpr() || !t2.isSafeWithThrow()))
         rmask = 3;
       }
     }
      else if (t2.isSafeExpr())
      {
       ...
       {
        if (!t1.isSafeWithThrow() || (!t0.isSafeExpr() &&
            (!t1.isSafeExpr() || !t0.isSafeWithThrow())))
         rmask = 3;
       }
        else
        {
         ...
         if (sym == LexTerm.EQUALS)
         {
          if (t2.isSafeWithThrow() && t1.isSafeWithThrow() &&
              (t0.isSafeExpr() || (t1.isSafeExpr() && t0.isSafeWithThrow())))
           rmask = 4;
          ...
          else if (!t0.isImmutable() && !t1.isImmutable() &&
                   (!t1.isSafeWithThrow() || (!t0.isSafeExpr() &&
                   (!t1.isSafeExpr() || !t0.isSafeWithThrow()))))
           rmask = 7;
        }
   ...
   int s0 = exprType0.objectSize();
   int s2;
   if ((rmask & 2) != 0 || (rmask == 0 && (isString ||
       sym == LexTerm.DIVIDE_EQUALS || sym == LexTerm.MOD_EQUALS ||
       sym == LexTerm.FLSHIFT_EQUALS || sym == LexTerm.SHRIGHT_EQUALS ||
       sym == LexTerm.SHLEFT_EQUALS || (sym != LexTerm.EQUALS &&
       (s2 = exprType2.objectSize()) >= Type.FLOAT && s0 < s2))))
    rcvr2 = ++curRcvrs[sym != LexTerm.EQUALS ? Type.NULLREF : Type.INT];
   if (rmask > 3)
   ...
   else
   {
    ...
    if (sym != LexTerm.EQUALS && (leftVar = t0.getVariable(true)) != null &&
        leftVar.used() && leftVar.isThreadVolatile())
    {
     rcvr1 = ++curRcvrs2[Type.NULLREF];
     rcvr2 = ++curRcvrs2[isString ? Type.NULLREF : exprType0.objectSize()];
    }
     else if (t0.isSafeWithThrow() ? !t2.isSafeExpr() && (!t0.isSafeExpr() ||
              (sym != LexTerm.EQUALS && !t2.isSafeWithThrow() &&
              ...
              !t2.isImmutable())
     {
      ...
      if (sym != LexTerm.EQUALS && !t2.isSafeWithThrow())
       rcvr2 = ++curRcvrs2[(s0 = exprType0.objectSize()) < Type.VOID ? s0 :
               ...
               sym == LexTerm.MOD_EQUALS || sym == LexTerm.FLSHIFT_EQUALS ||
               sym == LexTerm.SHRIGHT_EQUALS ||
               sym == LexTerm.SHLEFT_EQUALS || (sym != LexTerm.EQUALS &&
               (s2 = exprType2.objectSize()) >= Type.FLOAT &&
               exprType0.objectSize() < s2)) &&
               (!t0.isSafeExpr() || (!t2.isSafeWithThrow() &&
               (t0.isFieldAccessed(null) || t2.isAnyLocalVarChanged(t0)))))
       rcvr1 = ++curRcvrs2[Type.NULLREF];
   }
 ...
 VariableDefinition getVariable(boolean allowInstance)
 {
  return terms[1].getSym() == LexTerm.EQUALS ?
          terms[0].getVariable(allowInstance) : null;
 }
 ...
 boolean isSwitchMapAssign(boolean isMethodCall)
 {
  ...
  return v != null && v.isClassVariable() && v.isFinalVariable() &&
          ...
          terms[2].evaluateConstValue() != null && t1.isSwitchMapAssign(true);
 }
 ...
 String strLiteralValueGuess()
 {
  ...
  return terms[1].getSym() == LexTerm.EQUALS ?
          terms[2].strLiteralValueGuess() : null;
 }

 ExpressionType classLiteralValGuess()
 {
  return terms[1].getSym() == LexTerm.EQUALS ?
          terms[2].classLiteralValGuess() : null;
 }
 ...
 {
  return terms[1].getSym() == LexTerm.EQUALS ?
          terms[2].getClassNewInstanceCall() : null;
 }

 MethodSignature getConstructorInstanceSign()
 {
  return terms[1].getSym() == LexTerm.EQUALS ?
          terms[2].getConstructorInstanceSign() : null;
 }
 ...
 void discoverObjLeaks()
 {
  ...
  if (isString || (terms[1].getSym() == LexTerm.EQUALS &&
      exprType2.objectSize() >= Type.CLASSINTERFACE))
 ...
 void processOutput(OutputContext oc)
 {
  ...
  if ((rcvr1 | rcvr2 | rcvr3) > 0)
  {
   ...
   {
    rcvrStr1 =
     rcvr1 > 0 ? OutputContext.getRcvrName(rcvr1, Type.CLASSINTERFACE) :
     rcvr2 > 0 ? OutputContext.getRcvrName(rcvr2, sym != LexTerm.EQUALS ?
     Type.CLASSINTERFACE : Type.INT) : OutputContext.getRcvrName(rcvr3, s0);
    if ((rcvr1 > 0 && (rcvr2 | rcvr3) > 0) || (rcvr2 > 0 && rcvr3 > 0))
    {
     rcvrStr2 = rcvr1 > 0 && rcvr2 > 0 ? OutputContext.getRcvrName(rcvr2,
                 sym != LexTerm.EQUALS ? Type.CLASSINTERFACE : Type.INT) :
                 OutputContext.getRcvrName(rcvr3, s0);
    ...
    if ((rcvr2 | rcvr3) > 0)
    {
     if (sym != LexTerm.EQUALS)
     {
      ...
      oc.cPrint("= (void*)&");
      boolean forceCheck =
       ArrayAccess.outputArrAccess(oc, forceVmExc, s0, t0);
      if (rcvr1 > 0)
      {
       oc.cPrint("(");
    ...
    {
     rcvrStr1 = OutputContext.getRcvrName(rcvr1, Type.CLASSINTERFACE);
     if (rcvr2 > 0)
     {
  ...
  if (oldAssignmentVar != oc.assignmentVar && oldAssignmentRightType != null)
   oc.assignmentVar = null;
  ExpressionType assignRightType =
   oldAssignmentRightType == exprType0 || oldAssignmentRightType == null ?
   exprType0 : Main.dict.classTable[Type.VOID];
  if (sym == LexTerm.EQUALS)
  {
   if (t1 == null)
   {
    ...
    if (castToVoid)
    {
     ...
     else
     {
      ...
      if (s0 == Type.BOOLEAN || (exprType0 != exprType2 &&
          (!isVolatile || s0 < Type.CLASSINTERFACE)))
      {
       oc.cPrint("(");
             ...
             exprType0.signatureClass().hasInstantatedSubclasses(true))
    {
     ...
     boolean isNotNull0;
     if ((forceVmExc & (ClassDefinition.ARR_STORE_EXC |
         ClassDefinition.INDEX_OUT_EXC)) != 0 ||
         (!(isNotNull0 = t0.isNotNull()) &&
         (forceVmExc & ClassDefinition.NULL_PTR_EXC) != 0))
     {
      if (!isVoidExpr)
      {
       oc.cPrint("(");
      ...
      {
       oc.cPrint(isNotNull0 ? "JCGO_ARRAY_NZOBJSET(" : "JCGO_ARRAY_OBJSET(");
       oc.cPrint(isVoidExpr ? Type.cName[Type.VOID] : cname);
       oc.cPrint(", ");
      }
     if (rcvr1 > 0)
     ...
     if (rcvr2 > 0)
      oc.cPrint(rcvr1 > 0 ? rcvrStr2 : rcvrStr1);
      else t1.processOutput(oc);
     oc.cPrint(", ");
     if (rcvr3 > 0)
      oc.cPrint(rcvr1 > 0 && rcvr2 > 0 ? rcvrStr3 : (rcvr1 | rcvr2) > 0 ?
       rcvrStr2 : rcvrStr1);
      else
      {
       oc.assignmentRightType = assignRightType;
     ...
     else
     {
      ...
      if (forceCheck)
       oc.cPrint("[0]");
      oc.cPrint("= ");
      if (rcvr3 > 0 ? s0 >= Type.CLASSINTERFACE :
          s0 == Type.BOOLEAN || exprType0 != exprType2)
      {
       oc.cPrint("(");
       oc.cPrint(exprType0.castName());
       oc.cPrint(")");
   ...
   else
   {
        ...
        sym == LexTerm.SHLEFT_EQUALS)
    {
     if ((rcvr1 | rcvr2 | rcvr3) > 0)
     {
      ...
      oc.cPrint(t1 == null || rcvr1 == 0 || rcvr2 == 0 ? rcvrStr1 : rcvrStr2);
     }
      else
      {
       assertCond(!isVolatile);
     ...
     {
      MethodDefinition md =
       Main.dict.get(Names.JAVA_LANG_VMSYSTEM).getMethod(Names.SIGN_CONCAT0X);
      ...
      else
      {
       ...
       if (sym != LexTerm.MINUS_EQUALS && sym != LexTerm.PLUS_EQUALS &&
           sym != LexTerm.TIMES_EQUALS)
        BinaryOp.outputDivShift(oc, sym == LexTerm.DIVIDE_EQUALS ?
         LexTerm.DIVIDE : sym == LexTerm.MOD_EQUALS ? LexTerm.MOD :
         sym == LexTerm.FLSHIFT_EQUALS ? LexTerm.FILLSHIFT_RIGHT :
         sym == LexTerm.SHLEFT_EQUALS ? LexTerm.SHIFT_LEFT :
         LexTerm.SHIFT_RIGHT, s0 >= s2 || (sym != LexTerm.DIVIDE_EQUALS &&
         sym != LexTerm.MOD_EQUALS) ? s0 : s2);
       if (s2 >= Type.LONG && s0 < s2 && sym != LexTerm.FLSHIFT_EQUALS &&
           sym != LexTerm.SHRIGHT_EQUALS && sym != LexTerm.SHLEFT_EQUALS)
       {
        oc.cPrint("(");
     ...
     if (rcvr2 > 0 && (t1 == null || rcvr3 > 0))
      ...
      else
      {
       ...
       {
        oc.cPrint("*(");
        oc.cPrint(isString ? Type.cName[Type.CLASSINTERFACE] : cname);
        oc.cPrint("*)");
       }
       oc.cPrint((rcvr1 > 0 && rcvr2 > 0) || rcvr3 > 0 ? rcvrStr2 : rcvrStr1);
      }
     oc.assignmentRightType = assignRightType;
     if (isString)
     {
      ...
      else
      {
       oc.cPrint(sym == LexTerm.MINUS_EQUALS ? "- " :
        sym == LexTerm.PLUS_EQUALS ? "+ " : sym == LexTerm.TIMES_EQUALS ?
        "* " : ", ");
       if (sym == LexTerm.FLSHIFT_EQUALS || sym == LexTerm.SHRIGHT_EQUALS ||
           sym == LexTerm.SHLEFT_EQUALS)
       {
        ...
        else
        {
         if (s0 >= Type.LONG && s0 > s2)
         {
          ...
          else if (sym == LexTerm.DIVIDE_EQUALS || sym == LexTerm.MOD_EQUALS)
           t2.processOutput(oc);
           else t2.atomaryOutput(oc);
        }
      }
     oc.cPrint(")");
     if (!isString && s2 >= Type.FLOAT && s0 < s2 &&
 ...
 ExpressionType traceClassInit()
 {
  ...
  VariableDefinition v = terms[0].getVariable(false);
  if (v != null)
   v.setTraceExprType(curTraceType2 != null ? curTraceType2 : actualType2,
    false);
