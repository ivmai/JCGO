...
final class CondExpression extends LexNode
{
 ...
 void processPass1(Context c)
 {
  if (resType == null)
  {
   ...
   if (s1 != Type.BOOLEAN ? (s1 < Type.BYTE || s1 > Type.VOID ?
   ...
   if (s1 >= Type.BYTE && s1 <= Type.DOUBLE)
   {
    ...
    if (s1 > s2)
    {
     ...
     if (s1 == Type.INT &&
         ...
         (constVal1 = constVal1.isEqual(constVal1.castTo(s2))) != null &&
         constVal1.isNonZero())
     {
      resType = exprType2;
      castLeft = true;
     ...
     else
     {
      ...
      if (s2 == Type.INT &&
          ...
          (constVal2 = constVal2.isEqual(constVal2.castTo(s1))) != null &&
          constVal2.isNonZero())
      {
       resType = exprType1;
       castLeft = true;
   ...
   if (ClassDefinition.isAssignableFrom(exprType2, exprType1, c.forClass))
   {
    resType = exprType2;
    castLeft = true;
   }
 ...
 int tokenCount()
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].tokenCount() :
          terms[0].tokenCount() + terms[1].tokenCount() +
          terms[2].tokenCount() + 1;
 }

 int tokensExpandedCount()
 {
  return constVal0 != null ? terms[constVal0.isNonZero() ?
          1 : 2].tokensExpandedCount() : super.tokensExpandedCount();
 }

 ConstValue evaluateConstValue()
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].evaluateConstValue() : null;
 }

 boolean isFPZero()
 {
  assertCond(resType != null);
  return constVal0 != null && terms[constVal0.isNonZero() ? 1 : 2].isFPZero();
 }
 ...
 ExpressionType actualExprType()
 {
  ...
  if (s0 == Type.CLASSINTERFACE || s0 == Type.OBJECTARRAY)
  {
   ...
   ExpressionType actualType =
    ClassDefinition.maxCommonExprOf(terms[1].actualExprType(),
    terms[2].actualExprType(), forClass);
 ...
 boolean isLiteral()
 {
  assertCond(resType != null);
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isLiteral() :
          terms[0].isLiteral() && terms[1].isLiteral() &&
          terms[2].isLiteral();
 }
 ...
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isImmutable() :
          terms[0].isImmutable() && terms[1].isImmutable() &&
          terms[2].isImmutable();
 }
 ...
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isSafeExpr() :
          terms[0].isSafeExpr() && terms[1].isSafeExpr() &&
          terms[2].isSafeExpr();
 }
 ...
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isSafeWithThrow() :
          terms[0].isSafeWithThrow() && terms[1].isSafeWithThrow() &&
          terms[2].isSafeWithThrow();
 }

 boolean isFieldAccessed(VariableDefinition v)
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isFieldAccessed(v) :
          terms[0].isFieldAccessed(v) || terms[1].isFieldAccessed(v) ||
          terms[2].isFieldAccessed(v);
 }

 boolean isAnyLocalVarChanged(Term t)
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isAnyLocalVarChanged(t) :
          terms[0].isAnyLocalVarChanged(t) ||
 ...
 {
  return constVal0 != null ?
          terms[constVal0.isNonZero() ? 1 : 2].isNotNull() :
          terms[1].isNotNull() && terms[2].isNotNull();
 }

 VariableDefinition getVariable(boolean allowInstance)
 {
  if (constVal0 != null)
   return terms[constVal0.isNonZero() ? 1 : 2].getVariable(allowInstance);
 ...
 ExpressionType classLiteralValGuess()
 {
  ...
  if (exprType2 == null)
   return exprType1;
  ExpressionType exprType =
   ClassDefinition.maxCommonExprOf(exprType1, exprType2, forClass);
 ...
 {
  if (constVal0 != null)
   return terms[constVal0.isNonZero() ? 1 : 2].getClassNewInstanceCall();
 ...
 {
  if (constVal0 != null)
   return terms[constVal0.isNonZero() ? 1 : 2].getConstructorInstanceSign();
 ...
 ExpressionType traceClassInit()
 {
  ...
  {
   ExpressionType curType = ClassDefinition.maxCommonExprOf(curTraceType1,
                             curTraceType2, null);
