 ...
 **
 * Formats:
 * LPAREN Expression(Name) RPAREN UnaryExpressionNotPlusMinus
 * LPAREN Expression(PrimitiveType) RPAREN UnaryExpression
 * LPAREN Expression(TypeWithDims) RPAREN UnaryExpressionNotPlusMinus
 */
...
final class CastExpression extends LexNode
{
 ...
 Term processPassOneInner(Context c, boolean processSecondArg)
 {
  ...
  if (t0.isType())
  {
   ...
   if (s0 != Type.BOOLEAN ? (s0 < Type.BYTE || s0 > Type.VOID ?
       s1 >= Type.BOOLEAN && s1 <= Type.VOID : s0 == Type.VOID ||
       s1 <= Type.BOOLEAN || s1 >= Type.VOID) : s1 != Type.BOOLEAN)
   ...
   if (s1 == Type.CLASSINTERFACE || s1 == Type.OBJECTARRAY)
   {
    ...
    if (ClassDefinition.isAssignableFrom(exprType0, actualType0, c.forClass))
     actualType1 = actualType0;
     else actualType0 = exprType0;
   }
    else
    {
     if (s1 == Type.NULLREF || (s0 == Type.CLASSINTERFACE &&
         exprType0.signatureClass().superClass() == null))
     {
   ...
   else
   {
    ...
    actualType0 = exprType0;
    if (ClassDefinition.isAssignableFrom(exprType0, actualType1, c.forClass))
 ...
 ConstValue evaluateConstValue()
 {
  ...
  return constVal != null ? constVal.castTo(exprType0.objectSize()) : null;
 }
 ...
 boolean isLiteral()
 {
  assertCond(exprType0 != null);
  int s1;
  return t1IsLiteral() && (actualType1 != null ||
          ((s1 = exprType1.objectSize()) != Type.FLOAT &&
          s1 != Type.DOUBLE) || exprType0.objectSize() >= s1);
 }

 boolean isImmutable()
 {
  assertCond(exprType0 != null);
  return terms[1].isImmutable() && (actualType0 == actualType1 ||
 ...
 {
  assertCond(exprType0 != null);
  return terms[1].isSafeExpr() && (actualType0 == actualType1 ||
          exprType1.objectSize() <= Type.DOUBLE || t1IsLiteral());
 }
 ...
 String strLiteralValueGuess()
 {
  ...
  return exprType0 == Main.dict.get(Names.JAVA_LANG_OBJECT) ||
          exprType0 == Main.dict.get(Names.JAVA_LANG_STRING) ?
          terms[1].strLiteralValueGuess() : null;
 }
 ...
 ExpressionType classLiteralValGuess()
 {
  assertCond(exprType0 != null);
  return exprType0 == Main.dict.get(Names.JAVA_LANG_OBJECT) ||
          exprType0 == Main.dict.get(Names.JAVA_LANG_CLASS) ?
          terms[1].classLiteralValGuess() : null;
 }
 ...
 MethodInvocation getClassNewInstanceCall()
 {
  assertCond(exprType0 != null);
  return exprType0 == Main.dict.get(Names.JAVA_LANG_OBJECT) ?
          terms[1].getClassNewInstanceCall() : null;
 }
 ...
 MethodSignature getConstructorInstanceSign()
 {
  assertCond(exprType0 != null);
  return exprType0 == Main.dict.get(Names.JAVA_LANG_OBJECT) ?
          terms[1].getConstructorInstanceSign() : null;
 }
 ...
 static void outputFloatCast(OutputContext oc, int s0, int s1)
 {
  oc.cPrint(s1 > Type.FLOAT ? (s0 <= Type.INT ? "JCGO_JDOUBLE_TOJINT" :
   s0 < Type.FLOAT ? "JCGO_JDOUBLE_TOJLONG" : "JCGO_JDOUBLE_TOJFLOAT") :
 ...
 void processOutput(OutputContext oc)
 {
  ...
  if (actualType1 != null)
  {
   ...
   else
   {
    ...
    cd = null;
    if ((s1 != Type.FLOAT && s1 != Type.DOUBLE) || s0 >= s1 || s0 < Type.INT)
    {
     oc.cPrint("(");
     oc.cPrint(exprType0.castName());
     oc.cPrint(")");
  ...
  if (exprType0.hasRealInstances())
  {
   ...
   else
   {
    dims = 0;
    full = false;
    if (!cd.used() || cd.isInterface() || exprType0.signatureDimensions() > 0)
 ...
 ExpressionType traceClassInit()
 {
  ...
  if (!terms[0].isType())
  {
   ...
   if (!cd.name().equals(Names.JAVA_LANG_OBJECT) &&
    ...
    cd.traceReflectedConstructor(false, msig.signatureString(), false);
  }
  if (curTraceType1 != null &&
      (actualType0 == curTraceType1 || !actualType0.hasRealInstances() ||
      !ClassDefinition.isAssignableFrom(actualType0, curTraceType1, null)))
   curTraceType1 = null;
