...
final class BranchContext
{
 ...
 ExpressionType getActualType(VariableDefinition v)
 {
  ...
  return index >= 0 ? (ExpressionType) varActualTypes.elementAt(index) : null;
 }
 ...
 void intersectWith(BranchContext otherBranch, ClassDefinition forClass)
 {
  ...
  for (int i = 0; i < count; i++)
  {
   VariableDefinition v =
    (VariableDefinition) otherBranch.actTypeVars.elementAt(i);
   int index = actTypeVars.identityLastIndexOf(v);
   if (index >= 0)
   {
    ExpressionType actualType =
     ClassDefinition.maxCommonExprOf(
     (ExpressionType) varActualTypes.elementAt(index),
     (ExpressionType) otherBranch.varActualTypes.elementAt(i), forClass);
    if (!ClassDefinition.isAssignableFrom(v.exprType(), actualType, forClass))
  ...
  while (actTypeVars.size() > index)
   if (otherBranch.actTypeVars.identityLastIndexOf(
       actTypeVars.elementAt(index)) < 0)
   {
   ...
   boolean isThis)
 {
  ...
  if (ignoreLocals)
  {
   ...
   else vectorUnion(nonNullVars, otherBranch.nonNullVars);
 ...
 private static void vectorUnionDual(ObjVector vect, ObjVector vect2,
   ObjVector other, ObjVector other2, boolean ignoreLocals, boolean isThis)
 {
  int count = other.size();
  for (int i = 0; i < count; i++)
  {
   VariableDefinition v = (VariableDefinition) other.elementAt(i);
   if ((!ignoreLocals || (!v.isLocalOrParam() &&
       (isThis || v.isClassVariable()))) && vect.identityLastIndexOf(v) < 0)
   {
    vect.addElement(v);
