 ...
 **
 * Format:
 * RelationalExpression INSTANCEOF PrimitiveType/ClassOrIfaceType [Dims]
 */
...
final class InstanceOf extends LexNode
{
 ...
 void processPass1(Context c)
 {
  if (cd == null)
  {
   ...
   if (cd.isAssignableFrom(actualType0.signatureClass(),
       dims - actualType0.signatureDimensions(), c.forClass))
   {
 ...
 void updateCondBranch(Context c, boolean forTrue)
 {
  ...
  if (forTrue && (v = terms[0].getVariable(false)) != null)
  {
   ...
   if (!ClassDefinition.isAssignableFrom(exprType, actualType0, c.forClass))
 ...
 void processOutput(OutputContext oc)
 {
  ...
  if (dims == 0 && cd.name().equals(Names.JAVA_LANG_OBJECT))
  {
   ...
   else
   {
    ...
    if (!full && !cd.hasInstantatedSubclasses(false) && terms[0].isNotNull())
    {
