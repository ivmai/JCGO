...
class TryStatement extends LexNode
{
  ...
  if (terms[1].notEmpty() || terms[2].notEmpty())
  {
   ...
   if (terms[1].notEmpty())
   {
    ...
    while (en.hasMoreElements())
     c.forceVmExc |= ((ClassDefinition) en.nextElement()).getVMExcMask();
   }
   ...
   if (oldChangedVars != null && changedVars != null && !terms[2].notEmpty())
   {
 ...
 {
  return terms[2].notEmpty() ? terms[2].hasTailReturnOrThrow() :
          terms[0].hasTailReturnOrThrow() && terms[1].hasTailReturnOrThrow();
 }

 void discoverObjLeaks()
 {
  terms[0].discoverObjLeaks();
  if (terms[2].notEmpty() ? Main.dict.get(Names.JAVA_LANG_THROWABLE).used() :
      hasExceptionInstances())
  {
   ...
   if (accessedOuterObjLocals != null)
   {
    Enumeration en = accessedOuterObjLocals.elements();
    while (en.hasMoreElements())
     ((VariableDefinition) en.nextElement()).setStackObjVolatile();
    if (isThisAccessed)
    {
 ...
 final boolean outputFinallyGroupInner(OutputContext oc, TryStatement lastTry,
   boolean allowBreakThrow)
 {
  if (lastTry == this)
   return false;
 ...
 boolean isReturnAtEnd(boolean allowBreakThrow)
 {
  return terms[2].notEmpty() ? terms[2].isReturnAtEnd(allowBreakThrow) :
          terms[0].isReturnAtEnd(allowBreakThrow) && !hasExceptionInstances();
 }

 void processOutput(OutputContext oc)
 {
  if (terms[2].notEmpty() ? Main.dict.get(Names.JAVA_LANG_THROWABLE).used() :
      (!terms[1].isSwitchMapAssign(false) ||
      !terms[0].isSwitchMapAssign(false)) && hasExceptionInstances())
  {
