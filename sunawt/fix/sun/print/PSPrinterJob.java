...
/*
 * @(#)PSPrinterJob.java        1.46 03/01/23
 *
    ...
    private static String escapeParens(String str) {
     ...
     protected boolean textOut(Graphics g, String str, float x, float y) {
        ...
        if (mFontProps == null) {
            ...
            mPSStream.println(x + " " + y +
                              " moveto(" + escapeParens(str) + ")show");
        } else {

            prepDrawing(null);

            FontMetrics curMetrics = g.getFontMetrics();
            CharsetString[] acs =
                ((PlatformFont)
            ...
            if (psFonts != null) {
                ...
                for (int i = 0; i < acs.length; i++){
                    ...
                    Rectangle2D r2d =
                        curMetrics.getStringBounds(cs.charsetChars,
                                                   cs.offset,
                                                   cs.offset+cs.length, g);
                    ...
                    nativeStr.append('>');
                    mPSStream.println("/testString " + nativeStr.toString() + " D");
                    mPSStream.println("% Font[" + mLastFont.getName() + ", " +
                               PlatformFont.styleStr(mLastFont.getStyle()) + ", "
                               + mLastFont.getSize2D() + "]");
                    getGState().emitPSFont(psFonts[i], mLastFont.getSize2D(), desiredWidth);

                    // out String
                    mPSStream.println(nativeStr.toString());
                    mPSStream.println(" " + desiredWidth + " " + x + " " + y);
                    mPSStream.println(DrawStringName);
                    x += desiredWidth;
                }
            } else {
